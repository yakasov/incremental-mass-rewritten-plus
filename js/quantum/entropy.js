const ENTROPY = {
  ...
  gain() {
    ...
    if (player.md.break.upgs[6].gte(1))
      x = x.mul(tmp.bd.upgs[6].eff ? tmp.bd.upgs[6].eff[0] : 1);
...
function calcEntropy(dt) {
  ...
  if (player.qu.en.eth[0]) {
    ...
    let s = player.supernova.radiation.hz
      .div(
        player.supernova.radiation.hz
          .max(1)
          .pow(dt)

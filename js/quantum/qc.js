const QCs = {
    ...
    getMod(x) {
        return CHALS.inChal(15)
            ? [10, 5, 10, 10, 10, 10, 10, 10][x]
            : tmp.c16active || inDarkRun()
              ? 8
              : CHALS.inChal(14)
                ? 5
                : player.qu.rip.active
    ...
    ctn: [
        {
            ...
            effDesc(x) {
                return `<b>^${format(
                    x[0],
                )}</b> to exponent of all-star resources.<br><b>^${format(
                    x[1],
        ...
        },
        {
            ...
            effDesc(x) {
                return `<b>^${format(
                    x[0],
                )}</b> to starting of pre-Quantum scaling.<br><b>${format(
                    x[1] * 100,
...
function addQCPresetAs() {
    if (player.qu.qc.presets.length >= 5) {
        addNotify(
            "You cannot add QC Preset because of maxmium length of presets",
        );
        return;
    }

const QCs = {
  active() {
    return tmp.qu.qc.force != undefined || player.qu.qc.active;
  },
  getMod(x) {
    return (tmp.qu.qc.force || player.qu.qc.mods)[x] || 0;
  },
  incMod(x, i) {
    if (!this.active())
      player.qu.qc.mods[x] = Math.min(
        Math.max(player.qu.qc.mods[x] + i, 0),
        10
      );
  },
  enter() {
    ...
    if (player.qu.qc.active) CONFIRMS_FUNCTION.enterQC();
      ...
      createConfirm(
        "Are you sure to enter the Quantum Challenge? Entering it will force reset!",
        "qc",
        CONFIRMS_FUNCTION.enterQC
      );
  },
  names: [
    "Black Dwarf",
  ...
  ctn: [
    {
      ...
      effDesc(x) {
        return `<b>${formatPow(
          x[0]
        )}</b> to exponent of all-star resources.<br><b>${formatPow(
          x[1]
        )}</b> to strength of star generators.`;
      },
    },
    {
      unl: () => EVO.amt < 4,
    ...
    },
    {
      ...
      effDesc(x) {
        return `<b>${formatMult(x)}</b> to requirements of any Fermions.`;
      },
    },
    {
      eff: (i) => 0.9 ** (i ** 1.25),
      effDesc: (x) =>
        EVO.amt >= 4
          ? `<b>${formatMult(x)}</b> to Protostars.`
          : `<b>${formatPow(
              x
            )}</b> to multiplier from Bosonic & Radiation resources.`,
    },
    {
      eff: (i) => 0.8 ** (i ** 1.25),
      effDesc: (x) =>
        `<b>${formatPow(
          x
        )}</b> to multiplier from pre-Supernova resources, except all star resources.`,
    },
    {
      shown: () => EVO.amt < 4,
      eff(i) {
      ...
      effDesc(x) {
        return `<b>${formatMult(
          x
        )}</b> to requirements of any pre-Quantum Challenge.`;
      },
    },
    {
      unl: () => EVO.amt < 4,
      ...
      effDesc(x) {
        return `<b>^${format(x)}</b> to Mass Dilation’s penalty.`;
      },
    },
    {
      unl: () => EVO.amt < 4,
      eff(i) {
        ...
        if (hasCharger(7) && EVO.amt >= 2 && tmp.c16.in)
          i *= getEvo2Ch8Boost().toNumber();
      ...
      effDesc(x) {
        return `<b>${formatPow(
          x[0]
        )}</b> to starting of pre-Quantum scaling.<br><b>${formatPercent(
          x[1]
        )}%</b> to strength of pre-Quantum scaling.`;
      },
    },
    {
      shown: () => EVO.amt >= 4,
...
function deleteQCPreset(x) {
  createConfirm(
    ...
    () => {
      let represets = [];
      for (let y = 0; y < player.qu.qc.presets.length; y++)
        if (x != y) represets.push(player.qu.qc.presets[y]);
...
function updateQCTemp() {
  ...
  tqc.s_b = E(2);
  if (hasTree("qf4")) tqc.s_b = tqc.s_b.add(0.5);
  if (hasPrestige(0, 2)) tqc.s_b = tqc.s_b.add(0.5);
  ...
  if (hasZodiacUpg("gemini", "u3") && player.qu.rip.active)
    m_mult *= zodiacEff("gemini", "u3");
...
function updateQCHTML() {
  tmp.el.qc_shard.setTxt(
    player.qu.qc.shard +
      (tmp.qu.qc.s + tmp.qu.qc.s_bonus != player.qu.qc.shard
        ? ` (${
            tmp.qu.qc.s + tmp.qu.qc.s_bonus >= player.qu.qc.shard ? "+" : ""
          }${tmp.qu.qc.s + tmp.qu.qc.s_bonus - player.qu.qc.shard})`
        : "")
  ...
  for (let x = 0; x < 2; x++)
    tmp.el["qc_tab" + x].setDisplay(tmp.qu.qc.tab == x);
  if (tmp.qu.qc.tab == 0) {
    tmp.el.qc_btn.setDisplay(!tmp.qu.rip.in);
    tmp.el.qc_btn.setTxt(
      (QCs.active() ? "Exit" : "Enter") + " the Quantum Challenge"
    );
    for (let x = 0; x < QCs_len; x++) {
      let ctn = QCs.ctn[x];
      tmp.el["qcm_mod" + x].setTxt(QCs.getMod(x));
      tmp.el["qcm_div" + x].setDisplay(!ctn.shown || ctn.shown());
      tmp.el["qcm_btns" + x].setDisplay(
        !QCs.active() && (!ctn.unl || ctn.unl())
      );

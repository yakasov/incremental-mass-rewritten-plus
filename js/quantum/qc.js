const QCs = {
  active() {
    return tmp.qc_force != undefined || player.qu.qc.active;
  },
  getMod(x) {
    return (tmp.qc_force || player.qu.qc.mods)[x];
  },
  incMod(x, i) {
    if (!this.active())
  ...
  ctn: [
    ...
    },
    {
      ...
      effDesc(x) {
        return `<b>/${format(x, 0)}</b> to pre-Quantum global speed.`;
      },
    },
    {
      eff(i) {
      ...
      effDesc(x) {
        return `<b>x${format(x)}</b> to requirements of any Fermions.`;
      },
    },
    {
      eff(i) {
      ...
      effDesc(x) {
        return `<b>^${format(x)}</b> to multiplier from Bosonic & Radiation resources.`;
      },
    },
    {
      eff(i) {
      ...
      effDesc(x) {
        return `<b>^${format(x)}</b> to multiplier from pre-Supernova resources, except all star resources.`;
      },
    },
    {
      eff(i) {
      ...
      effDesc(x) {
        return `<b>x${format(x)}</b> to requirements of any pre-Quantum Challenge.`;
      },
    },
    {
      eff(i) {
    ...
    },
    {
      eff(i) {
        ...
        if (hasCharger(7) && OURO.evo >= 2 && tmp.c16.in)
          i *= getEvo2Ch8Boost().toNumber();
        let x = [Math.max(0, 1 - 0.05 * i), i / 10 + 1];
        return x;
      },
      effDesc(x) {
        return `<b>^${format(x[0])}</b> to starting of pre-Quantum scaling.<br><b>${format(x[1] * 100)}%</b> to strength of pre-Quantum scaling.`;
      },
    },
  ],
};
...
function updateQCTemp() {
  ...
  tmp.qu.qc_s_b = E(2);
  if (hasTree("qf4")) tmp.qu.qc_s_b = tmp.qu.qc_s_b.add(0.5);
  if (hasPrestige(0, 2)) tmp.qu.qc_s_b = tmp.qu.qc_s_b.add(0.5);
  ...
  tmp.el.qc_shard.setTxt(
    player.qu.qc.shard +
      (tmp.qu.qc_s + tmp.qu.qc_s_bouns != player.qu.qc.shard
        ? ` (${tmp.qu.qc_s + tmp.qu.qc_s_bouns >= player.qu.qc.shard ? "+" : ""}${tmp.qu.qc_s + tmp.qu.qc_s_bouns - player.qu.qc.shard})`
        : ""),
  );

const BIG_RIP = {
  rip() {
    if (!player.qu.rip.active && player.confirms.br)
      createConfirm(
        `Are you sure you want to Big Rip the Dimension?`,
        "br",
        CONFIRMS_FUNCTION.bigRip
      );
  ...
  gain() {
    ...
    if (player.md.break.upgs[6].gte(1))
      x = x.mul(tmp.bd.upgs[6].eff ? tmp.bd.upgs[6].eff[1] : 1);

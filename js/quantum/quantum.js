const QUANTUM = {
  ...
  gainTimes() {
    ...
    if (tmp.qu.mil_reached[7] && EVO.amt >= 4)
      x = x.mul(player.qu.points.add(1).log10());
    if (tmp.qu.mil_reached[8] && EVO.amt >= 4)
      x = x.mul(E(2).pow(player.qu.qc.shard));
    return x;
  },
  enter(auto = false, force = false, rip = false) {
    if (tmp.qu.gain.gte(1) || force)
      getResetConfirm("qu", () => CONFIRMS_FUNCTION.qu(auto, force, rip));
  },
  performReset(force, rip) {
    if (!force) {
      ...
      if (player.qu.qc.active) {
        player.qu.qc.shard = Math.max(
          player.qu.qc.shard,
          tmp.qu.qc.s + tmp.qu.qc.s_bonus
        );
  ...
  doReset(force = false, dark = false, metaF = false) {
    ...
    if (!hasTree("ct8")) {
      ...
      for (let x = 0; x < tmp.sn.tree_had.length; x++)
        if (TREE_UPGS.ids[tmp.sn.tree_had[x]].qf) keep.push(tmp.sn.tree_had[x]);
      if (tmp.qu.mil_reached[2])
        keep.push(
          "chal1",
          "chal2",
          "chal3",
      ...
      if (!c16 && hasUpgrade("br", 6) && !keep.includes("unl1"))
        keep.push("unl1");
    ...
    player.supernova.radiation.hz =
      !c16 && hasUpgrade("br", 6) ? E(1e50) : E(0);
    for (let x = 0; x < 7; x++) {
    ...
    for (let x = 1; x <= 12; x++)
      if (!hasTree("qu_qol7") || x <= 8 || force || dark)
        if (!hasElement(122) || x != 12 || dark) player.chal.comps[x] = E(0);
  ...
  bpGain() {
    ...
    if (tmp.qu.mil_reached[5])
      x = x.mul(tmp.qu.speed.max(1).root(2).softcap(1e50, 0.95, 2));
  ...
  bpEff() {
    let x = hasElement(101)
      ? player.qu.bp
          .add(1)
          .log10()
          .add(1)
          .tetrate(hasUpgrade("br", 15) ? 1.35 : 1.25)
      : player.qu.bp.add(1).log10().add(1).pow(1.5);
    return x;
  },
  cosmic_str: {
    buy() {
  ...
  mils: [
    ...
    [
      E(8),
      `Pre-Quantum global speed affects Blueprint Particles and Chroma at a reduced rate.`,
    ],
    [
      E(10),
      `Supernova stars are boosted by Quantizes (capped at 1e10). Unlock Auto-Quantum.`,
    ],
    [
      E(20),
      `Unlock Primordium. Quantum Foam boosts Quantizes at a logarithmic rate.`,
    ],
  ...
  auto: {
    mode: ["Amount", "Time"],
    switch() {
      player.qu.auto.enabled = !player.qu.auto.enabled;
      player.qu.auto.time = 0;
    },
    switchMode() {
      player.qu.auto.mode = (player.qu.auto.mode + 1) % this.mode.length;
      player.qu.auto.time = 0;
    },
    temp() {
      let n,
        i = player.qu.auto.input;
...
function quUnl() {
  return EVO.amt < 5 && player.qu.times.gte(1);
}
...
function calcQuantum(dt) {
  ...
  if (quUnl()) {
    ...
    if (player.qu.auto.enabled) {
      ...
      if (player.qu.auto.mode == 0) can = tmp.qu.gain.gte(tmp.qu.auto_input);
        ...
        can = player.qu.auto.time >= tmp.qu.auto_input;
    ...
    if (hasUpgrade("br", 8)) {
      ...
      if (player.qu.rip.active || hasElement(147))
        player.qu.rip.amt = player.qu.rip.amt.add(
          tmp.qu.rip.gain.mul(inf_gs).div(10)
        );
    }

    if (hasElement(139))
      player.qu.times = player.qu.times.add(tmp.qu.gainTimes.mul(inf_gs));
  }
  ...
  if (tmp.sn.unl && (hasTree("qu_qol1") || hasInfUpgrade(4)))
    for (let x = 0; x < TREE_TYPES.normal.length; x++)
      TREE_UPGS.buy(TREE_TYPES.normal[x], true);
...
function updateQuantumTemp() {
  ...
  tmp.qu.theories = player.qu.times
    .sub(player.qu.chr_get.length)
    .max(0)
    .min(3)
    .toNumber();
  ...
  for (let x = 0; x < QUANTUM.mils.length; x++)
    tmp.qu.mil_reached[x] = player.qu.times.gte(QUANTUM.mils[x][0]);

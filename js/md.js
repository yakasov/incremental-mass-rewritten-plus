const MASS_DILATION = {
  unlocked() {
    return hasElement(21);
  },
  penalty() {
    let x = 0.8;
    if (FERMIONS.onActive("02")) x **= 2;
    if (QCs.active() && (player.md.break.active ? !player.qu.rip.active : true))
      x **= tmp.qu.qc_eff[6];
    return x;
  },
  onactive() {
    ...
    if (player.md.active)
      player.md.particles = player.md.particles.add(tmp.md.rp_gain);
  ...
  upgs: {
    ...
    ids: [
        ...
        effect(x) {
          if (tmp.md.bd3)
            return x
              ...
              .root(
                player.qu.rip.active || tmp.c16active || inDarkRun() ? 3 : 2
              )
              .mul(
      ...
      },
      {
        ...
        cost(x) {
          return E(10)
            .pow(x.pow(E(1.25).pow(tmp.md.upgs[4].eff || 1)))
            .mul(1000);
        },
        bulk() {
        ...
        effDesc(x) {
          return (
            format(x, 0) +
            "x" +
            (x.gte(1e25) ? " <span class='soft'>(softcapped)</span>" : "")
          );
        },
      },
      {
        ...
        effect(x) {
          return player.md.mass.max(1).log(100).root(3).div(8).add(1);
        },
        effDesc(x) {
          return format(x) + "x";
        },
      ...
      {
        unl() {
          return STARS.unlocked() || player.supernova.times.gte(1);
        },
      ...
      },
      {
        ...
        effect(x) {
          return E(2)
            .pow(x)
            .softcap(1e25, 2 / 3, 0); //.softcap("ee12",0.8,2)
        },
        effDesc(x) {
          return (
            format(x) +
      ...
      },
      {
        ...
        effect(i) {
          if (player.md.break.upgs[3].gte(1))
            i = i.pow(1.5).softcap(1e18, 1 / 1.5, 0);
        ...
        effDesc(x) {
          return (
            "+" +
            format(x) +
            (x.gte(0.2) ? " <span class='soft'>(softcapped)</span>" : "")
          );
        },
      },
      {

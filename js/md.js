const MASS_DILATION = {
  unlocked() {
    return hasElement(21) && OURO.evo < 3;
  },
  penalty() {
    let x = 0.8;
    if (FERMIONS.onActive("02")) x **= 2;
    if (QCs.active() && (brokeDil() ? !player.qu.rip.active : true))
      x **= tmp.qu.qc_eff[6];
    return x;
  },
  onactive() {
  ...
  RPexpgain() {
    let x = E(2)
      .add(mdEff(5, false, 0))
      .mul(tmp.chal && !CHALS.inChal(10) ? tmp.chal.eff[10] : 1);
    if (!inMD() && hasTree("d1")) x = x.mul(1.25);
    if (FERMIONS.onActive("01")) x = x.div(10);
    if (QCs.active()) x = x.mul(tmp.qu.qc_eff[4]);
    if (hasElement(24) && hasPrestige(0, 40))
      x = x.mul(tmp.elements.effect[24]);
  ...
  RPgain(m = player.mass) {
    ...
    let x = m
      ...
      .sub(14)
      .max(0)
      .pow(tmp.md.rp_exp_gain)
      .mul(tmp.md.rp_mult_gain);
    ...
    let x = tmp.md.bd3
      ? player.qu.rip.active
        ? player.md.mass
            ...
            .add(1)
            .root(2)
        : player.md.mass.max(1).log10().max(1).log10().add(1).root(3)
      : player.md.mass.max(1).log10().add(1).root(3);
    x = x.mul(mdEff(1));
    return x;
  },
  upgs: {
    buy(x) {
      if (tmp.md.upgs[x].can) {
        if (!hasElement(43))
          player.md.mass = player.md.mass
            .sub(this.ids[x].cost(tmp.md.upgs[x].bulk.sub(1)))
            .max(0);
    ...
    ids: [
      {
        ...
        cost(x) {
          return E(10).pow(x).mul(10);
        },
        bulk() {
          return player.md.mass.gte(10)
            ? player.md.mass.div(10).max(1).log10().add(1).floor()
            : E(0);
        },
        effect(x) {
          let b = 2;
          if (hasElement(25)) b++;
          return E(b)
            .pow(x.mul(tmp.md.upgs[11].eff || 1))
            .softcap("e1.2e4", 0.96, 2)
            .overflow("ee11000", 0.5, 2);
        },
        effDesc(x) {
          return (
            format(x, 0) +
            "x" +
            (x.gte("e1.2e4") ? ` <span class='soft'>(softcapped)</span>` : "")
          );
        },
      },
      {
        ...
        cost(x) {
          return tmp.md.bd3
            ? E(10).pow(E(1.25).pow(x)).mul(100)
            : E(10).pow(x).mul(100);
        },
        bulk() {
          return player.md.mass.gte(100)
            ? tmp.md.bd3
              ? player.md.mass
                  .div(100)
                  .max(1)
                  ...
                  .add(1)
                  .floor()
              : player.md.mass.div(100).max(1).log10().add(1).floor()
            : E(0);
        },
        effect(x) {
          if (tmp.md.bd3)
            return x
              .mul(mdEff(11))
              .root(tmp.rip.in ? 3 : 2)
              .mul(tmp.rip.in ? 0.05 : 0.1)
              .add(1);
          if (hasElement(83)) return expMult(x, 2, 1.5).add(1);
          return hasMDUpg(7)
            ? x.mul(mdEff(11)).root(1.5).mul(0.25).add(1)
            : x
        ...
        effDesc(x) {
          return (
            (x.gte(10) ? format(x) + "x" : format(x.sub(1).mul(100)) + "%") +
            " stronger"
          );
        },
      },
      {
        ...
        cost(x) {
          return E(10)
            .pow(x.pow(E(1.25).pow(mdEff(4))))
            .mul(1000);
        },
        bulk() {
          return player.md.mass.gte(1000)
            ? player.md.mass
                .div(1000)
                .max(1)
                .log10()
                .root(E(1.25).pow(tmp.md.upgs[4].eff || 1))
                .add(1)
                .floor()
            : E(0);
        },
        effect(x) {
          if (OURO.evo < 2)
            return E(2)
              .pow(x.softcap(2.5e26, 0.1, 0).mul(tmp.md.upgs[11].eff || 1))
              .softcap(1e25, 0.75, 0);
      ...
      },
      {
        ...
        maxLvl: 1,
        cost(x) {
          return E(1.619e20).mul(25);
        },
        bulk() {
          return player.md.mass.gte(E(1.619e20).mul(25)) ? E(1) : E(0);
        },
        effect(x) {
      ...
      },
      {
        ...
        maxLvl: 5,
        cost(x) {
          return E(1e5).pow(x).mul(E(1.619e20).mul(1e4));
        },
        bulk() {
          return player.md.mass.gte(E(1.619e20).mul(1e4))
            ? player.md.mass
                .div(E(1.619e20).mul(1e4))
                .max(1)
                .log(1e5)
                .add(1)
                .floor()
      ...
      },
      {
        ...
        cost(x) {
          return E(1e3).pow(x.pow(1.5)).mul(1.5e73);
        },
        bulk() {
          return player.md.mass.gte(1.5e73)
            ? player.md.mass
                .div(1.5e73)
                .max(1)
                .log(1e3)
        ...
        effDesc(x) {
          return (
            "+^" +
            format(x) +
            (x.gte(1e3) ? " <span class='soft'>(softcapped)</span>" : "")
          );
        },
      },
      {
        ...
        bulk() {
          return player.md.mass.gte(1.5e191) ? E(1) : E(0);
        },
        effect(x) {
          return E(5).pow(player.md.mass.max(1).log10().root(2));
        },
        effDesc(x) {
          return format(x) + "x";
        },
      },
      {
        ...
        bulk() {
          return player.md.mass.gte(1.5e246) ? E(1) : E(0);
        },
      },
      {
        unl() {
          return (
            (tmp.star_unl || player.supernova.times.gte(1)) && OURO.evo == 0
          );
        },
        ...
        bulk() {
          return player.md.mass.gte(1.5e296) ? E(1) : E(0);
        },
        effect(x) {
          return player.build.tickspeed.amt.add(1).pow(2 / 3);
        },
        effDesc(x) {
          return format(x) + "x";
        },
      ...
      {
        unl() {
          return tmp.star_unl || player.supernova.times.gte(1);
        },
        desc: `Double quarks gain.`,
        cost(x) {
          return E(5).pow(x).mul("1.50001e536");
        },
        bulk() {
          return player.md.mass.gte("1.50001e536")
            ? player.md.mass.div("1.50001e536").max(1).log(5).add(1).floor()
            : E(0);
        },
        effect(x) {
          let r = E(2).pow(x);
      ...
      },
      {
        ...
        cost(x) {
          return E(1e50).pow(x.pow(1.5)).mul("1.50001e1556");
        },
        bulk() {
          return player.md.mass.gte("1.50001e1556")
            ? player.md.mass
                .div("1.50001e1556")
                .max(1)
                .log(1e50)
      ...
      {
        unl() {
          return player.supernova.post_10 && OURO.evo == 0;
        },
        desc: `First 3 Mass Dilation upgrades are stronger.`,
        cost(x) {
          return E(1e100).pow(x.pow(2)).mul("1.5e8056");
        },
        bulk() {
          return player.md.mass.gte("1.5e8056")
            ? player.md.mass
                .div("1.5e8056")
                .max(1)
                .log(1e100)
  ...
  break: {
    toggle() {
      ...
      if (bd.active)
        createConfirm("Are you sure you want to fix Dilation?", "bd", () => {
          bd.active = false;
    ...
    upgs: {
      buy(x) {
        if (tmp.bd.upgs[x].can) {
          if (!hasElement(123))
            player.md.break.mass = player.md.break.mass
      ...
      ids: [
        {
          ...
          cost(x) {
            return E(10).pow(x.pow(1.1)).mul(1e5);
          },
          bulk() {
            return player.md.break.mass.gte(1e5)
          ...
          effDesc(x) {
            return format(x, 0) + "x" + x.softcapHTML(1e15);
          },
        },
        {
          desc: `Increase the exponent of the Dilated Mass formula.`,
          cost(x) {
            return E(10).pow(x.pow(1.25)).mul(1e7);
          },
          bulk() {
            return player.md.break.mass.gte(1e7)
              ? player.md.break.mass
                  .div(1e7)
                  .max(1)
                  .log10()
        ...
        },
        {
          ...
          bulk() {
            return player.md.break.mass.gte(1.619e23) ? E(1) : E(0);
          },
        },
        {
          ...
          bulk() {
            return player.md.break.mass.gte(1.989e33) ? E(1) : E(0);
          },
        },
        {
          get desc() {
            return hasUpgrade("br", 24)
              ? `Instant-Rank scales later`
              : `Meta-Rank scales later.`;
          },
          cost(x) {
            return E(10).pow(x.pow(2)).mul(1.989e36);
          },
          bulk() {
            return player.md.break.mass.gte(1.989e36)
              ? player.md.break.mass
                  .div(1.989e36)
                  .max(1)
                  .log10()
          ...
          effect(y) {
            let x = hasUpgrade("br", 24)
              ? Decimal.pow(2, y.overflow(1e5, 1 / 3).root(2))
              : y.div(10).add(1);
        ...
        },
        {
          ...
          cost(x) {
            return E(10).pow(x.pow(1.5)).mul(2.9835e48);
          },
          bulk() {
            return player.md.break.mass.gte(2.9835e48)
              ? player.md.break.mass
                  .div(2.9835e48)
                  .max(1)
                  .log10()
        ...
        },
        {
          ...
          bulk() {
            return player.md.break.mass.gte(uni(1e35)) ? E(1) : E(0);
          },
          effect(y) {
            let x = hasElement(220)
              ? [
                  expMult(player.qu.rip.amt, 0.5),
                  expMult(player.qu.en.amt, 1 / 3),
                ]
              : [
                  player.qu.rip.amt.add(1).log10().add(1).pow(2),
                  player.qu.en.amt.add(1).log10().add(1).pow(1.5),
                ];
        ...
        },
        {
          ...
          bulk() {
            return player.md.break.mass.gte(uni(1e48)) ? E(1) : E(0);
          },
          effect(y) {
        ...
        },
        {
          ...
          bulk() {
            return player.md.break.mass.gte(uni(1e100)) ? E(1) : E(0);
          },
          effect(y) {
        ...
        },
        {
          ...
          bulk() {
            return player.md.break.mass.gte(uni(1e120)) ? E(1) : E(0);
          },
        },
        {
          unl: () => player.dark.unl,
          ...
          bulk() {
            return player.md.break.mass.gte(uni(1e240)) ? E(1) : E(0);
          },
        },
        {
          unl: () => player.dark.unl,
...
function updateMDTemp() {
  ...
  for (let x = MASS_DILATION.upgs.ids.length - 1; x >= 0; x--) {
    ...
    tmp.md.upgs[x].can =
      player.md.mass.gte(tmp.md.upgs[x].cost) &&
      player.md.upgs[x].lt(upg.maxLvl || EINF);
    if (upg.effect)
      tmp.md.upgs[x].eff = upg.effect(player.md.upgs[x].mul(mdub));
  }
  ...
  tmp.md.passive_rp_gain = hasTree("qol3")
    ? MASS_DILATION.RPgain(expMult(player.mass, tmp.md.pen))
    : E(0);
...
function updateBDTemp() {
  ...
  for (let x = MASS_DILATION.break.upgs.ids.length - 1; x >= 0; x--) {
    ...
    bd.upgs[x].can =
      player.md.break.mass.gte(bd.upgs[x].cost) &&
      player.md.break.upgs[x].lt(upg.maxLvl || EINF);
...
function updateMDHTML() {
  ...
  tmp.el.md_btn.setTxt(
    inMD()
      ? tmp.md.rp_gain.gte(1)
        ? `Cancel for ${format(tmp.md.rp_gain, 0)} Relativistic particles`
        : `Reach ${formatMass(tmp.md.mass_req)} to gain Relativistic particles, or cancel dilation`
      : "Dilate Mass",
  );
  for (let x = 0; x < MASS_DILATION.upgs.ids.length; x++) {
    ...
    if (unl) {
      tmp.el["md_upg" + x + "_div"].setClasses({
        btn: true,
        full: true,
        md: true,
        locked: !tmp.md.upgs[x].can,
      });
      if ((upg.maxLvl || 1 / 0) > 1)
        tmp.el["md_upg" + x + "_lvl"].setTxt(
          format(player.md.upgs[x], 0) +
            (upg.maxLvl !== undefined ? " / " + format(upg.maxLvl, 0) : ""),
        );
      if (upg.effDesc) {
  ...
  tmp.el.dmOverflow.setHTML(
    `Because of dilated mass overflow at <b>${formatMass(tmp.overflow_start.dm)}</b>, your dilated mass is ${overflowFormat(tmp.overflow.dm || 1)}!`,
  );
}
...
function updateBDHTML() {
  ...
  for (let x = 0; x < MASS_DILATION.break.upgs.ids.length; x++) {
    ...
    if (unl) {
      tmp.el["bd_upg" + x + "_div"].setClasses({
        btn: true,
        full: true,
        bd: true,
        locked: !tmp.bd.upgs[x].can,
        corrupted_text2: x == 2 && c16,
      });
      if ((upg.maxLvl || 1 / 0) > 1)
        tmp.el["bd_upg" + x + "_lvl"].setTxt(
          format(bd.upgs[x], 0) +
            (upg.maxLvl !== undefined ? " / " + format(upg.maxLvl, 0) : ""),
        );
      if (upg.effDesc)
        tmp.el["bd_upg" + x + "_eff"].setHTML(upg.effDesc(tmp.bd.upgs[x].eff));

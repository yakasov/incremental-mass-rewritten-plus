...
const SCALE_TYPE = [
    ...
    "instant",
    "mega",
]; // super, hyper, ultra, meta, exotic
const FULL_SCALE_NAME = [
    "Super",
    "Hyper",
    "Ultra",
    "Meta",
...
const SCALING_RES = {
    rank(x = 0) {
        return player.ranks.rank;
    },
    tier(x = 0) {
        return player.ranks.tier;
    },
    tetr(x = 0) {
        return player.ranks.tetr;
    },
    pent(x = 0) {
        return player.ranks.pent;
    },
    hex(x = 0) {
        return player.ranks.hex;
    },
    tickspeed(x = 0) {
        return player.build.tickspeed.amt;
    },
    massUpg(x = 1) {
        return player.build["mass_" + (x + 1)].amt;
    },
    bh_condenser(x = 0) {
        return player.build.bhc.amt;
    },
    gamma_ray(x = 0) {
        return player.build.cosmic_ray.amt;
    },
    supernova(x = 0) {
        return player.supernova.times;
    },
    fTier(x = 0, y = 0) {
        return player.supernova.fermions.tiers[x][y];
    },
    cosmic_str(x = 0) {
        return player.build.cosmic_string.amt;
    },
    prestige0() {
        return player.prestiges[0];
    },
    prestige1() {
        return player.prestiges[1];
    },
    prestige2() {
        return player.prestiges[2];
    },
    prestige3() {
        return player.prestiges[3];
    },
    prestige4() {
        return player.prestiges[4];
    },
    massUpg4() {
        return player.build.mass_4.amt;
    },
    FSS() {
        return player.dark.matters.final;
    },
    fvm() {
        return player.build.fvm.amt;
    },
    pe() {
        return player.build.pe.amt;
    },
    inf_theorem() {
        return player.inf.theorem;
    },
    ascension0() {
        return player.ascensions[0];
    },
    ascension1() {
        return player.ascensions[1];
    },
    gal_prestige() {
        return player.gal_prestige;
    },
};
    ...
    for (let x = 0; x < SCALE_TYPE.length; x++) {
        ...
        if (player.scaling_ch == x) {
            for (let key in SCALE_START[type]) {
                ...
                if (have) {
                    let p = tmp.scaling_power[type][key],
                        q = Decimal.pow(SCALE_POWER[type][key], p);
                    tmp.el["scaling_" + x + "_" + key + "_power"].setTxt(
                        format(p.mul(100)) +
                            "%, " +
                            (x % 4 == 3
                                ? q.format() + "^"
                    ...
                    tmp.el["scaling_" + x + "_" + key + "_start"].setTxt(
                        format(tmp.scaling_start[type][key], 0),
                    );
                }
            }
        }
...
function updateScalingTemp() {
    for (let x = 0; x < SCALE_TYPE.length; x++) {
        ...
        let sp = tmp.scaling_power[st],
            ss = tmp.scaling_start[st],
            ns = tmp.no_scalings[st];
        ...
        for (let y = 0; y < key.length; y++) {
            ...
            if (noScalings(x, sn)) ns.push(sn);
                ...
                else if (scalingActive(sn, SCALING_RES[sn](), st))
                    tmp.scaling[st].push(sn);
            }
        }
    }
    ...
    if (
        ...
        if (player.mainUpg.br.includes(2))
            sqc8.push("massUpg", "rank", "tier", "tetr", "pent", "hex");
...
function scalingActive(name, amt, type) {
    if (
        tmp.no_scalings[type].includes(name) ||
        SCALE_START[type][name] === undefined
    )
        return false;
    return Decimal.gte(amt, tmp.scaling_start[type][name]);
}

function scaleStart(type, name) {
    return tmp.scaling_start[type][name] || SCALE_START[type][name];
}
...
function getScalingName(name, x = 0, y = 0) {
    ...
    for (let n = cap - 1; n >= 0; n--) {
        if (scalingActive(name, amt, Object.keys(SCALE_START)[n]))
            return (
                capitalFirst(Object.keys(SCALE_START)[n]) +
                (n % 4 == 3 ? "-" : " ")
            );
    }
    return current;
...
function getScalingStart(type, name) {
    ...
    if (type == 0) {
        if (name == "rank") {
            ...
            if (CHALS.inChal(1) || CHALS.inChal(10)) return E(25);
            if (player.mainUpg.bh.includes(3))
                start = start.add(
                    tmp.upgs
                        ? tmp.upgs.main
                            ? tmp.upgs.main[2][3].effect
                            : 0
                        : 0,
                );
            ...
            if (hasBeyondRank(5, 2))
                start = start.add(beyondRankEffect(5, 2, 0));
        ...
        if (name == "tickspeed") {
            ...
            if (player.ranks.tetr.gte(5))
                start = start.add(RANKS.effect.tetr[5]());
        } else if (name == "rank") {
            if (player.mainUpg.atom.includes(10))
                start = start.add(
                    tmp.upgs
                        ? tmp.upgs.main
                            ? tmp.upgs.main[3][10].effect
                            : 0
                        : 0,
                );
        } else if (name == "prestige0") {
            ...
            if (hasElement(55, 1) && hasBeyondRank(5, 2))
                start = start.add(beyondRankEffect(5, 2, 0));
        }
    } else if (type == 2) {
        if (name == "rank") {
        ...
        if (name == "rank") {
            ...
            if (player.ranks.pent.gte(5))
                start = start.mul(RANKS.effect.pent[5]());
            ...
            if (player.ranks.pent.gte(4))
                start = start.mul(RANKS.effect.pent[4]());
            ...
            if (hasBeyondRank(14, 1))
                start = start.mul(beyondRankEffect(14, 1));
        } else if (name == "fTier") {
        ...
        if (name == "rank") {
            ...
            if (hasBeyondRank(2, 17))
                start = start.mul(beyondRankEffect(2, 17)[1]);
        }
    } else if (type == 5) {
        if (name == "rank") {
            if (tmp.chal && hasBeyondRank(2, 20))
                start = start.mul(tmp.chal.eff[1].scrank);
        } else if (name == "supernova") {
            if (hasBeyondRank(4, 1))
                start = start.add(beyondRankEffect(4, 1, 0));
        }
    } else if (type == 6) {
        if (name == "rank") {
            if (hasUpgrade("br", 24)) start = start.mul(tmp.bd.upgs[4].eff);
    ...
    if (
        ...
        start = start.mul(10);
    if (QCs.active() && QCM8_SCALES.includes(name))
        if (!tmp.scaling_qc8.includes(name))
            start = start.pow(tmp.qu.qc_eff[7][0]);
    if (hasUpgrade("br", 14) && name == "fTier" && type == 0)
        start = start.add(10);
    if (hasElement(88) && name == "tickspeed")
        start = start.mul(player.qu.rip.active ? 100 : 10);
    return start.floor();
}
...
function getScalingPower(type, name) {
    ...
    if (name == "massUpg" && type < 2 && hasElement(84))
        power = power.mul(tmp.elements.effect[84]);
    if (type == 0) {
        if (name == "rank") {
            if (player.mainUpg.rp.includes(10)) power = power.mul(0.8);
            if (player.ranks.tetr.gte(4))
                power = power.mul(RANKS.effect.tetr[4]());
        } else if (name == "tier") {
            ...
            if (player.mainUpg.rp.includes(8))
                power = power.mul(
                    tmp.upgs.main ? tmp.upgs.main[1][8].effect : 1,
                );
        } else if (name == "tickspeed") {
            ...
            if (tmp.chal && hasBeyondRank(2, 20))
                power = power.mul(tmp.chal.eff[1].over);
        } else if (name == "FSS") {
            if (hasBeyondRank(3, 18))
                power = power.mul(beyondRankEffect(3, 18));
        } else if (name == "inf_theorem") {
        ...
        if (name == "hex") {
            if (tmp.chal && hasAscension(0, 22))
                power = power.mul(tmp.chal.eff[5]);
        } else if (name == "tickspeed") {
            if (hasElement(27)) power = power.mul(0.75);
            if (hasElement(58)) power = power.mul(tmp.elements.effect[58]);
        } else if (name == "bh_condenser") {
            ...
            if (!hasAscension(0, 22) && tmp.chal && hasCharger(3))
                power = power.mul(tmp.chal.eff[5]);
        }
    } else if (type == 3) {
        if (name == "supernova") {
    ...
    if (
        hasElement(108) &&
        ["rank", "tier", "tetr", "pent"].includes(name) &&
        type < 4
    )
        power = power.mul(player.qu.rip.active ? 0.98 : 0.9);
    ...
    if (
        ...
        (name == "hex"
            ? type < 2
            : player.chal.comps[18].gte(1)
              ? type < 5
              : type < 4)
    )
        power = power.mul(tmp.qu.chroma_eff[1][1]);

    let qf = tmp.qu.qc_eff[7][1];
    if (!tmp.c16active)
        if (
            player.dark.run.upg[4] &&
            inDarkRun() &&
            ["rank", "tier", "tetr", "pent", "hex"].includes(name)
        )
            qf **= 0.75;
    if (QCs.active() && QCM8_SCALES.includes(name))
        if (!tmp.scaling_qc8.includes(name)) power = power.mul(qf);
    if (PreQ_SCALES.includes(name) && type < 3)
        power = power.mul(getEnRewardEff(5));

    let p = ["prestige0", "prestige1"];
    if (hasPrestige(3, 10)) p.push("prestige2");
    if (hasPrestige(0, 388) && p.includes(name) && type < 3)
        power = power.mul(prestigeEff(0, 388, 1));

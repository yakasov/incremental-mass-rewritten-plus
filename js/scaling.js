...
const SCALING_RES = {
  ...
  supernova(x = 0) {
    return tmp.sn.unl ? player.supernova.times : E(0);
  },
  fTier(x = 0, y = 0) {
    return tmp.sn.unl ? player.supernova.fermions.tiers[x][y] : E(0);
  },
  cosmic_str(x = 0) {
  ...
  inf_theorem() {
    return tmp.inf_unl ? player.inf.theorem : E(0);
  },
  ascension0() {
    return tmp.inf_unl ? player.ascensions[0] : E(0);
  },
  ascension1() {
    return tmp.inf_unl ? player.ascensions[1] : E(0);
  },
  gal_prestige() {
    return tmp.inf_unl ? player.gal_prestige : E(0);
  },
};
...
function getScalingName(name, x = 0, y = 0) {
  ...
  for (let n = cap - 1; n >= 0; n--) {
    if (scalingActive(name, amt, Object.keys(SCALE_START)[n]))
      return (
        capitalFirst(Object.keys(SCALE_START)[n]) + (n % 4 == 3 ? "-" : " ")
      );
  }
  return current;
}
...
function getScalingStart(type, name) {
  ...
  if (type == 0) {
    if (name == "rank") {
      ...
      if (hasUpgrade("bh", 3))
        start = start.add(
          tmp.upgs ? (tmp.upgs ? tmp.upgs[2][3].effect : 0) : 0,
        );
    ...
    if (name == "tickspeed") {
      ...
      if (hasUpgrade("atom", 10))
        start = start.add(
          tmp.upgs ? (tmp.upgs ? tmp.upgs[3][10].effect : 0) : 0,
        );
    } else if (name == "prestige0") {
      if (hasElement(175)) start = start.add(30);
      if (hasElement(194)) start = start.mul(2);
  ...
  if (
    (name == "bh_condenser" || name == "gamma_ray" || name == "tickspeed") &&
    hasUpgrade("atom", 14)
  )
    start = start.mul(10);
  if (QCs.active() && QCM8_SCALES.includes(name) && type < 4)
    if (!tmp.scaling_qc8.includes(name)) start = start.pow(tmp.qu.qc_eff[7][0]);
  ...
  return start
    .max(
      type % 4 == 3
        ? Decimal.pow(
            SCALE_POWER[t_name][name],
            tmp.scaling_power[t_name][name],
          )
            .sub(1)
            .pow(-1)
            .max(2)
...
function getScalingPower(type, name) {
  ...
  if (type == 0) {
    if (name == "rank") {
      ...
      if (hasUpgrade("rp", 8))
        power = power.mul(tmp.upgs ? tmp.upgs[1][8].effect : 1);
  ...
  if (
    ...
    power = power.mul(tmp.chal.eff[18][1]);
  ...
  if (OURO.evo >= 3 && PreD_SCALES.includes(name) && type < 6)
    power = power.mul(nebulaEff("magenta"));

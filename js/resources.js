const RESOURCES_DIS = {
  ...
  mass: {
    ...
    desc: (gs) =>
      ...
      "<br>" +
      formatGain(player.mass, tmp.massGain.mul(gs), true),
  ...
  quarks: {
    ...
    desc: (gs) =>
      ...
      (hasElement(14)
        ? formatGain(
            player.atom.quarks,
            tmp.atom.quarkGain.mul(tmp.atom.quarkGainSec).mul(gs)
          )
  ...
  rp: {
    ...
    desc: (gs) =>
      ...
      "<br>" +
      (tmp.passive >= 1
        ? formatGain(player.rp.points, tmp.rp.gain.mul(gs))
        : tmp.rp.can
        ? "(+" + format(tmp.rp.gain, 0) + ")"
        : `(requires ${formatMass(1e14)})`),
  ...
  cp: {
    ...
    desc: (gs) =>
      format(player.evo.cp.points, 0) +
      "<br>" +
      (tmp.passive >= 1
        ? formatGain(player.evo.cp.points, cpProd())
        : tmp.rp.can
        ? "(+" + format(tmp.rp.gain, 0) + ")"
        : `(requires ${formatMass(1e14)})`),
  ...
  dm: {
    ...
    desc: (gs) =>
      format(player.bh.dm, 0) +
      "<br>" +
      (tmp.passive >= 2
        ? formatGain(player.bh.dm, tmp.bh.dm_gain.mul(gs))
        : tmp.bh.dm_can
        ? "(+" + format(tmp.bh.dm_gain, 0) + ")"
        : EVO.amt >= 1
        ? `(requires ${format(5e3, 0)} CP)`
        : `(requires ${format(1e25, 0)} RP)`),
  ...
  fabric: {
    ...
    desc: (gs) =>
      format(player.evo.wh.fabric, 0) +
      "<br>" +
      (tmp.passive >= 2
        ? formatGain(player.evo.wh.fabric, tmp.bh.dm_gain)
        : tmp.bh.dm_can
        ? "(+" + format(tmp.bh.dm_gain, 0) + ")"
        : `(requires ${format(1e5, 0)} CP)`),
  ...
  bh: {
    ...
    desc: (gs) =>
      ...
      "<br>" +
      formatGain(player.bh.mass, tmp.bh.mass_gain.mul(gs), true),
  },
  wormhole: {
  ...
  atom: {
    ...
    desc: (gs) =>
      format(player.atom.points, 0) +
      "<br>" +
      (tmp.passive >= 3
        ? formatGain(player.atom.points, tmp.atom.gain.mul(gs))
        : tmp.atom.canReset
  ...
  protostar: {
    ...
    desc: () =>
      format(player.evo.proto.star, 0) +
      "<br>" +
      (tmp.passive >= 3
        ? formatGain(player.evo.proto.star, tmp.atom.gain)
        : tmp.atom.canReset
        ? "(+" + format(tmp.atom.gain, 0) + ")"
        : `(requires ${format(1e3, 0)} Fabric)`),
  ...
  md: {
    ...
    desc: (gs) =>
      ...
      (inMD()
        ...
        : hasTree("qol3")
        ? formatGain(player.md.particles, tmp.md.passive_rp_gain.mul(gs))
        : "(inactive)"),
  ...
  sn: {
    ...
    desc: (gs) => {
      ...
      return (
        format(player.supernova.times, 0) +
        (player.supernova.post_10 ? "<br>(+" + h + ")" : "")
      );
    },
  ...
  qu: {
    unl: () =>
      EVO.amt >= 4 ? EVO.amt < 5 : quUnl() || player.chal.comps[12].gte(1),
    icon: "qu",
    class: "light_green",

    desc: (gs) =>
      format(player.qu.points, 0) +
      "<br>" +
      (hasUpgrade("br", 8)
        ? player.qu.points.formatGain(tmp.qu.gain.div(10).mul(gs))
        : tmp.qu.gain.gt(0)
        ? "(+" + format(tmp.qu.gain, 0) + ")"
        : `(requires ${formatMass(EVO.amt >= 4 ? "ee12" : "ee13", 0)})`),
  ...
  ue: {
    ...
    desc: (gs) =>
      format(player.evo.cosmo.elixir, 0) +
      "<br>" +
      (hasUpgrade("br", 8)
        ? player.evo.cosmo.elixir.formatGain(tmp.qu.gain.div(10).mul(gs))
        : tmp.qu.gain.gt(0)
        ? "(+" + format(tmp.qu.gain, 0) + ")"
        : `(requires ${formatMass("ee12", 0)})`),
  ...
  br: {
    unl: () =>
      EVO.amt < 5 && (EVO.amt >= 4 ? tmp.qu.mil_reached[9] : hasTree("unl4")),
    icon: "br",
    class: "red",

    desc: (gs) =>
      player.qu.rip.amt.format(0) +
      "<br>" +
      (player.qu.rip.active || hasElement(147)
        ? hasUpgrade("br", 8)
          ? player.qu.rip.amt.formatGain(tmp.qu.rip.gain.div(10).mul(gs))
          : `(+${tmp.qu.rip.gain.format(0)})`
        : "(inactive)"),
  ...
  fss: {
    ...
    desc: (gs) =>
      format(player.dark.matters.final, 0) +
      "<br>(+" +
      (tmp.matters.FSS_base.gte(tmp.matters.FSS_req) ? 1 : 0) +
      ")",

    resetBtn() {
      MATTERS.final_star_shard.reset();
    },
  },
  corrupt: {
    ...
    desc: (gs) =>
      format(player.dark.c16.shard, 0) +
      "<br>" +
      (hasElement(232)
        ? player.dark.c16.shard.formatGain(tmp.c16.shardGain)
        : tmp.c16.in
        ? "(+" + tmp.c16.shardGain.format(0) + ")"
        : "(inactive)"),
    ...
    desc(gs) {
      let h = `(${formatPercent(
        player.mass
          ...
          .min(1),
        4
      )} to ${tmp.inf_unl ? "next " : ""}Infinity)`;
      if (tmp.inf_unl)
        h =
          player.inf.points.format(0) +
          "<br>(+" +
          tmp.IP_gain.format(0) +
          ")<br>" +
          h;
      return h;
    },
    ...
    desc: (gs) =>
      OURO.unl
        ? "Evolution " +
          player.evo.times +
          "<br>" +
          (player.chal.comps[20].gte(1) ? "(Ready to Evolve)" : "(Not Ready)")
        : "Something Happened...",
  ...
  speed: {
    ...
    desc() {
      let h = "";
      if (quUnl())
        h += `<span class='light_green'>${formatMult(tmp.qu.speed)}</span><br>`;
      if (tmp.inf_unl)
        h += `<span class='yellow'>${formatMult(
          tmp.preInfGlobalSpeed
        )}</span><br>`;
      if (devSpeed != 1)
        h += `<span class='white'>${formatMult(devSpeed)}</span><br>`;
      return h;
    },
  },
  ...
  apple: {
    ...
    desc: (gs) =>
      format(player.ouro.apple, 0) +
      `<br>(+${format(tmp.ouro.apple_gain, 0)}/feed)`,
  },
  berry: {
    snake: true,
    icon: "snake/berry",
    ...
    desc: (gs) =>
      format(player.ouro.berry, 0) +
      `<br>(+${format(tmp.ouro.berry_gain, 0)}/feed)`,
  },
  energy: {
    unl: () => boomUnl(),
    snake: true,
...
function setupResourcesHTML() {
  ...
  for (i in RESOURCES_DIS) {
    ...
    h1 += `<div id="${i}_res_div">
			...
			${
        i in TOOLTIP_RES
          ? `id="${i}_tooltip" tooltip-pos="left" tooltip-align="left" tooltip-text-align="left"`
          : ""
      }
...
function updateResourcesHTML() {
  ...
  let qu_gs = tmp.qu.speed,
    inf_gs = tmp.preInfGlobalSpeed;

const RESOURCES_DIS = {
  ...
  rp: {
    ...
    desc: (gs) =>
      ...
      "<br>" +
      (player.mainUpg.bh.includes(6) || player.mainUpg.atom.includes(6)
  ...
  dm: {
    ...
    desc: (gs) =>
      ...
      "<br>" +
      (player.mainUpg.atom.includes(6)
  ...
  quarks: {
    ...
    desc: (gs) =>
      ...
      (hasElement(14)
        ? formatGain(
            player.atom.quarks,
            tmp.atom ? tmp.atom.quarkGain.mul(tmp.atom.quarkGainSec).mul(gs) : 0
          )
  ...
  sn: {
    ...
    desc: (gs) => {
      let g = tmp.SN_passive
        ? tmp.supernova.passive.div(FPS)
        : tmp.supernova.bulk.sub(player.supernova.times).max(0);
  ...
  br: {
    ...
    desc: (gs) =>
      ...
      (player.qu.rip.active || hasElement(147)
        ? hasUpgrade("br", 8)
          ? player.qu.rip.amt.formatGain(tmp.rip.gain.div(10).mul(gs))
          : `(+${tmp.rip.gain.format(0)})`
        : "(inactive)"),
  ...
  speed: {
    ...
    desc: (gs) =>
      formatMult(tmp.preQUGlobalSpeed) +
      (tmp.inf_unl
        ? "<br><span class='yellow'>" +
          formatMult(tmp.preInfGlobalSpeed) +
          "</span>"
        : ""),
  },
  inf: {
...
function reset_res_btn(id) {
  RESOURCES_DIS[id].resetBtn();
}

function hide_res(id) {
  player.options.res_hide[id] = !player.options.res_hide[id];
}
  ...
  for (i in RESOURCES_DIS) {
    ...
    h1 += `
        <div id="${i}_res_div">
            <div ${
              i in TOOLTIP_RES
                ? `id="${i}_tooltip" class="tooltip ${
                    rd.class || ""
                  }" tooltip-pos="left" tooltip-align="left" tooltip-text-align="left"`
                : `class="${rd.class || ""}"`
            }>
    ...
    h2 += `
        <div id="${i}_res_hide_div">
            <div><img src="images/${
              rd.icon || "mass"
            }.png"><button style="margin-left: 10px; width: 100px;" onclick="hide_res('${i}')" id="${i}_res_hide_btn" class="btn">OFF</button></div>
        </div>
        `;
  }

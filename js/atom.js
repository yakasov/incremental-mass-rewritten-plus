const ATOM = {
    ...
    doReset(chal_reset = true) {
        ...
        for (let x = 0; x < player.mainUpg.bh.length; x++)
            if ([5].includes(player.mainUpg.bh[x]))
                keep.push(player.mainUpg.bh[x]);
    ...
    atomic: {
        ...
        effect() {
            ...
            if (!hasElement(75))
                x = x.softcap(5e4, 0.75, 0).softcap(4e6, 0.25, 0);
            ...
            (x) => {
                let a = hasPrestige(1, 400)
                    ? overflow(
                          Decimal.pow(
                              2,
                              x.add(1).log10().add(1).log10().root(2)
                          ),
                          10,
                          0.5
                      )

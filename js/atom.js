const ATOM = {
  gain() {
    ...
    let x = player.bh.mass.div(
      player.mainUpg.br.includes(1) ? 1.5e156 ** 0.5 : 1.5e156
    );
    if (x.lt(1)) return E(0);
    x = x.root(5);
    if (player.mainUpg.rp.includes(15))
      x = x.mul(tmp.upgs.main ? tmp.upgs.main[1][15].effect : E(1));
    x = hasElement(204)
      ? x.pow(tmp.bosons.upgs.gluon[0].effect)
      : x.mul(tmp.bosons.upgs.gluon[0].effect);
  ...
  quarkGain() {
    ...
    if (player.mainUpg.atom.includes(8))
      x = x.mul(tmp.upgs.main ? tmp.upgs.main[3][8].effect : E(1));
    ...
    let o = x;
    let os = tmp.c16active
        ? E("ee6")
        : E("ee90").pow(treeEff("ct13") ? tmp.chal.eff[15] : 1),
      op = E(0.5);
  ...
  doReset(chal_reset = true) {
    ...
    BUILDINGS.reset("bhc");
    let keep = [];
    for (let x = 0; x < player.mainUpg.bh.length; x++)
      if ([5].includes(player.mainUpg.bh[x])) keep.push(player.mainUpg.bh[x]);
    if (!hasInfUpgrade(18)) player.mainUpg.bh = keep;
    if (chal_reset && !player.mainUpg.atom.includes(4) && !hasTree("chal2"))
  ...
  atomic: {
    gain() {
      let greff = {
        eff: BUILDINGS.eff("cosmic_ray"),
        exp: BUILDINGS.eff("cosmic_ray", "exp"),
      };
      ...
      let os = tmp.c16active
        ? E("e500")
        : E("ee82").pow(treeEff("ct13") ? tmp.chal.eff[15] : 1);
    ...
    effect() {
      ...
      x = x
        .softcap(hasUpgrade("atom", 13) ? 1e11 : 1e10, w, 0)
        .softcap(2.5e35, w, 0);
  ...
  gamma_ray: {
    buy() {
      if (tmp.atom.gamma_ray_can) {
        player.atom.points = player.atom.points
          .sub(tmp.atom.gamma_ray_cost)
          .max(0);
  ...
  particles: {
    ...
    assignAll() {
      let sum =
        player.atom.dRatio[0] + player.atom.dRatio[1] + player.atom.dRatio[2];
      if (
        player.atom.quarks.lt(sum) ||
        CHALS.inChal(9) ||
        FERMIONS.onActive("12")
      )
        return;
    ...
    effect(i) {
      ...
      if (!hasElement(169))
        x = x.softcap("ee11", 0.9, 2).softcap("ee13", 0.9, 2);
      return x;
    },
    gain(i) {
      let x = tmp.atom.particles[i] ? tmp.atom.particles[i].effect : E(0);
      if (player.mainUpg.atom.includes(7))
        x = x.mul(tmp.upgs.main ? tmp.upgs.main[3][7].effect : E(1));
      if (QCs.active()) x = x.pow(tmp.qu.qc_eff[4]);
    ...
    desc: [
      (x) => {
        return `
                Boost Mass gain by ${
                  hasElement(105) ? "^" + format(x.eff1) : format(x.eff1) + "x"
                }<br><br>
...
function updateAtomicHTML() {
  ...
  tmp.el.atomicEff.setHTML(
    format(tmp.atom.atomicEff, 0) +
      (tmp.atom.atomicEff.gte(5e4)

const UPGS = {
  ...
  main: {
    ...
    1: {
      ...
      set res(x) {
        return (player.rp.points = E(x));
      },
      unl() {
        return tmp.rp.unl;
      },
      auto_unl() {
        return hasUpgrade("bh", 4) || tmp.inf_unl;
      },
      lens: 25,
      1: {
    ...
    2: {
      ...
      set res(x) {
        return (player.bh.dm = E(x));
      },
      unl() {
        return tmp.bh.unl;
      },
      auto_unl() {
        return hasUpgrade("atom", 2) || tmp.inf_unl;
      },
      lens: 25,
      1: {
      ...
      12: {
        unl() {
          return player.atom.unl && OURO.evo < 1;
        },
    ...
    3: {
      ...
      6: {
        get desc() {
          return `Gain 100% of Dark Matters gained from reset per second. ${OURO.evo >= 2 ? "Increase Wormhole loselessness" : "Mass gain from Black Hole softcap starts later"} based on Atomic Powers.`;
        },
        get cost() {
          return OURO.evo >= 2 ? E(1e3) : E(1e18);
        },
        effect() {
      ...
      12: {
        ...
        get desc() {
          return OURO.evo >= 2
            ? "Wormhole effects are better."
            : "Mass from Black Hole effect is better.";
        },
        cost: E("e2015"),
      },
      13: {
        unl() {
          return (brokeDil() && player.qu.rip.active) || hasElement(128);
        },
      ...
      18: {
        unl() {
          return (tmp.mass4Unl || tmp.inf_unl) && OURO.evo < 2;
        },
      ...
      24: {
        unl() {
          return tmp.fifthRowUnl && OURO.evo < 2;
        },
    ...
    4: {
      ...
      10: {
        unl() {
          return brokeDil() || tmp.inf_unl || OURO.evo >= 3;
        },
      ...
      12: {
        unl() {
          return OURO.evo < 3 && (brokeDil() || tmp.inf_unl);
        },
      ...
      17: {
        unl() {
          return OURO.evo < 2 && (tmp.mass4Unl || tmp.inf_unl);
        },
...
function canGetUpgrade(id, x) {
  let idx = UPGS.main.ids.indexOf(id),
    ud = UPGS.main[idx],
    ut = tmp.upgs[idx][x];
...
function hasUpgrade(id, x) {
  return tmp.upgs[UPGS.main.ids.indexOf(id)]?.[x]?.has;
}
function upgEffect(id, x, def = E(1)) {
  return tmp.upgs[id][x] ? tmp.upgs[id][x].effect : def;
}

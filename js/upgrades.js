const UPGS = {
  mass: {
    ...
    1: {
      ...
      bonus() {
        ...
        if (player.mainUpg.rp.includes(2))
          x = hasAscension(0, 1)
            ? x.mul(tmp.upgs.mass[2].bonus.add(1))
    ...
    2: {
      ...
      bonus() {
        ...
        if (player.mainUpg.rp.includes(7))
          x = hasAscension(0, 1)
            ? x.mul(tmp.upgs.mass[3].bonus.add(1))
            : x.add(tmp.upgs.mass[3].bonus);
        x = x.mul(getEnRewardEff(4));
        return x;
      },
    },
    3: {
      ...
      inc: E(9),
      effect(x) {
        let xx = hasAscension(0, 1)
          ? x.add(1).mul(tmp.upgs.mass[3].bonus.add(1))
          : x.add(tmp.upgs.mass[3].bonus);
          ...
          eff:
            ...
            (eff.eff.gte(eff.ss)
              ? ` <span class='soft'>(softcapped${
                  eff.eff.gte(1.8e5)
                    ? eff.eff.gte(5e15) && !player.ranks.pent.gte(15)
                      ? "^3"
                      : "^2"
                    : ""
                })</span>`
  ...
  main: {
    temp() {
      for (let x = 1; x <= this.cols; x++) {
        for (let y = 1; y <= this[x].lens; y++) {
          let u = this[x][y];
          if (u.effDesc)
            tmp.upgs.main[x][y] = { effect: u.effect(), effDesc: u.effDesc() };
        }
      }
    },
    ...
    1: {
      ...
      can(x) {
        return (
          player.rp.points.gte(this[x].cost) && !player.mainUpg.rp.includes(x)
        );
      },
      buy(x) {
        if (this.can(x)) {
      ...
      11: {
        ...
        effDesc(x = this.effect()) {
          return (
            format(x) +
            "x" +
            (x.gte("e4000") ? " <span class='soft'>(softcapped)</span>" : "")
          );
        },
      },
      12: {
      ...
      3: {
        ...
        effDesc(x = this.effect()) {
          return (
            ...
            format(x, 0) +
            " later" +
            (x.gte(100) ? " <span class='soft'>(softcapped)</span>" : "")
          );
        },
      },
      4: {
    ...
    3: {
      ...
      can(x) {
        return (
          player.atom.points.gte(this[x].cost) &&
          !player.mainUpg.atom.includes(x)
        );
      },
      buy(x) {
        if (this.can(x)) {
      ...
      13: {
        unl() {
          return (
            (player.md.break.active && player.qu.rip.active) || hasElement(128)
          );
        },
    ...
    4: {
      ...
      can(x) {
        return (
          player.qu.rip.amt.gte(this[x].cost) && !player.mainUpg.br.includes(x)
        );
      },
      buy(x) {
        if (this.can(x)) {
      ...
      3: {
        ...
        effDesc(x = this.effect()) {
          return "^" + format(x) + x.softcapHTML(30);
        },
      },
      4: {
...
function upgEffect(id, x, def = E(1)) {
  return tmp.upgs.main[id][x] ? tmp.upgs.main[id][x].effect : def;
}
function resetMainUpgs(id, keep = []) {
  let k = [];
  let id2 = UPGS.main.ids[id];
  for (let x = 0; x < player.mainUpg[id2].length; x++)
    if (keep.includes(player.mainUpg[id2][x])) k.push(player.mainUpg[id2][x]);

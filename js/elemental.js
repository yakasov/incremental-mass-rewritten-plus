const ELEMENTS = {
    ...
    canBuy(x) {
        ...
        return (
            res.gte(u.cost) &&
            !hasElement(x) &&
            ((hasInfUpgrade(6) && x <= 218) ||
                player.qu.rip.active ||
                !BR_ELEM.includes(x)) &&
            (tmp.c16active || !C16_ELEM.includes(x)) &&
            !tmp.elements.cannot.includes(x) &&
            !(CHALS.inChal(14) && x < 118)
        );
    },
    buyUpg(x) {
        if (this.canBuy(x)) {
            ...
            if (u.inf) player.inf.points = player.inf.points.sub(u.cost);
                ...
                player.dark.shadow = player.dark.shadow.sub(u.cost);
    ...
    upgs: [
        ...
        },
        {
            ...
            effect() {
                ...
                if (hasPrestige(0, 867)) {
                    ...
                    x = player.atom
                        ? player.atom.powers[2].add(1).root(2)
                        : E(1);
                    if (x.gte("e1e4"))
                        x = expMult(x.div("e1e4"), 0.9).mul("e1e4");
        ...
        },
        {
            ...
            effDesc(x) {
                return hasElement(276) ? "^" + format(x) : formatMult(x);
            },
        },
        {
        ...
        {
            desc: `Electron power's effects are better. Passively gain 10% of each quark assignable particle.`,
            cost: E(1e107),
        },
        {
        ...
        {
            desc: `Mass gain softcap<sup>2</sup> is 10% weaker.`,
            cost: E("e2400"),
        },
        {
        ...
        {
            desc: `Entropic Converter nerf is 10% weaker.`,
            cost: E("e202000"),
        },
        {
        ...
        {
            dark: true,
            desc: `Uncap [Neut-Muon]’s effect. It is stronger when the effect is greater than 33%.`,
            cost: E("1e84"),
        },
        {
            br: true,
        ...
        {
            dark: true,
            desc: `Exotic Rank and Ultra Prestige Level scale 10% weaker.`,
            cost: E("e435"),
        },
        {
        ...
        {
            desc: `Unlock Accelerators and Tickspeed now provides an exponential boost, but nullify Argon-18 and Unpentnilium-150 (except in 15th Challenge).`,
            cost: E("e8.6e95"),
        },
        {
            br: true,
        ...
        },
        {
            ...
            effect() {
                ...
                if (hasElement(228))
                    x = x.mul(Decimal.pow(1.1, m.max(1).log10()));
        ...
        },
        {
            ...
            effDesc(x) {
                return "^" + format(x) + (tmp.c16active ? "" : " to exponent");
            },
        },
        {
        ...
        {
            inf: true,
            desc: `Allow you to form any Theorem into its fragments, providing benefits.`,
            cost: E("5e17"),
        },
        {
            c16: true,
        ...
        {
            dark: true,
            desc: `Remove the softcap of Abyssal Blot’s seventh reward.`,
            cost: E("e1800000"),
        },
        {
            inf: true,
        ...
        {
            dark: true,
            desc: `Abyssal Blot’s eighth reward now works in C16.`,
            cost: E("e1.3e10"),
        },
        {
            c16: true,
...
function hasElement(x, layer = 0) {
    return (
        player.atom[["elements", "muonic_el"][layer]].includes(x) &&
        !(tmp.c16active && isElemCorrupted(x))
    );
}
...
function updateElementsHTML() {
    ...
    if (ch) {
        ...
        let res = [
            eu.inf ? " Infinity Points" : eu.dark ? " Dark Shadows" : " Quarks",
            eu.cs ? " Corrupted Stars" : " Exotic Atoms",
        ][elayer];
...
function updateElementsTemp() {
    ...
    for (let x = tElem.upg_length; x >= 1; x--)
        if (ELEMENTS.upgs[x].effect) {

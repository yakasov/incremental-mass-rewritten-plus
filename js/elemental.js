const ELEMENTS = {
  ...
  upgs: [
    null,
    {
      get desc() {
        return EVO.amt >= 3
          ? `+0.25 to the exponent of Quark gain from Protostars.`
          : `Quark gain formula is better.`;
      },
      cost: E(1e6),
    },
    {
    ...
    },
    {
      ...
      effDesc(x) {
        return hasPrestige(0, 867)
          ? "^" + format(x)
          : format(x) + "x" + softcapHTML(x, "ee4");
      },
    },
    {
      ...
      effect() {
        let x = tmp.atom.unl
          ? player.atom.powers[0].max(1).log10().pow(0.8).div(50).add(1)
          : E(1);
        if (EVO.amt >= 2) x = x.min(5e7);
        return overflow(x.softcap(1e45, 0.1, 0), "e60000", 0.5).min("ee6");
      },
      effDesc(x) {
        return format(x) + "x stronger";
      },
    },
    {
    ...
    },
    {
      ...
      effect() {
        ...
        if (hasElement(276)) {
          x = E(1);
          for (let i = 1; i <= CHALS.cols; i++)
            x = x.mul(player.chal.comps[i].add(1));
      ...
      effDesc(x) {
        return hasElement(276) ? formatPow(x) : formatMult(x);
      },
    },
    {
    ...
    },
    {
      get desc() {
        return EVO.amt >= 2
          ? `Raise Fabric by +^0.1.`
          : `Super BH Condenser & Cosmic Ray scale 20% weaker.`;
      },
      cost: E(1e34),
    },
    {
      desc: `Silicon's effect is +2% stronger for each Element bought.`,
      cost: E(5e38),
      effect() {
    ...
    {
      desc: `You can now automatically buy Cosmic Rays. Cosmic Rays raise Tickspeed's effect slightly.`,
      cost: E(1e44),
      effect() {
        let x = overflow(
          hasElement(129)
            ? player.build.cosmic_ray.amt.pow(0.5).mul(0.02).add(1)
            : player.build.cosmic_ray.amt.pow(0.35).mul(0.01).add(1),
          1000,
          0.5
        );
    ...
    },
    {
      ...
      effect() {
        ...
        let x = hasPrestige(0, 40)
          ? player.atom.atomic.max(1).log10().add(1).log10().add(1).root(2)
          : player.atom.atomic.max(1).log10().add(1).pow(0.4);
        return x;
      },
      effDesc(x) {
        return hasPrestige(0, 40) ? formatPow(x) : format(x) + "x";
      },
    },
    {
    ...
    },
    {
      ...
      effect() {
        let x = E(0.99)
          .pow(E(getElemLength(0)).softcap(30, 2 / 3, 0))
          .max(0.5);
        return x;
      },
      effDesc(x) {
    ...
    {
      desc: `Electron Power's effects are better. Passively gain 10% of each particle that you would gain by assigning Quarks.`,
      cost: E(1e107),
    },
    {
      ...
      effDesc(x) {
        return format(x) + "x" + x.softcapHTML("ee27");
      },
    },
    {
    ...
    },
    {
      ...
      cost: E(1e175),
      effect: () =>
        tmp.rp.unl ? player.rp.points.max(1).log10().add(1).pow(0.75) : E(1),
      effDesc(x) {
        return format(x) + "x";
      },
    },
    {
      ...
      cost: E(1e210),
      effect: () =>
        tmp.bh.unl ? player.bh.mass.max(1).log10().add(1).pow(0.8) : E(1),
      effDesc(x) {
        return format(x) + "x";
      },
    },
    ...
    {
      desc: `Cosmic Ray's free Tickspeeds now add to Rage Upgrade 2x2 ('Tickspeed adds Stronger').`,
      cost: E(1e260),
      effect() {
      ...
      effDesc(x) {
        return "+" + format(x, 0) + " to Rage Power Upgrade 7";
      },
    },
    {
    ...
    },
    {
      ...
      effDesc(x) {
        return (
          format(x[0]) + "x" + (hasPrestige(0, 190) ? ", ^" + format(x[1]) : "")
        );
      },
    },
    {
    ...
    {
      desc: `Collapsed Stars' effect boosts Black Hole Mass gain at a reduced rate.`,
      cost: E("e510"),
      effect() {
    ...
    {
      desc: `Collapsed Stars boost the last type of stars.`,
      cost: E("e1000"),
      effect() {
        if (!tmp.star_unl) return E(1);
    ...
    {
      desc: `Star Generators are ^1.05 stronger.`,
      cost: E("e2000"),
    },
    {
    ...
    {
      desc: `Black Hole Mass boosts Atomic Power gain at a reduced rate.`,
      cost: E("e2800"),
    ...
    },
    {
      get desc() {
        return EVO.amt >= 2
          ? `Square Atomic Upgrade 6.`
          : `Hyper/Ultra BH Condenser & Cosmic Ray scale 25% weaker.`;
      },
      cost: E("e1.6e4"),
    },
    {
    ...
    },
    {
      ...
      cost: E("e3.6e4"),
      effect() {
        let x = BUILDINGS.eff("tickspeed", "power")
          .max(1)
          .log10()
          .div(10)
          .max(1);
    ...
    },
    {
      get desc() {
        return EVO.amt >= 2
          ? `Increase Wormhole loss by 20%.`
          : `Non-Bonus Tickspeeds are 25x more effective.`;
      },
      cost: E("e3e5"),
    },
    {
    ...
    },
    {
      ...
      effect() {
        if (!tmp.star_unl) return E(1);
        return hasElement(236) || EVO.amt >= 2
          ? Decimal.pow(1.1, player.stars.points.add(1).log10().add(1).log10())
          : overflow(
      ...
      effDesc(x) {
        return hasElement(236) || EVO.amt >= 2 ? formatPow(x) : format(x) + "x";
      },
    },
    {
    ...
    {
      desc: `Pent is now added to the Mass gain formula from Collapsed Stars.`,
      cost: E("e3.6e7"),
    },
    {
      desc: `Add 200 more C7 & C8 maximum completions.`,
    ...
    },
    {
      get desc() {
        return EVO.amt >= 2
          ? `Raise Fabric by +^0.05.`
          : `BH formula softcap starts later based on Supernovas.`;
      },
      cost: E("e1.6e8"),
      effect() {
      ...
      effDesc(x) {
        return EVO.amt >= 2 ? undefined : formatPow(x) + " later";
      },
    },
    {
    ...
    {
      desc: `Add more Challenge 5, 6 & 8 maximum completions based on Supernovas.`,
      cost: E("e1.3e9"),
      effect() {
        if (!tmp.sn.unl) return E(0);
    ...
    {
      desc: `Collapsed Stars' effect is 25% stronger.`,
      cost: E("e3.75e10"),
    },
    {
    ...
    },
    {
      get desc() {
        return EVO.amt >= 2
          ? `Gain 100x more Quantum Foam.`
          : `Stronger and Tickspeed are 10x stronger.`;
      },
      get cost() {
        return EVO.amt >= 2 ? E("e7e12") : E("e1.4e13");
      },
    },
    {
      desc: `Stronger is ^1.1 stronger. Stronger is now stronger, making Stronger stronger.`,
      cost: E("e2.8e13"),
    },
    {
    ...
    {
      desc: `Pre-Ultra Mass Upgrades scale weaker based on Cosmic Rays' free Tickspeeds.`,
      cost: E("e7e14"),
      effect() {
        if (!tmp.atom.unl) return E(1);
        return E(EVO.amt >= 2 ? 0.998 : 0.9).pow(
          tmp.atom.atomicEff[0]
            .add(1)
            .log10()
            .pow(2 / 3)
        );
    ...
    {
      desc: `Carbon-6's effect is overpowered, but Sodium-11 is disabled.`,
      cost: E("e150"),
    },
    {
      desc: `All Rickspeed scalings start 100x later (after nerf from 8th QC modifier).`,
      cost: E("e500"),
    },
    {
    ...
    },
    {
      ...
      cost: E("e1300"),
      effect: () =>
        tmp.atom.unl ? player.md.mass.add(1).log10().add(1).pow(0.5) : E(1),
      effDesc(x) {
        return "x" + x.format();
      },
    },
    {
      desc: `Entropic Accelerator & Booster nerfs are 10% weaker.`,
      cost: E("e2700"),
    },
    {
    ...
    },
    {
      ...
      effect() {
        ...
        let x = E(1.1).pow(s);
        return x
          .softcap(player.qu.rip.active ? "e130" : "e308", 0.01, 0)
          .min("e2e4");
      },
      effDesc(x) {
        return "x" + x.format();
    ...
    {
      desc: `Epsilon Particles are enabled in Big Rip at 10% of normal power.`,
      cost: E("e34500"),
    },
    {
      desc: `Entropic Converter nerfs are 10% weaker.`,
      cost: E("e202000"),
    },
    {
    ...
    {
      desc: `Weaken softcap^2 and remove softcap^3 from Photon Upgrade 3's effect.`,
      cost: E("e2.15e7"),
    },
    {
      ...
      effect() {
        ...
        let x = hasElement(195)
          ? pent.softcap(2e5, 0.25, 0).root(1.5).div(400)
          : pent.root(2).div(1e3);
    ...
    {
      desc: `All scalings from Ranks to Pent scale 10% (2% in Big Rip) weaker.`,
      cost: E("e1.6e10"),
    },
    {
    ...
    {
      desc: `Mass gain softcap^4 is 50% (20% in Big Rip) weaker.`,
      cost: E("e6e10"),
    },
    {
    ...
    {
      dark: true,
      desc: `You can now automatically complete Challenges 9-11. Keep Challenge 12 completions on Big Rip or when starting QC.`,
      cost: E("e6"),
    },
    {
      br: true,
      get desc() {
        return EVO.amt >= 3
          ? `Death Shards boost Protostar gain.`
          : `You can now automatically buy broken Dilation upgrades. They no longer spent Relativistic Mass.`;
      },
      cost: E("ee19"),
      effect() {
        if (EVO.amt < 3) return E(1);
        let x = hasElement(200)
          ? expMult(player.qu.rip.amt.add(1), 0.5)
          : player.qu.rip.amt.add(1).log10().add(1).pow(2);
        return x;
      },
      effDesc(x) {
        return EVO.amt < 3 ? undefined : formatMult(x);
      },
    },
    {
      dark: true,
      desc: `Keep Quantum tree when going Dark.`,
      cost: E("e7"),
    },
    {
      get desc() {
        return EVO.amt >= 2
          ? `Improve the Wormhole.`
          : `7th challenge's effect gives more C9-12 completions at 10% rate.`;
      },
      cost: E("e9e24"),
      effect() {
      ...
      effDesc(x) {
        return EVO.amt >= 2 ? undefined : "+" + format(x, 0);
      },
    },
    {
      dark: true,
    ...
    {
      br: true,
      desc: `Argon-18 is overpowered and can affect BHC & Cosmic Ray exponents.`,
      cost: E("e1.7e20"),
    },
    {
      br: true,
    ...
    },
    {
      get desc() {
        return EVO.amt >= 2
          ...
          : `Make the 3rd, 4th & 8th Challenge effects better.`;
      },
      cost: E("ee29"),
    },
    {
      desc: `Super Prestige & Honor scalings are 5% weaker.`,
      cost: E("e1e30"),
    },
    {
      br: true,
    ...
    {
      br: true,
      desc: `Supernova boosts Blueprint Particle gain.`,
      cost: E("e8.6e26"),
      effect() {
        if (!tmp.sn.unl) return E(1);
        return Decimal.pow(
          1.1,
          player.supernova.times.overflow(1e75, 0.1).softcap(2e5, 0.25, 0)
        );
      },
      effDesc(x) {
        return formatMult(x, 1);
    ...
    {
      dark: true,
      desc: `Dark Shadow's second effect is better. Keep pre-118 Big Rip elements when going Dark.`,
      cost: E("e27"),
    },
    {
      dark: true,
    ...
    },
    {
      ...
      effect() {
        ...
        let x = hasPrestige(0, 218)
          ? E(10).pow(pb.add(1).log10().root(2))
          : pb.add(1).log10().add(1);
    ...
    {
      dark: true,
      desc: `You can now gain Death Shards outside of Big Rip. Automate Cosmic Strings.`,
      cost: E("e40"),
    },
    {
      br: true,
    ...
    },
    {
      get desc() {
        return EVO.amt >= 2
          ? `Untritrium-133 effect in Big Rip works outside of Big Rip.`
          : `Stronger's effect softcap is slightly weaker.`;
      },
      cost: E("e4e45"),
    },
    {
      get desc() {
        return EVO.amt >= 2
          ? `Improve 4th Wormhole.`
          : `Stronger's effect softcap is slightly weaker again. Tickspeed's effect is overpowered.`;
      },
      cost: E("ee54"),
    },
    {
    ...
    },
    {
      ...
      cost: E("e3.6e61"),
      effect: () =>
        player.atom.quarks.add(1).log10().add(1).log10().add(1).pow(1.5),
      effDesc(x) {
        return formatMult(x);
      },
    },
    {
      ...
      effDesc(x) {
        return formatPow(x) + x.softcapHTML(400);
      },
    },
    {
    ...
    {
      dark: true,
      desc: `Uncap [Neut-Muon]'s effect, gaining an additional boost if its effect is greater than 33%.`,
      cost: E("e84"),
    },
    {
      br: true,
      get desc() {
        return EVO.amt >= 2
          ? `Fabric boosts Wormhole more.`
          : EVO.amt >= 1
          ? `Raise Meditation' level to the 1.5th power.`
          : `Meta-Tickspeed scaling starts ^2 later.`;
      },
      cost: E("e2.5e53"),
    },
    {
      desc: `Abyssal Blot's second effect applies to Mass gain softcaps^7-8, and both softcaps are 20% weaker.`,
      get cost() {
        return E(EVO.amt >= 1 ? "e8e68" : "e2.2e69");
      },
    },
    {
      br: true,
    ...
    {
      dark: true,
      desc: `Unlock Dark Run. Keep Oganesson-118 when going Dark.`,
      cost: E("e96"),
    },
    {
      desc: `Collapsed Stars' effect raises normal Mass. This exponent also raises Black Hole Mass.`,
      get cost() {
        return E(EVO.amt >= 1 ? "ee69" : "ee76");
      },
    },
    {
    ...
    },
    {
      ...
      get cost() {
        return E(EVO.amt >= 2 ? "ee66" : EVO.amt >= 1 ? "e6e68" : "ee75");
      }, // nice
    },
    {
      br: true,
    ...
    {
      desc: `Hex and Glory requirements are slightly weaker.`,
      get cost() {
        return E(EVO.amt >= 1 ? "ee71" : "e2e79");
      },
    },
    {
      dark: true,
    ...
    },
    {
      get desc() {
        return EVO.amt >= 3
          ? `+^0.05 to Protostars. Nebulae Tier 1 work in Big Rip.`
          : `Remove two softcaps of Particle Power gains.`;
      },
      get cost() {
        return E(EVO.amt >= 4 ? "ee25" : "ee80");
      },
    },
    {
      br: true,
      desc: `Collapsed Stars' effects are even better.`,
      cost: E("ee83"),
    },
    {
      dark: true,
    ...
    {
      desc: `Uncap Bottom. Now you can fall forever!`,
      cost: E("e5e86"),
    },
    {
      desc: `Neutronium-0 affects Challenges 9-12 at a reduced rate.`,
      cost: E("ee87"),
    },
    {
      br: true,
    ...
    {
      desc: `Exotic Rank starts later based on Meta-Rank start.`,
      cost: E("e5e89"),
      effect() {
        if (!tmp.scaling_start.meta || !tmp.scaling_start.meta.rank)
          return E(1);
    ...
    {
      br: true,
      desc: `Reduce first 12 Challenges' scaling strengths by 30%.`,
      cost: E("ee86"),
    },
    {
    ...
    {
      desc: `Raise Collapsed Star gain after softcap by 10.`,
      cost: E("ee90"),
    },
    {
      br: true,
      ...
      cost: E("ee88"),
      effect() {
        let x = Decimal.pow(1.1, player.qu.en.amt.add(1).log10().pow(0.9));
    ...
    {
      dark: true,
      desc: `Keep Prestige tiers when going Dark. Super and Hyper Prestiges start x2 later.`,
      cost: E("e360"),
    },
    {
    ...
    {
      desc: `Unlock Accelerators. Tickspeeds now provide an exponential boost. Argon-18 and Unpentnilium-150 are nullified (except in 15th Challenge).`,
      cost: E("e1.3e103"),
    },
    {
      br: true,
      get desc() {
        return EVO.amt >= 3
          ? `Improve Unbitrium-123.`
          : `15th Challenge reward applies to Black Hole overflow.`;
      },
      cost: E("e8e101"),
    },
    {
    ...
    {
      desc: `Booster boosts its effect. In other words, Booster is boosting Booster's boost.`,
      cost: E("ee108"),
      effect() {
    ...
    {
      dark: true,
      desc: `1st and 3rd Photon & Gluon upgrades provide an exponential boost. Keep Big Rip upgrades when going Dark.`,
      cost: E("e605"),
    },
    {
    ...
    },
    {
      ...
      effect: () =>
        tmp.bh.unl
          ? player.bh.dm.add(1).log10().add(1).log10().add(1).log10().div(10)
          : E(0),
      effDesc(x) {
        return "+^" + format(x);
      },
    ...
    {
      br: true,
      desc: `Hybridized Uran-Astatine's second effect is stronger and now applies to Hex scalings.`,
      cost: E("e9e106"),
    },
    {
    ...
    },
    {
      ...
      get desc() {
        return (
          `Stronger overflow starts later based on FSS.` +
          (EVO.amt == 3
            ? " You can buy Protostar Elements during Big Rip."
            : "")
        );
      },
      get cost() {
        return E(EVO.amt >= 4 ? "e645" : "e710");
      },
      effect: () =>
    ...
    },
    {
      ...
      get desc() {
        return EVO.amt >= 4
          ? `+^0.01 to quark formula from Protostars.`
    ...
    {
      c16: true,
      desc: `Improve the formula of Corrupted Shard gain.`,
      cost: E("ee23"),
    },
    {
    ...
    {
      inf: true,
      desc: `Infinity Theorems increase Parallel Extruder's power. Muon-Catalyzed Fusion no longer resets.`,
      cost: E("e14"),
      effect() {
    ...
    },
    {
      ...
      effDesc(x) {
        return formatPow(x, 1);
      },
    },
    {
      c16: true,
      desc: `Matter exponent boosts Matters gain outside C16.`,
      cost: E("ee25"),
      effect() {
      ...
      effDesc(x) {
        return formatPow(x) + (tmp.c16.in ? "" : " to exponent");
      },
    },
    {
    ...
    {
      dark: true,
      desc: `Increase C16's maximum completions to 100. Keep C16 completions on Infinity.`,
      cost: E("e810000"),
    },
    {
      inf: true,
      desc: `Allow you to form any Theorem into fragments which provide passive bonuses.`,
      cost: E("5e17"),
    },
    {
      c16: true,
    ...
    {
      dark: true,
      desc: `Remove the softcap of Abyssal Blots' seventh reward.`,
      cost: E("e1800000"),
    },
    {
      inf: true,
      desc: `Passively gain 1% of best IP gained on Infinity. The softcap of Theorem's level starts an additional +5 later.`,
      cost: E("5e22"),
    },
    {
    ...
    {
      dark: true,
      desc: `Abyssal Blots' eighth reward is even stronger.`,
      cost: E("e6e6"),
    },
    {
      inf: true,
    ...
    {
      dark: true,
      desc: `Remove all scalings from Pent. Hybridized Uran-Astatine's first effect now works with Ranks.`,
      cost: E("e9.2e6"),
    },
    {
      ...
      effect() {
        ...
        return hasElement(273)
          ? E(10).pow(player.inf.theorem.pow(2))
    ...
    {
      c16: true,
      desc: `FSS's first reward is slightly stronger in C16.`,
      cost: E("e2e55"),
    },
    {
    ...
    {
      dark: true,
      desc: `The first softcap of Abyssal Blots' tenth reward is slightly weaker.`,
      cost: E("e8.1e7"),
    },
    {
    ...
    {
      inf: true,
      desc: `Passively gain 1% of Fragments formed from Theorem in the Core.`,
      cost: E(1e35),
    },
    {
      c16: true,
    ...
    {
      dark: true,
      desc: `C17's completions boost Super Parallel Extruders.`,
      cost: E("e5e8"),
      effect() {
    ...
    {
      dark: true,
      desc: `Abyssal Blots' eighth reward now works in C16.`,
      cost: E("e1.3e10"),
    },
    {
      c16: true,
      desc: `Add 100 more C16 maximum completions.`,
      cost: E("ee219"),
    },
    {
      inf: true,
    ...
    {
      dark: true,
      desc: `The softcap of Abyssal Blots tenth reward is slightly weaker.`,
      cost: E("e1.7e10"),
    },
    {
      c16: true,
    ...
    {
      c16: true,
      desc: `Newton Theorem's fifth star now affects Black Hole overflow^2.`,
      cost: E("ee1745"),
      effect() {
    ...
    {
      dark: true,
      desc: `Add 300 more C16 maximum completions.`,
      cost: E("e2.5e10"),
    },
    {
      inf: true,
    ...
    {
      dark: true,
      desc: `Dark Shadows' fourth reward now affects Supernova generation at reduced rate.`,
      get cost() {
        return EVO.amt >= 2 ? E("e8e10") : E("e1.67e11");
      },
    ...
    {
      c16: true,
      desc: `You can buy False Vacuum Manipulators outside of C16.`,
      cost: E("ee6170"),
    },
    {
    ...
    {
      dark: true,
      desc: `The base of Collapsed Stars' effect for Supernova generation is slightly stronger.`,
      get cost() {
    ...
    },
    {
      ...
      get cost() {
        return E(EVO.amt >= 3 ? 1e100 : EVO.amt >= 1 ? "e105" : "e110");
      },
    },
    {
      get desc() {
        return EVO.amt >= 4
          ? "Improve Limitless Dust effect and Galactic Prestige."
          : `Supernovas boost Galactic Prestige's resources at a reduced rate.`;
      },
      cost: E("ee25400"),
      effect() {
        if (!tmp.sn.unl) return E(1);
    ...
    },
    {
      ...
      effect: () =>
        EVO.amt >= 4
          ? expMult(
              player.dark.c16.totalS,
              1 / (hasInfUpgrade(20) ? 1.9 : 2)
            ).root(30)
          : player.dark.c16.totalS.add(10).log10(),
      effDesc(x) {
        return formatMult(x);
      },
    },
    ...
    {
      desc: `Add 500 more C16 maximum completions.`,
      cost: E("ee64600"),
    },
    {
      dark: true,
    ...
    },
    {
      ...
      get cost() {
        return E(
          OURO.evo == 3 ? 1e300 : OURO.evo == 2 ? 1e295 : Number.MAX_VALUE
        );
      },
    },
    {
    ...
    },
    {
      ...
      get cost() {
        return E(EVO.amt >= 4 ? 1e14 : 1e15);
      },
      effect: () =>
        hasElement(310)
          ? expMult(player.atom.quarks.add(1).log10().add(1).root(1.5), 0.5)
          : player.atom.quarks
              .add(1)
              .log10()
              .add(1)
              ...
              .softcap(1e6, 3, "log"),
      effDesc(x) {
        return formatPow(x) + x.softcapHTML(1e6, hasElement(310));
      },
    },
    {
    ...
    },
    {
      get desc() {
        return EVO.amt >= 4
          ? `Dark Rays boost Protostars.`
          : `Collapsed Stars boost Protostars.`;
      },
      get cost() {
        return E(EVO.amt >= 4 ? 1e28 : 1e42);
      },
      effect() {
        if (EVO.amt >= 4)
          return expMult(player.dark.rays.add(1), 0.5).pow(
            hasElement(306) ? 1.5 : 1
          );
    ...
    {
      desc: `The softcap of Quark gain from Protostars is weaker.`,
      cost: E("e700"),
    },
    {
    ...
    {
      desc: `Quantum Shards improve Quark gain.`,
      cost: E(1e17),
    ...
    },
    {
      ...
      effect: () =>
        player.dark.c16.shard.max(1).log10().div(50).add(0.6).min(0.85),
    ...
    },
    {
      ...
      effect() {
        ...
        return sd.div(1e12).max(1).log10().div(5).max(1).pow(-0.6);
      },
...
const ELEM_TYPES = {
  ...
  final: {
    ...
    set res(x) {
      player.atom.quarks = E(x);
    },
    resDisp: (x) =>
      format(x, 0) +
      (EVO.amt >= 5 || hasInfUpgrade(6) ? " Quarks" : " Quarks in Big Rip"),
  },
  ...
  dark: {
    ...
    get: (i, l, eu) => eu.dark,
    get res() {
      return player.dark.shadow;
    },
    set res(x) {
      player.dark.shadow = E(x);
    },
  ...
  inf: {
    ...
    get: (i, l, eu) => eu.inf,
    get res() {
      return tmp.inf_unl ? player.inf.points : E(0);
    },
    set res(x) {
      player.inf.points = E(x);
    },
  ...
  cs: {
    ...
    get: (i, l, eu) => eu.cs,
    get res() {
      return tmp.inf_unl ? player.inf.cs_amount : E(0);
    },
    set res(x) {
      player.inf.cs_amount = E(x);
    },
  ...
  berry: {
    ...
    get: (i, l, eu) => eu.berry,
    get res() {
      return player.ouro.berry;
    },
    set res(x) {
      player.ouro.berry = E(x);
    },
  ...
  proto: {
    ...
    can: () =>
      EVO.amt >= 4
        ? true
        : !tmp.c16.in && (hasElement(210) || !player.qu.rip.active),
    get res() {
      return player.evo.proto.star;
    },
    set res(x) {
      player.evo.proto.star = E(x);
    },
...
const ELEM_EFFECT = (() => {
  let x = [];
  for (let [i, upg] of Object.entries(ELEMENTS.upgs))
    if (upg?.effect) x.push(Number(i));
  return x;
})();

const BR_ELEM = (() => {
  let x = [];
  for (let i in ELEMENTS.upgs)
    if ((i > 86 && i <= 118) || (i > 0 && ELEMENTS.upgs[i].br))
      x.push(Number(i));
  return x;
})();
...
const C16_ELEM = (() => {
  let x = [];
  for (let i in ELEMENTS.upgs)
    if (i > 0 && ELEMENTS.upgs[i].c16) x.push(Number(i));
  return x;
})();
...
for (let x = 1; x <= MAX_ELEM_TIERS; x++) {
  ...
  // Muonic Elements
  for (let y = ts + 1; y <= te; y++) {
    if (!MUONIC_ELEM.upgs[y])
      MUONIC_ELEM.upgs.push({
        desc: `Placeholder.`,
        cost: EINF,
      });
  }
...
function isElemCorrupted(x, layer = 0) {
  return (
    layer == 0 &&
    !tmp.elements.deCorrupt.includes(x) &&
    CORRUPTED_ELEMENTS.includes(x)
  );
}

function elemEffect(x, def = 1) {
  return tmp.elements.effect[x] || def;
}
...
function buyAllElements(layer = player.atom.elemLayer) {
  for (let i = 1; i <= tmp.elements.unl_length[layer]; i++)
    buyElement(i, layer);
}
...
function setupElementsHTML() {
  ...
  for (let k = 1; k <= MAX_ELEM_TIERS; k++) {
    ...
    for (let i = 0; i < ELEMENTS.map[k - 1].length; i++) {
      ...
      if (m == "v") table += `</div><div class="table_center">`;
      else if (m == "_" || !isNaN(Number(m)))
        table += `<div ${
          ELEMENTS.la[m] !== undefined && k == 1 ? `id='element_la_${m}'` : ""
        } style="width: 50px; height: 50px">${
          ELEMENTS.la[m] !== undefined ? "<br>" + ELEMENTS.la[m] : ""
        }</div>`;
      else if (m == "x") {
        num++;
        table +=
          ELEMENTS.upgs[num] === undefined
        ...
        if (k == 1) {
          ...
          if (n == 0) {
            ...
            if (num == xs + WE(k + 3, n) + 2)
              num = xs + p + WE(k + 3, n - 1) + 2;
...
function updateElementsHTML() {
  let tElem = tmp.elements,
    c16 = tmp.c16.in;
  let et = player.atom.elemTier,
    elayer = player.atom.elemLayer;
  ...
  if (ch) {
    ...
    tmp.el.elem_desc.setHTML(
      "<b>[" +
        ["", "Muonic "][elayer] +
        ELEMENTS.fullNames[ch] +
        "]</b> " +
        (fed ? EVO.fed_msg[fed] : eu.desc)
    );
    tmp.el.elem_desc.setClasses({
      sky: true,
      corrupted_text2: c16 && isElemCorrupted(ch, elayer),
    });
    tmp.el.elem_cost.setTxt(
      fed
        ? ""
        : "Cost: " +
            ELEM_TYPES[getElementClass(ch, elayer)].resDisp(eu.cost) +
            (tElem.cannot.includes(ch) ? " [CANNOT AFFORD in Big Rip]" : "")
    );
      ...
      for (let x = tElem.ts + 1; x <= len; x++) {
        ...
        if (upg) {
          ...
          if (unl2) {
            ...
            upg.setClasses(
              fed
                ? { elements: true, locked: true, [fed]: true }
                : c16 && isElemCorrupted(x, elayer)
                ? { elements: true, locked: true, corrupted: true }
                : elayer == 0 && tElem.cannot.includes(ch)
                ? { elements: true, locked: true, cannot: true }
                : {
                    elements: true,
...
function getElementClass(i, layer) {
  ...
  for (var [key, d] of Object.entries(ELEM_TYPES))
    if (d.get(i, layer, upgs[i])) return key;
}
...
function setupNextElements() {
  let h = ``;
  for (var i = 0; i < 3; i++)
    h += `<div class='elem_detailed' id="elem_next_div${i}" onclick="buyNextElement(${i})">
...
function calcNextElements() {
  ...
  let i = ch[0] != 1 ? 0 : ch[1],
    data = [ELEMENTS, MUONIC_ELEM][layer].upgs;
  nextElm = [];
  ...
  for (let i = nextElm.length; i < 3; i++)
    tmp.el["elem_next_div" + i].setDisplay(false);
}

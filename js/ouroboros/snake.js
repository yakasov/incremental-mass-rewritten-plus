...
function snakeMove(s, you) {
  ...
  if (s.bodies.length == s.len)
    s.bodies[s.len - 1][1] = s.bodies[s.len - 1][1][0] + "t";
}
...
function isSnakeOccupied(p) {
  return (
    snake.occupy
      .concat(snake.apples)
      .concat(snake.new_apples)
      .findIndex((x) => x.x == p.x && x.y == p.y) != -1
  );
}
...
function spawnApples() {
  ...
  while (!origin) {
    let rp = {
      x: Math.randomInt(0, s[0]) - Math.floor(s[0] / 2),
      y: Math.randomInt(0, s[1]) - Math.floor(s[1] / 2),
    };
    if (!isSnakeOccupied(rp)) origin = rp;
  }

  let berry = Math.random() < 1 / 3,
    luck = 1 / Math.random(),
    rot = Math.randomInt(0, 4);
  for (var i = 1; i <= len; i++) {
    let type = "apple",
      tier;
...
function drawSnake() {
  ...
  for (let sn of snake.snakes) {
    for (let i = sn.len - 1; i >= 0; i--) {
      ...
      let x = b[0].x * 32 + snake.cam_pos.x,
        y = b[0].y * 32 + snake.cam_pos.y;
...
function appleEffect(id, def = 1) {
  return tmp.ouro.apple_eff[id] ?? def;
}

const WORMHOLE = {
  step(x, t, i) {
    ...
    x = Decimal.pow(p.add(1.5), x.root(pp))
      .root(pp)
      .add(t)
      .pow(pp)
      .log(p.add(1.5))
  ...
  effects: [
    [
      (m) => {
        m = hasUpgrade("atom", 12)
          ? m.add(1).pow(0.3)
          : m.add(1).log10().add(1).pow(2);
    ...
    [
      (m) =>
        m
          ...
          .div(5)
          .add(1)
          .pow(hasElement(150) ? -0.2 : hasElement(125) ? -0.15 : -0.1),
      (x) =>
        `Reduce Meditation's softcap weakness <b>${formatReduction(x)}</b>`,
    ],
    [
      (m) =>
        E(1)
    ...
    [
      ...
      (x) =>
        `Raise meditation levels. <b>^${format(x, 2)}</b>` + softcapHTML(x, 3),
    ],
    [
      (m) => {
        m = m
  ...
  canAuto(i) {
    ...
    return (
      ...
      i != 6 &&
      o != i &&
      (o == 6 || player.evo.wh.mass[i].eq(0) || player.evo.wh.auto[i])
    );
  },
};
...
function splitWormhole(origin, mode) {
  let wh = player.evo.wh,
    mass = wh.mass;
  let split = [];
  for (let x = 0; x < 6; x++)
    if (x != origin && (mode != "auto" || wh.auto[x])) split.push(x);

  let sum = E(0),
    toAdd = mass[origin].mul(0.4 / split.length);
  for (let x of split) {

const MEDITATION = {
  ...
  calc(dt) {
    if (tmp.passive >= 1)
      player.evo.cp.points = player.evo.cp.points.add(cpProd().mul(dt));
    ...
    if (player.evo.cp.level.gte(10))
      player.evo.cp.level = E(0.99).pow(dt).mul(player.evo.cp.level).max(10);
  },
};

  ...
  zodiac: {
    aries: {
      ...
      upgs: {
        u1: {
          get desc() {
            return `Apples boost all zodiac resources.`;
          },
          pos: [360, 50],
          cost: E(250),
        ...
        u2: {
          branch: ["u1"],
          get desc() {
            return `Aries boosts Meditation.`;
          },
          cost: E(500),
          pos: [240, 120],
  ...
  html() {
    ...
    if (unlocked) {
      let ztp = player.evo.const[zodiac_tab],
        ztt = tmp.evo.zodiac[zodiac_tab];

      tmp.el.zodiac_amount.setHTML(
        `Level ${format(ztp.level, 0)} / ${format(ztt.cap, 0)}<br><h3>${ztp.amount.format(0)} ${this.zodiac[zodiac_tab].name}</h3><br>${ztp.amount.formatGain(ztt.gain)}`,
      );
          ...
          for (let [ui, u] of Object.entries(upgs)) {
            ...
            if (unl) {
              ...
              u_el.setAttr(
                ...
                u.desc +
                  (u.effDesc && bought
                    ? "<br class='line'> Effect: " +
                      u.effDesc(tmp.evo.zodiac_eff[zi + "-" + ui])
                    : bought
                      ? ""
                      : "<br class='line'> Cost: " +
                        u.cost.format(0) +
...
function hasZodiacUpg(zi, ui) {
  return tmp.ouro.unl && player.evo.const.upg[zi + "-" + ui];
}
function zodiacUpgEff(zi, ui, def = E(1)) {
  return tmp.evo.zodiac_eff[zi + "-" + ui] ?? def;
}

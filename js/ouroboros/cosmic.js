const COSMIC = {
  ...
  setupHTML() {
    let h = ``;
    for (var i = 0; i < this.len; i++)
      h += `<button id="uni_${i}" onclick="COSMIC.click(${i})"></button>`;
  ...
  html() {
    ...
    for (var i = 0; i < this.len; i++) {
      let g = cs.uni[i];
      tmp.el["uni_" + i].setClasses({
        cosmo: true,
        highlight: ct.highlight.includes(i),
      });
      tmp.el["uni_" + i].setHTML(
        g != undefined
          ? `
    ...
    let h = "";
    for (var [i, c] of Object.entries(ct.eff))
      h += this.cluster_effs[i].disp(c) + "<br>";
    tmp.el["cluster_div"].setHTML(
      `<h4>Score: ${format(ct.score, 0)} | Best: ${format(
        cs.score,
        0
      )}</h4><br>` + h
    );

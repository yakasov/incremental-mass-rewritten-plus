const CHARGERS = [
  ...
  },
  {
    req: E("e33000"),
    get cost() {
      return E(EVO.amt >= 4 ? 5e6 : EVO.amt >= 2 ? 2e7 : 5e8);
    },
  ...
  },
  {
    req: E("ee6"),
    get cost() {
      return E(EVO.amt >= 4 ? 1e7 : EVO.amt >= 2 ? 3e10 : 1e26);
    },
    get desc() {
      return EVO.amt >= 2
        ? `Corrupted Shards formula is better. Corrupted Shards boost Anti-Wormhole.`
...
const UNSTABLE_BH = {
  ...
  getProduction(x, gain) {
    return E(10)
      .pow(x.max(0).root(tmp.unstable_bh.p.mul(2)))
      .add(gain)
      .log10()
...
function buyCharger(i) {
  ...
  if (player.dark.c16.shard.gte(cost)) {
    ...
    if (i === [-1, -1, 2, 5].includes(EVO.amt) && !tmp.inf_unl)
      playSavedAnimation();
...
function updateC16HTML() {
  ...
  for (let i in CHARGERS) {
    ...
    tmp.el[id + "_req"].setHTML(
      EVO.fed_msg[tmp.ouro.fed["ch" + i]] ??
        `Req: <b>${formatMass(c.req)}</b> of black hole.`
    );

const CHARGERS = [
  ...
  },
  {
    ...
    get desc() {
      return OURO.evo >= 2
        ? `Unlock Anti-Wormhole.<br><b class='saved_text'>[ Evolved Exotic ]</b>`
  ...
  },
  {
    ...
    get desc() {
      return OURO.evo >= 2
        ? `Anti-Wormhole boosts Corrupted Shards before logarithmic boost.`
  ...
  },
  {
    req: E("e77000"),
    get cost() {
      return E(OURO.evo >= 3 ? 1e14 : OURO.evo >= 2 ? 5e7 : 5e10);
    },
    get desc() {
      return OURO.evo >= 3
        ? `Unlock Exotic Protostars.<br><b class='saved_text'>[ Evolved Exotic ]</b>`
  ...
  },
  {
    req: E("ee6"),
    get cost() {
      return E(OURO.evo >= 2 ? 3e10 : 1e26);
    },
    get desc() {
      return OURO.evo >= 2
        ? `Corrupted Shards formula is better. Triple Anti-Wormhole.`
  ...
  },
  {
    req: E("e1.6e6"),
    get cost() {
      return E(OURO.evo >= 2 ? 2e17 : 5e30);
    },
    get desc() {
      return OURO.evo >= 2
        ? `Muon-Catalyzed Fusion Tier weakens Mass Upgrade scalings. In C16, this weakens Extreme Scaling too.`
...
const UNSTABLE_BH = {
  ...
  getProduction(x, gain) {
    return Decimal.pow(10, x.max(0).root(tmp.unstable_bh.p.mul(2)))
      .add(gain)
      .log(10)
      .pow(tmp.unstable_bh.p.mul(2));
  },
  calcProduction() {
...
function canCharge(i) {
  return OURO.evo >= 2
    ? !EVO.isFed("ch" + i)
    : !tmp.c16.in && player.dark.c16.bestBH.gte(CHARGERS[i].req);
}
function hasCharger(i) {
  return player.dark.c16.charger.includes(i);
}
function buyCharger(i) {
  ...
  if (player.dark.c16.shard.gte(cost)) {
    ...
    if (i === [-1, -1, 2, 5].includes(OURO.evo) && !tmp.inf_unl)
      playSavedAnimation();
...
function updateC16Temp() {
  tmp.c16.best_bh_eff =
    OURO.evo >= 2
      ? expMult(player.dark.c16.bestBH.root(2), 2 / 3)
      : player.dark.c16.bestBH.add(1).log10();
...
function updateC16HTML() {
  let evo2 = OURO.evo >= 2,
    c16 = tmp.c16.in;
  let bh = player.dark.c16.bestBH,
    cs = player.dark.c16.shard;
  ...
  for (let i in CHARGERS) {
    ...
    tmp.el[id + "_div"].setClasses({
      ...
      full: true,
      charger: true,
      locked: !req || cs.lt(c.cost) || hasCharger(i),
    });
  }
}
...
let C16_ANI = {
  ...
  time: 0,
  get on() {
    return tmp.c16.in && !CHALS.inChal(20);
  },
};
...
function drawC16() {
  ...
  if (c16c.on && c16c.time && c16c.squares.length < 50) {
    c16c.squares.push({
      x: -10,
      y: Math.random() * 100,
      size: Math.randomInt(10, 20),
      color: Math.randomInt(0, 3),
      speed: Math.randomInt(10, 40),
    });
  ...
  for (let sq of c16c.squares) {
    ...
    let sqx = (sq.x / 100) * c16c.canvas.width,
      sqy = (sq.y / 100) * c16c.canvas.height;

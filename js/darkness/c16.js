...
const UNSTABLE_BH = {
  ...
  fvm: {
    can() {
      return tmp.c16active && player.bh.dm.gte(tmp.unstable_bh.fvm_cost);
    },
    buy() {
    ...
    buyMax() {
      if (this.can())
        player.bh.fvm = player.bh.fvm.max(tmp.unstable_bh.fvm_bulk);
    },
    effect() {
...
function updateC16HTML() {
  ...
  for (let i in CHARGERS) {
    ...
    tmp.el[id + "_div"].setClasses({
      ...
      full: true,
      charger: true,
      locked: !req || cs.lt(c.cost) || hasCharger(i) || c16,
    });
  }
}

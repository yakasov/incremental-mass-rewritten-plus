const MUONIC_ELEM = {
    canBuy(x) {
        ...
        let upg = this.upgs[x],
            amt = upg.cs ? player.inf.cs_amount : tmp.exotic_atom.amount;
    ...
    buyUpg(x) {
        if (this.canBuy(x)) {
            ...
            if (upg.cs)
                player.inf.cs_amount = player.inf.cs_amount.sub(upg.cost);
    ...
    upgs: [
        ...
        {
            desc: `Prestige Base boosts Stronger overflow^1-2 starting.`,
            cost: E("e1050"),
            eff() {
        ...
        {
            desc: `Remove the scaling from primordium theorem. The bonus of each primordium particles boosts its level instead of adding.`,
            cost: E("e155000"),
        },
        {
            cs: true,

const MUONIC_ELEM = {
  upgs: [
    null,
    {
      get desc() {
        return EVO.amt >= 2
          ? `Anti-Wormhole boosts Pion gain.`
          : `Mass of unstable black hole boosts Pion gain.`;
      },
      cost: E(1000),
      eff() {
        if (EVO.amt >= 2)
          return player.evo.wh.mass[6]
            .div(1e4)
            .add(1)
            .root(EVO.amt >= 3 ? 3 : 1);
    ...
    },
    {
      ...
      eff() {
        ...
        for (let i = 1; i <= 15; i++)
          x = x.mul(
            Decimal.pow(c16 ? 1.25 : 1.1, player.chal.comps[i].root(2))
          );
        return x;
      },
      effDesc: (x) => formatMult(x),
    ...
    },
    {
      get desc() {
        return EVO.amt >= 2
          ? `Stronger boosts after overflow.`
          : `C9’s effect softcap is 1% weaker.`;
      },
      get cost() {
        return E(EVO.amt >= 4 ? 1e25 : EVO.amt >= 2 ? 1e29 : 1e32);
      },
    },
    {
      ...
      eff() {
        ...
        return EVO.amt >= 2
          ? player.supernova.times.div(1e5).add(1).pow(2)
          : player.supernova.times.div(1e6).add(1);
      },
      effDesc: (x) => formatMult(x),
    },
    {
    ...
    },
    {
      ...
      get cost() {
        return EVO.amt >= 2 ? E(1e90) : E(1e100);
      },
      eff() {
    ...
    },
    {
      get desc() {
        return EVO.amt >= 2
          ? "Greatly improve FSS base formula."
          : `C15's reward affects mass overflow^2 starting.`;
      },
      get cost() {
        return EVO.amt >= 2 ? E(1e95) : E(1e111);
      },
      eff() {
    ...
    },
    {
      get desc() {
        return EVO.amt >= 2
          ? `Raise Dark Shadows by ^1.5.`
          : `Accelerators raise the Argon-18's effect at an extremely reduced rate (after first overflow).`;
      },
      get cost() {
        return EVO.amt >= 2 ? E(1e160) : E(1e170);
      },
    ...
    },
    {
      ...
      eff() {
        if (EVO.amt >= 4)
          return tmp.prestiges.base.max(1).log10().div(1e5).max(1).pow(100);
    ...
    },
    {
      ...
      get cost() {
        return EVO.amt >= 2 ? E("e4000") : E("e7300");
      },
    },
    {
      cs: true,
    ...
    },
    {
      ...
      get cost() {
        return EVO.amt >= 2 ? E("e4500") : E("e8100");
      },
    ...
    },
    {
      ...
      get cost() {
        return EVO.amt >= 2 ? E("e4700") : E("e8600");
      },
      eff() {
    ...
    },
    {
      ...
      eff: () =>
        tmp.inf_unl ? player.inf.cs_amount.add(1).overflow(10, 0.5) : E(1),
      effDesc: (x) => formatMult(x),
    },
    {
      get desc() {
        return EVO.amt >= 2
          ? `Infinity Theorems raise Pions and Kaons, and boost its reward strength.`
          : `Hawking Theorem’s fifth star now affects black hole’s effect.`;
      },
      get cost() {
        return EVO.amt >= 2 ? E("e5460") : E("e14900");
      },

      eff: () =>
        tmp.inf_unl ? player.inf.theorem.add(1).log10().add(1.5) : E(1),
      effDesc: (x) => formatPow(x),
    },
    {
      ...
      cost: E("e15900"),
      eff: () =>
        tmp.inf_unl ? expMult(tmp.preInfGlobalSpeed.max(1), 0.5).pow(2) : E(1),
      effDesc: (x) => formatMult(x),
    },
    {
      cs: true,
      ...
      eff: () =>
        tmp.inf_unl ? player.inf.cs_amount.add(1).log10().add(1).pow(3) : E(1),
      effDesc: (x) => formatMult(x),
    },
    {
    ...
    },
    {
      ...
      get desc() {
        return `${
          EVO.amt >= 4 ? "Stardust" : "Supernova"
        } cheapens Corrupted Star upgrades.`;
      },
      cost: E("e440"),
      eff() {
        let x = E(1);
    ...
    },
    {
      ...
      get desc() {
        return EVO.amt >= 4
          ? `Unlock Theorem Stars 6 - 7.`
          : `Unlock sixth star in the theorem.`;
      },
      get cost() {
        return E(EVO.amt >= 4 ? "e700" : EVO.amt >= 2 ? "e780" : "e840");
      },
    },
    {
    ...
    },
    {
      ...
      eff: () =>
        tmp.inf_unl ? player.inf.theorem.div(10).add(1).root(2) : E(1),
      effDesc: (x) => formatPow(x),
    },
    {
      cs: true,
    ...
    },
    {
      ...
      cost: E("e161800"),
      eff: () =>
        tmp.inf_unl ? expMult(player.gp_resources[4].add(1), 0.5) : E(1),
      effDesc: (x) => formatMult(x) + " later",
    },
    {
      cs: true,
    ...
    },
    {
      ...
      cost: E(500),
      eff: () =>
        EVO.amt >= 1 ? player.evo.cp.level.div(1e7).add(1).sqrt() : E(1),
      effDesc: (x) => formatMult(x),
    },
    {
      berry: true,
    ...
    },
    {
      ...
      cost: E(1e14),
      eff: () => (OURO.unl ? tmp.ouro.apple_gain.pow(0.1).max(3) : E(3)),
      effDesc: (x) => formatMult(x),
    },
    {
      berry: true,
...
function muElemEff(x, def = 1) {
  return tmp.elements.mu_effect[x] || def;
}
...
const EXOTIC_ATOM = {
  ...
  req() {
    ...
    if (t.lt(12)) r = this.requirement[t.toNumber()];
      ...
      r = Decimal.pow("e1000", Decimal.pow(1.25, t.sub(12).div(tmp.ea.req_fp)));
  ...
  gain() {
    ...
    if (hasElement(12, 1) && hasPrestige(1, 247))
      y = y.mul(prestigeEff(1, 247));
  ...
  milestones: [
    [
      [
        (a) => {
          let x = hasElement(32, 1)
            ? expMult(a.add(1), 1.4)
            : overflow(a.add(1).root(2), 100, 0.5);
          return x;
        },
      ...
      [
        ...
        (x) =>
          `Increase the base of Prestige 382 for Collapsed Star's effect, the base of Binilunium-201 for BH's effect by <b>+${format(
            x
          )}</b>`,
      ],
    ],
    [
      [
        ...
        (x) =>
          `Boosts mass of unstable BH gain by <b>${formatMult(x[0])}</b>` +
...
function updateExoticAtomsTemp() {
  let tea = tmp.ea,
    t = player.dark.exotic_atom.tier;
  ...
  tea.amount =
    EVO.amt >= 3 ? player.evo.proto.exotic_atoms : EXOTIC_ATOM.getAmount();
  ...
  for (let i = 0; i < 2; i++) {
    ...
    for (let j = 0; j < m.length; j++)
      if (m[j] && j < Math.floor((tt + 1 - i) / 2)) tea.eff[i][j] = m[j][0](a);
  }
}

function exoticAEff(i, j, def = 1) {
  return tmp.ea.eff[i][j] || def;
}

function updateExoticAtomsHTML() {
  let ea = player.dark.exotic_atom,
    tea = tmp.ea,
    t = ea.tier;
  ...
  tmp.el.mcf_btn.setHTML(`
    ...
    `);
  tmp.el.mcf_btn.setClasses({
    btn: true,
    half_full: true,
    locked: tea.amount.lt(tea.req),
  });

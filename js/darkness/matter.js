const MATTERS = {
  ...
  gain(i) {
    ...
    if (hasElement(227))
      x = c16 ? x.pow(elemEffect(227)) : expMult(x, elemEffect(227));
    if (i < MATTERS_LEN - 1) {
      x = c16
  ...
  firstUpgData(i) {
    ...
    let bulk = (
      c16
        ? player.dark.matters.amt[i].max(1).log(100).root(pow)
        : player.dark.matters.amt[i]
            .max(1)
            .log(1e10)
            .root(pow)
            .sub(1)
  ...
  final_star_shard: {
    base() {
      ...
      for (let i = 0; i < 13; i++)
        x = x.mul(
          player.dark.matters.amt[i].add(1).log10().add(1).log10().add(1)
        );
...
function updateMattersHTML() {
  ...
  if (hasElement(256))
    h += c16 ? `</sup>×(next matter)` : `×lg(next matter)</sup>`;
...
function updateMattersTemp() {
  ...
  tmp.matters.str = E(1);
  if (hasBeyondRank(1, 2))
    tmp.matters.str = tmp.matters.str.mul(beyondRankEffect(1, 2));

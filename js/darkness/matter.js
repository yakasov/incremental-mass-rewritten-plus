const MATTERS = {
  ...
  gain(i) {
    ...
    let x,
      m0 = i == 0 ? tmp.matters.amt_0 : player.dark.matters.amt[i - 1];
    if (c16) {
    ...
    if (rdc == 2) {
      ...
      if (i < MATTERS_LEN - 1) {
        x = rdc
          ? x.mul(tmp.matters.upg[i + 1].eff)
          : x.pow(tmp.matters.upg[i + 1].eff);
        if (hasElement(256))
          x = rdc
            ? x.mul(player.dark.matters.amt[i + 1])
            : x.pow(player.dark.matters.amt[i + 1].max(1).log10().add(1));
      }
      if (!rdc) {
        x = rdc
          ? x.mul(tmp.dark.abEff.mexp || 1)
          : x.pow(tmp.dark.abEff.mexp || 1);
        x = rdc ? x.mul(glyphUpgEff(14, 1)) : x.pow(glyphUpgEff(14, 1));
        if (hasBeyondRank(1, 7))
          x = rdc
            ? x.mul(beyondRankEffect(1, 7))
            : x.pow(beyondRankEffect(1, 7));
      }
      if (hasElement(11, 1) || !rdc)
        x = rdc ? x.mul(tmp.matters.FSS_eff[0]) : x.pow(tmp.matters.FSS_eff[0]);
      if (hasElement(4, 1)) x = rdc ? x.pow(1.1) : expMult(x, 1.05);
      if (hasElement(227))
        x = rdc ? x.pow(elemEffect(227)) : expMult(x, elemEffect(227));
      if (i < MATTERS_LEN - 1)
        x = rdc
          ? x.pow(tmp.matters.upg[i + 1].exp)
          : expMult(x, tmp.matters.upg[i + 1].exp);
    }
  ...
  firstUpgData(i) {
    ...
    let lvl = player.dark.matters.upg[i],
      pow = c16 ? 1.25 : Math.max(i - 2, 0) / 10 + 1.5;
    let cost = c16
      ? Decimal.pow(100, lvl.add(1).pow(pow))
      : Decimal.pow(
          1e10,
          lvl
    ...
    let bulk = (
      c16
        ? m0.max(1).log(100).root(pow)
        : m0
            .max(1)
            .log(1e10)
            .root(pow)
            .sub(1)
            .scale(i > 0 ? 25 : 50, 1.05, 1, true)
            .add(1)
    ).floor();
  ...
  final_star_shard: {
    ...
    req() {
      ...
      f = f.scaleEvery("FSS", false, [
        1,
        hasTree("ct10") ? treeEff("ct10").pow(-1) : 1,
      ]);
    ...
    reset(force = false) {
      if (force || tmp.matters.FSS_base.gte(tmp.matters.FSS_req)) {
        if (!force)
          player.dark.matters.final = player.dark.matters.final.add(1);
...
function updateMattersHTML() {
  ...
  else if (hasElement(256))
    h += tmp.c16.in ? `</sup>×(next matter)` : `×lg(next matter)</sup>`;

  tmp.el.matter_formula.setHTML(h);
  tmp.el.matter_req_div.setDisplay(player.dark.matters.unls < 14);
  if (player.dark.matters.unls < 14)
  ...
  if (unl) {
    ...
    tmp.el.FSS_btn.setClasses({
      btn: true,
      full: true,
      locked: tmp.matters.FSS_base.lt(tmp.matters.FSS_req),
    });
  }

  ...
  upg: [
    ...
    },
    {
      ...
      cost(i) {
        i *= Math.max(1, i - 4) ** 0.5;
        return {
          0: Math.floor(20 + 20 * i),
          1: Math.floor(20 + 20 * i),
          2: Math.floor(20 + 20 * i),
        };
      },
      eff(i) {
        return 3 ** i;
    ...
    },
    {
      ...
      cost(i) {
        return { 1: 200 + 10 * i, 2: 200 + 10 * i, 5: 40 + 5 * i };
      },
      eff(i) {
        return 1.5 ** i;
      },
    ...
    },
    {
      ...
      cost(i) {
        i *= Math.max(1, i - 4) ** 0.5;
        return {
          0: Math.floor(270 + 10 * i),
          3: Math.floor(150 + 10 * i),
          4: Math.floor(140 + 10 * i),
        };
      },
      eff(i) {
        return i / 50;
...
function mgEff(i, def = 1) {
  return tmp.dark.mass_glyph_eff[i] || def;
}
...
function glyphButton(i) {
  if (
    ...
    tmp.dark.mass_glyph_gain[i].gt(0) &&
    !GLYPH_SEL.includes(i)
  ) {
    GLYPH_SEL.push(i);
...
function darkRun(round) {
  ...
  if (run.active)
    for (var i of GLYPH_SEL)
      run.glyphs[i] = run.glyphs[i].add(tmp.dark.mass_glyph_gain[i]);
...
function isAffordGlyphCost(cost) {
  for (let c in cost)
    if (
      Decimal.max(player.dark.run.glyphs[c], tmp.dark.mg_passive[c]).lt(cost[c])
    )
      return false;
  return true;
}

function hasGlyphUpg(i) {
  return player.dark.run.upg[i] > 0;
}
function glyphUpgEff(i, def = 1) {
  return tmp.glyph_upg_eff[i] || def;
}
...
function updateDarkRunHTML() {
  ...
  for (let x = 0; x < MASS_GLYPHS_LEN; x++) {
    let cost =
      gum == 0 ? {} : DARK_RUN.upg[gum].cost(player.dark.run.upg[gum] || 0);
    let shown = gum == 0 ? true : Object.keys(cost).includes(x.toString());
    tmp.el["mass_glyph_img" + x].setClasses({
      glyph: true,
      selected: player.dark.run.active && GLYPH_SEL.includes(x),
    });
  ...
  if (gum > 0) {
    ...
    let desc =
      "<span class='sky'>" +
      (typeof u.desc == "function" ? u.desc() : u.desc) +
      "</span>";
    if (c16 && gum == 14) desc = desc.corrupt();
    msg =
      "[Level " +
      format(ua, 0) +
      (isFinite(max) ? " / " + format(max, 0) : "") +
      "]<br>" +
      desc +
      "<br>";
    ...
    if (ua < max) {
      let cr = "",
        cost = u.cost(ua),
        n = 0,
        cl = Object.keys(cost).length;
      for (let c in cost) {
        cr +=
          format(cost[c], 0) +
          " " +
          DARK_RUN.mass_glyph_name[c] +
          (n + 1 < cl ? ", " : "");
        n++;
      }
  ...
  for (let x = 1; x < GLYPH_UPG_LEN; x++) {
    ...
    tmp.el["glyph_upg" + x].setClasses({
      img_btn: true,
      locked: !isAffordGlyphCost(u.cost(ua)) && ua < max,
      bought: ua >= max,
    });
  }

  ...
  mass_glyph_gain: [
    ...
    () =>
      player.md.mass.gte("e1e21")
        ...
        : E(0),
    () =>
      player.stars.points.gte("e1.5e24")
        ? player.stars.points
            .log10()
            .div(1.5e24)
            .log(1.1)
  ...
  if (isAffordGlyphCost(cost) && ua < max) {
    ...
    for (let c in cost)
      if (tmp.dark.mg_passive[c] <= 0)
        player.dark.run.glyphs[c] = player.dark.run.glyphs[c].sub(cost[c]);
...
function updateDarkRunHTML() {
  ...
  for (let x = 0; x < MASS_GLYPHS_LEN; x++) {
    tmp.el["mass_glyph" + x].setHTML(
      c16
        ? "Corrupted"
        : format(player.dark.run.glyphs[x], 0) +
            (dra

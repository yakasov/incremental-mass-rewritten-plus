...
const BUILDINGS_DATA = {
    mass_1: {
        ...
        get isUnlocked() {
            return player.ranks.rank.gte(1) || player.mainUpg.atom.includes(1);
        },
        get autoUnlocked() {
            return hasUpgrade("rp", 3);
        },
        get noSpend() {
        ...
        cost(x = this.level) {
            return getMassUpgradeCost(1, x);
        },
        get bulk() {
            return getMassUpgradeBulk(1, this.res);
        },

        get_cost: (x) => formatMass(x),

        get beMultiplicative() {
            return hasAscension(0, 1);
        },
        ...
        get bonus() {
            ...
            if (player.mainUpg.rp.includes(1))
                x = x.add(tmp.upgs.main ? tmp.upgs.main[1][1].effect : E(0));
            if (player.mainUpg.rp.includes(2))
                x = hasAscension(0, 1)
                    ? x.mul(tmp.build.mass_2.bonus.add(1))
                    : x.add(tmp.build.mass_2.bonus);
    ...
    mass_2: {
        ...
        get isUnlocked() {
            return player.ranks.rank.gte(2) || player.mainUpg.atom.includes(1);
        },
        get autoUnlocked() {
            return hasUpgrade("rp", 3);
        },
        ...
        cost(x = this.level) {
            return getMassUpgradeCost(2, x);
        },
        get bulk() {
            return getMassUpgradeBulk(2, this.res);
        },
        ...
        get bonus() {
            let x = E(0);
            if (player.mainUpg.rp.includes(2))
                x = x.add(tmp.upgs.main ? tmp.upgs.main[1][2].effect : E(0));
            if (player.mainUpg.rp.includes(7))
                x = hasAscension(0, 1)
                    ? x.mul(tmp.build.mass_3.bonus.add(1))
                    : x.add(tmp.build.mass_3.bonus);
            x = x.mul(getEnRewardEff(4));
            return x;
        },
    ...
    mass_3: {
        ...
        get isUnlocked() {
            return player.ranks.rank.gte(3) || player.mainUpg.atom.includes(1);
        },
        get autoUnlocked() {
            return hasUpgrade("rp", 3);
        },
        ...
        cost(x = this.level) {
            return getMassUpgradeCost(3, x);
        },
        get bulk() {
            return getMassUpgradeBulk(3, this.res);
        },
        ...
        effect(x) {
            ...
            if (player.mainUpg.bh.includes(9))
                ss = ss.add(tmp.upgs.main ? tmp.upgs.main[2][9].effect : E(0));
            let step = E(1);
            if (player.ranks.tetr.gte(2))
                step = step.add(RANKS.effect.tetr[2]());
        ...
        get bonus() {
            let x = E(0);
            if (player.mainUpg.rp.includes(7))
                x = x.add(tmp.upgs.main ? tmp.upgs.main[1][7].effect : 0);
            x = x.mul(getEnRewardEff(4));
            return x;
        },
    ...
    mass_4: {
        ...
        cost(x = this.level) {
            return getMassUpgradeCost(4, x);
        },
        get bulk() {
            return getMassUpgradeBulk(4, this.res);
        },
    ...
    tickspeed: {
        ...
        get allowPurchase() {
            return !CHALS.inChal(2) && !CHALS.inChal(6) && !CHALS.inChal(10);
        },

        get res() {
            return player.rp.points;
        },
        set res(v) {
            player.rp.points = v;
        },
        ...
        effect(x) {
            ...
            if (hasBeyondRank(2, 4))
                step = step.pow(BUILDINGS.eff("accelerator"));
            ...
            if (hasElement(199) && !CHALS.inChal(15)) {
                eff = eff
                    ...
                    .add(9)
                    .log10()
                    .pow(BUILDINGS.eff("accelerator").mul(0.1));
        ...
        get_effect: (x) =>
            (hasElement(199) && !CHALS.inChal(15)
                ? "^" + format(x.effect)
                : formatMult(x.effect)) + " to mass gain",
    },
    accelerator: {
        name: "Accelerator",
        icon: "accelerator",
        ...
        get isUnlocked() {
            return player.rp.unl && hasElement(199);
        },
        get autoUnlocked() {
            return true;
        },
    ...
    bhc: {
        ...
        get allowPurchase() {
            return !CHALS.inChal(6) && !CHALS.inChal(10);
        },
        ...
        get bulk() {
            ...
            return this.res
                .max(1)
                .log(1.75)
                .scaleEvery("bh_condenser", true, [1, 1, 1, fp])
                .mul(fp2)
                .add(1)
                .floor();
        },
        ...
        get bonus() {
            ...
            if (player.mainUpg.bh.includes(15))
                x = x.add(tmp.upgs.main ? tmp.upgs.main[2][15].effect : E(0));
            x = x.mul(getEnRewardEff(4));
            return x;
        },
    ...
    fvm: {
        ...
        get isUnlocked() {
            return player.bh.unl && hasCharger(1);
        },
        get autoUnlocked() {
            return true;
        },
        ...
        get allowPurchase() {
            return tmp.c16active || hasElement(277);
        },
        ...
        get_power: (x) => formatMult(x.power),
        get_effect: (x) =>
            formatMult(x.effect) + " to Unstable BH production speed",
    },
    cosmic_ray: {
        name: "Cosmic Ray",
        icon: "gamma_ray",
        ...
        get res() {
            return player.atom.points;
        },
        set res(v) {
            player.atom.points = v;
        },
        ...
        get bulk() {
            ...
            return this.res
                .max(1)
                .log(2)
                .scaleEvery("gamma_ray", true, [1, 1, 1, fp])
                .mul(fp2)
                .add(1)
                .floor();
        },
    ...
    star_booster: {
        ...
        effect(x) {
            ...
            let eff = pow
                .pow(x.mul(tmp.chal ? tmp.chal.eff[11] : 1))
                .softcap("e3e18", 0.95, 2, a22);
        ...
        get_power: (x) =>
            formatMult(x.power) +
            x.power.softcapHTML(1e13, hasUpgrade("atom", 22)),
        get_effect: (x) =>
            formatMult(x.effect) +
            " to star generators" +
            x.effect.softcapHTML("e3e18", hasUpgrade("atom", 22)),
    },
    cosmic_string: {
        name: "Cosmic String",
        icon: "cosmic_string",
...
Object.keys(BUILDINGS_DATA).forEach((i) => {
    ...
    Object.defineProperty(b, "level", {
        get() {
            return player.build[i].amt;
        },
        set(value) {
            player.build[i].amt = value;
        },
    });
});
...
const BUILDINGS = {
    ...
    tick() {
        for (var [i, b] of Object.entries(BUILDINGS_DATA)) {
            if (b.isUnlocked && b.autoUnlocked && player.build[i].auto)
                this.buy(i, true);
        }
    },
    temp() {
        ...
        for (var i of BUILDINGS_ORDER) {
            ...
            if (b.isUnlocked || b.forceEffect) {
                let bonus = b.bonus;
                let total = b.beMultiplicative
                    ? b.level.add(1).mul(bonus.add(1)).sub(1)
                    : b.level.add(bonus);
    ...
    setup() {
        for (var [i, b] of Object.entries(BUILDINGS_DATA)) {
            ...
            if (el.el)
                el.setHTML(`<div class="table_center upgrade" style="width: 100%; margin-bottom: 5px;">
    ...
    update(i) {
        ...
        tmp.el["building_lvl_" + i].setHTML(
            b.level.format(0) +
                (bt.bonus.gt(0)
...
function getMassUpgradeBulk(i) {
    ...
    if (i == 4) {
        ...
        if (player.mass.gte(start))
            bulk = player.mass
                .div(start)
                .max(1)
                .log(inc)
                .scaleEvery("massUpg", true)
                .mul(fp)
                .add(1)
                .floor();
    }
...
function checkBuildings() {
    ...
    if (
        b.amt.lte(0) &&
        player.accelerator &&
        Decimal.gt(player.accelerator, 0)
    ) {
    ...
    if (
        b.amt.lte(0) &&
        player.bh.condenser &&
        Decimal.gt(player.bh.condenser, 0)
    ) {
    ...
    if (
        ...
        Decimal.gt(player.atom.gamma_ray, 0)
    ) {
    ...
    if (
        b.amt.lte(0) &&
        player.stars.boost &&
        Decimal.gt(player.stars.boost, 0)
    ) {
    ...
    if (
        ...
        Decimal.gt(player.qu.cosmic_str, 0)
    ) {

const RANKS = {
    ...
    reset(type) {
        if (tmp.ranks[type].can) {
            ...
            else if (type == "rank" && player.mainUpg.rp.includes(4))
                reset = false;
            else if (type == "tier" && player.mainUpg.bh.includes(4))
                reset = false;
    ...
    unl: {
        tier() {
            return (
                player.ranks.rank.gte(3) ||
                player.ranks.tier.gte(1) ||
                player.mainUpg.atom.includes(3) ||
                tmp.radiation.unl ||
                tmp.inf_unl
            );
        },
    ...
    effect: {
        rank: {
            ...
            40() {
                ...
                if (player.ranks.rank.gte(220))
                    ret = player.ranks.rank.div(100);
                return ret;
            },
            45() {
        ...
        tier: {
            ...
            6() {
                ...
                if (player.ranks.tier.gte(8))
                    ret = ret.pow(RANKS.effect.tier[8]());
        ...
        hex: {
            4() {
                ...
                if (hex.gte(43))
                    ret = ret.pow(hex.min(1e18).div(10).add(1).root(2));
...
const PRESTIGES = {
    ...
    req(i) {
        ...
        switch (i) {
            ...
            case 3:
                x = y
                    .scaleEvery("prestige3", false, [0, fp])
                    .pow(1.25)
                    .mul(3)
                    .add(9);
                break;
        ...
        },
        {
            ...
            45: [
                () => {
                    ...
                    let x = hasElement(224)
                        ? Decimal.pow(1.1, y.root(4))
                        : y.add(1);
                    if (tmp.c16active)
                        x = overflow(x.log10().add(1).root(2), 10, 0.5);
...
function hasPrestige(x, y) {
    return (
        ...
        !(tmp.c16active && CORRUPTED_PRES[x] && CORRUPTED_PRES[x].includes(y))
    );
}
...
function updateRanksTemp() {
    ...
    let rt_fp2 =
        !hasElement(243) && hasPrestige(1, 127)
            ? tmp.c16active
                ? 5e2
                : 1
            : fp2;
    ...
    for (let x = 0; x < PRES_LEN; x++) {
        ...
        for (let y in PRESTIGES.rewardEff[x]) {
            if (PRESTIGES.rewardEff[x][y])
                tmp.prestiges.eff[x][y] = PRESTIGES.rewardEff[x][y][0]();
        }
    }
...
const BEYOND_RANKS = {
    req() {
        ...
        rc.reduction = p.gte(rc.start)
            ? x.log(p.pow(1.25).mul(10).add(180))
            : E(1);
...
function updateRanksHTML() {
    ...
    if (tmp.rank_tab == 0) {
        for (let x = 0; x < RANKS.names.length; x++) {
            ...
            let unl =
                (!tmp.brUnl || x > 3) &&
                (RANKS.unl[rn] ? RANKS.unl[rn]() : true);
        ...
        if (unl) {
            ...
            for (tt in BEYOND_RANKS.rewards) {
                ...
                for (tr in BEYOND_RANKS.rewards[tt]) {
                    ...
                    if (
                        t.lt(tt) ||
                        (tmp.beyond_ranks.latestRank.lt(tr) && t.eq(tt))
                    ) {
                        r =
                            "At " +

    ...
    rank: {
      ...
      6() {
        let ret = player.ranks.rank
          .add(1)
          .pow(
            player.ranks.rank.gte(17) ? player.ranks.rank.add(1).root(3) : 2
          );
        return ret;
      },
      40() {
...
function updateRanksTemp() {
  ...
  let rt_fp2 =
    !hasElement(243) && hasPrestige(1, 127) ? (tmp.c16active ? 5e2 : 1) : fp2;
  let ffp = E(1);
  let ffp2 = 1;
      ...
      if (unl) {
        ...
        for (let i = 0; i < keys.length; i++) {
          if (player.ranks[rn].lt(keys[i])) {
            desc = ` At ${RANKS.fullNames[x]} ${format(keys[i], 0)} - ${
      ...
      if (unl) {
        ...
        for (let i = 0; i < keys.length; i++) {
          if (
            ...
            desc = ` At ${PRESTIGES.fullNames[x]} ${format(keys[i], 0)} - ${

const RANKS = {
  ...
  unl: {
    tier() {
      return (
        EVO.amt >= 3 ||
        player.ranks.rank.gte(1) ||
        player.ranks.tier.gte(1) ||
        FORMS.rp.unl()
      );
    },
    tetr() {
      return (
        EVO.amt >= 3 || hasUpgrade("atom", 3) || hasTree("unl1") || tmp.inf_unl
      );
    },
    pent() {
      return EVO.amt >= 4 || hasTree("unl1") || tmp.inf_unl;
    },
    hex() {
  ...
  autoUnl: {
    rank() {
      return EVO.amt >= 1 || player.ranks.tier.gte(1) || FORMS.rp.unl();
    },
    tier() {
      return EVO.amt >= 1 || hasUpgrade("rp", 6);
    },
    tetr() {
      return EVO.amt >= 1 || hasUpgrade("atom", 4);
    },
    pent() {
      return EVO.amt >= 1 || hasTree("qol8");
    },
    hex() {
      return true;
    },
  ...
  effect: {
    rank: {
      ...
      40() {
        ...
        if (player.ranks.rank.gte(90))
          ret = player.ranks.rank.root(1.6).div(100);
    ...
    tier: {
      4() {
        ...
        if (player.ranks.tier.gte(12)) ret = player.ranks.tier.mul(0.1);
          ...
          ret = player.ranks.tier.mul(0.05).add(1).softcap(1.4, 0.75, 0).sub(1);
        return ret;
      },
      6() {
...
const PRESTIGES = {
  ...
  req(i) {
    ...
    switch (i) {
      ...
      case 2:
        x = hasElement(167)
          ? y.div(fp).scaleEvery("prestige2", false).pow(1.25).mul(3.5).add(5)
          : y.pow(1.3).mul(4).add(6);
        break;
      case 3:
  ...
  bulk(i) {
    ...
    switch (i) {
      case 0:
        if (y.gte(EVO.amt >= 2 ? 2e12 : 2e13))
          x = y
            .div(EVO.amt >= 2 ? 2e12 : 2e13)
            .max(1)
            .log(1.1)
            .max(0)
            .root(1.1)
            .scaleEvery("prestige0", true, [0, 0, 0, fp])
            .add(1);
        break;
      case 1:
        if (y.gte(4))
        ...
        if (y.gte(6))
          x = hasElement(167)
            ? y
                ...
                .max(0)
                .root(1.25)
                .scaleEvery("prestige2", true)
                .mul(fp)
                .add(1)
            : y.sub(6).div(4).max(0).root(1.3).mul(fp).add(1);
        break;
      case 3:
        if (y.gte(9))
          x = y
            ...
            .max(0)
            .root(1.25)
            .scaleEvery("prestige3", true, [0, fp])
            .add(1);
        break;
      case 4:
        if (y.gte(12))
  ...
  fp(i) {
    ...
    if (hasUpgrade("br", 19) && i < (hasAscension(1, 1) ? 4 : 3))
      fp = fp.mul(upgEffect(4, 19));
    return fp;
  },
  unl: [
    () => true,
  ...
  rewards: [
    ...
    },
    {
      ...
      45: `Unstable BH affects Black Hole Mass overflow^2 starting.`,
  ...
  rewardEff: [
    {
      ...
      233: [
        () => {
          let x = player.dark.matters.amt[0]
            .add(1)
            .log10()
            .add(1)
            .log10()
    ...
    },
    {
      ...
      34: [
        () => {
          let x = player.dark.exotic_atom.amount[1]
            .add(1)
            .log10()
            .add(1)
            .pow(1.5);
      ...
      58: [
        () => {
          let x = tmp.beyond_ranks.max_tier.mul(0.05);
          return x;
        },
        (x) => "+" + formatPercent(x),
      ],
  ...
  reset(i, bulk = false) {
    ...
    if (
      i == 0
        ? tmp.prestiges.base.gte(tmp.prestiges.req[i])
        : player.prestiges[i - 1].gte(tmp.prestiges.req[i])
    )
...
function hasPrestige(x, y) {
  return (
    player.prestiges[x].gte(y) &&
    (x || !tmp.c16.in || CORRUPTED_PRES.includes(y))
  );
}
...
function updateRanksTemp() {
  ...
  for (let x = 0; x < RANKS.names.length; x++)
    if (!tmp.ranks[RANKS.names[x]]) tmp.ranks[RANKS.names[x]] = {};
  let ifp = E(1);
  if (tmp.inf_unl) ifp = ifp.mul(theoremEff("mass", 2));
  let fp2 = tmp.qu.chroma_eff[1][0];
  let tetr_fp2 =
    hasAscension(0, 15) && EVO.amt >= 4
      ? 1
      : !hasElement(243) && hasCharger(8)
      ? 1
      : fp2;
  let rt_fp2 =
    !hasElement(243) && hasPrestige(1, 127) ? (tmp.c16.in ? 5e2 : 1) : fp2;
  ...
  if (player.mass.gte(5))
    tmp.ranks.rank.bulk = player.mass
      ...
      .mul(fp)
      .root(rooted_fp)
      .scaleEvery("rank", true, [1, 1, 1, 1, rt_fp2, 1, ifp])
      .mul(ffp2)
      .add(1)
      .floor();
  tmp.ranks.rank.can =
    player.mass.gte(tmp.ranks.rank.req) &&
    !CHALS.inChal(5) &&
    !CHALS.inChal(10) &&
    !FERMIONS.onActive("03");
  ...
  fp = E(1).mul(ffp);
  let fpa =
    hasPrestige(1, 33) && !hasElement(311)
      ? [1, 1, 1, prestigeEff(1, 33, 1)]
      : [];
  ...
  for (let x = 0; x < RANKS.names.length; x++) {
    ...
    if (x > 0)
      tmp.ranks[rn].can =
        RANKS.gain(rn).gt(0) &&
        player.ranks[RANKS.names[x - 1]].gte(tmp.ranks[rn].req);
  }
  ...
  for (let x in BEYOND_RANKS.rewardEff) {
    for (let y in BEYOND_RANKS.rewardEff[x]) {
      if (hasBeyondRank(x, y) && BEYOND_RANKS.rewardEff[x][y])
        tmp.beyond_ranks.eff[x][y] = BEYOND_RANKS.rewardEff[x][y][0]();
    }
  }
}
...
const BEYOND_RANKS = {
  req() {
    let p = player.ranks.beyond,
      rc = tmp.ranks.collapse;
  ...
  bulk() {
    ...
    let x = player.ranks.hex.gte(180)
      ? player.ranks.hex
          .sub(180)
          .div(10)
          .max(0)
          .root(1.25)
          .scale(rc.start, rc.power, 2, true)
          .add(1)
          .floor()
      : E(0);
  ...
  getTier(r = player.ranks.beyond) {
    let x = r.gt(0)
      ? r
          ...
          .max(0)
          .pow(0.8)
          .mul(tmp.beyond_ranks.tier_power)
          .add(1)
          .scale(
  ...
  getRankDisplayFromValue(r) {
    let tier = this.getTier(r),
      current = this.getRankFromTier(tier, r);
    return getRankTierName(tier.add(5)) + " " + current.format(0);
  },

  reset(auto = false) {
    if (
      player.ranks.hex.gte(tmp.beyond_ranks.req) &&
      (!auto || tmp.beyond_ranks.bulk.gt(player.ranks.beyond))
    ) {
      player.ranks.beyond = auto
  ...
  rewards: {
    ...
    2: {
      ...
      10: `Raise Black Hole Mass to the 1.2th power.`,
      15: `Remove all scalings from mass upgrades 1-3.`,
      17: `[qu9] is more effective based on Black Hole Mass. Exotic Supernova starts later based on Quantizes.`,
  ...
  rewardEff: {
    1: {
      ...
      4: [
        () => player.dark.matters.final.pow(0.75).div(10).add(1),
        (x) => formatPercent(x - 1) + " stronger",
      ],
      7: [() => player.ranks.beyond.add(1).root(2), (x) => formatPow(x)],
    },
    2: {
      1: [
        () => {
      ...
      7: [
        () => {
          let x = hasPrestige(4, 12)
            ? player.ranks.beyond.add(1).pow(0.4)
            : player.ranks.beyond
                .add(1)
                .log10()
                .add(1)
      ...
      17: [
        () => {
          let x = tmp.bh.unl
            ? player.bh.mass.add(1).log10().add(1).log10().add(1).pow(2)
            : E(1);
    ...
    4: {
      1: [
        () => {
          let x = overflow(tmp.qu.prim.eff[7].div(5), 1e6, 0.5).softcap(
            1e7,
            1 / 3,
            0
          );
      ...
      2: [
        () => {
          let x = tmp.beyond_ranks.max_tier
            .sub(3)
            .pow(hasBeyondRank(14, 1) ? 1 : 0.2)
            .mul(0.2)
            .add(1); // (tmp.beyond_ranks.max_tier-3)**0.2*0.2+1
...
function updateRanksHTML() {
  ...
  for (let x = 0; x < 3; x++)
    tmp.el["rank_tab" + x].setDisplay(tmp.ranks.tab == x);

  if (tmp.ranks.tab == 0) {
    for (let x = 0; x < RANKS.names.length; x++) {
      let rn = RANKS.names[x];
      let unl =
        (!tmp.brUnl || x > 3) && (RANKS.unl[rn] ? RANKS.unl[rn]() : true);
      ...
      if (unl) {
        ...
        for (let i = 0; i < keys.length; i++) {
          if (player.ranks[rn].lt(keys[i])) {
            desc = `<br class='line'>${RANKS.fullNames[x]} ${format(
              keys[i],
              0
        ...
        tmp.el["ranks_amt_" + x].setTxt(
          format(player.ranks[rn], 0) +
            (!RANKS.autoUnl[rn]() && can ? "+" + format(RANKS.gain(rn), 0) : "")
        );
        tmp.el["ranks_" + x].setClasses({
          btn: true,
          reset: true,
          locked: !can,
        });
        tmp.el["ranks_desc_" + x].setHTML(desc);
        tmp.el["ranks_req_" + x].setTxt(
          x == 0
            ? formatMass(tmp.ranks[rn].req)
            : RANKS.fullNames[x - 1] + " " + format(tmp.ranks[rn].req, 0)
        );
      }
    }
    ...
    if (unl) {
      ...
      for (let x = Math.min(3, t.toNumber()) - 1; x >= 0; x--) {
        h +=
          getRankTierName(t.add(5).sub(x)) +
          " <h4>" +
          (x == 0
            ? tmp.beyond_ranks.latestRank.format(0)
            : BEYOND_RANKS.getRankFromTier(t.sub(x)).format(0)) +
          "</h4>" +
          (x > 0 ? "<br>" : "");
      }
      ...
      for (tt in BEYOND_RANKS.rewards) {
        ...
        for (tr in BEYOND_RANKS.rewards[tt]) {
          ...
          if (t.lt(tt) || (tmp.beyond_ranks.latestRank.lt(tr) && t.eq(tt))) {
            r =
              "<br class='line'>" +
              getRankTierName(tt + 5) +
              " " +
              format(tr, 0) +
              ": " +
              BEYOND_RANKS.rewards[tt][tr];
            b = true;
            break;
          }
        }
        ...
        t == 1
          ...
          : BEYOND_RANKS.getRequirementFromTier(
              ...
              t.sub(1)
                ...
                ${getRankTierName(t.add(6))} up: ${getRankTierName(
        t.add(5)
      ...
      tmp.el.br_desc.setClasses({
        btn: true,
        reset: true,
        locked: player.ranks.hex.lt(tmp.beyond_ranks.req),
      });
    }
    ...
    for (let x = 0; x < PRES_LEN; x++) {
      ...
      if (unl) {
        ...
        for (let i = 0; i < keys.length; i++) {
          if (
            p.lt(keys[i]) &&
            (tmp.chal13comp || p.lte(PRES_BEFOREC13[x] || Infinity))
          ) {
            desc = `<br class='line'>${PRESTIGES.fullNames[x]} ${format(
              keys[i],
              0
        ...
        tmp.el["pres_" + x].setClasses({
          ...
          locked:
            x == 0
              ? tmp.prestiges.base.lt(tmp.prestiges.req[x])
              : player.prestiges[x - 1].lt(tmp.prestiges.req[x]),
        });
        tmp.el["pres_desc_" + x].setHTML(desc);
        tmp.el["pres_req_" + x].setTxt(
          x == 0
            ? format(tmp.prestiges.req[x], 0) + " of Prestige Base"
            : PRESTIGES.fullNames[x - 1] + " " + format(tmp.prestiges.req[x], 0)
        );
      }
    }
...
const GAL_PRESTIGE = {
  req: () =>
    E(10)
      .pow(
        player.gal_prestige
          .scaleEvery("gal_prestige")
          .pow(EVO.amt >= 4 && hasElement(281) ? 1.25 : 1.5)
      )
      .mul(EVO.amt >= 4 ? 1e30 : EVO.amt >= 2 ? 1e13 : 1e17),
  reset() {
  ...
  gain(i) {
    let x = E(0),
      gp = player.gal_prestige;

    switch (i) {
      case 0:
        if (gp.gte(1) && EVO.amt < 4)
          x = player.stars.points
            .add(1)
            .log10()
            .add(1)
            .log10()
      ...
      case 1:
        if (gp.gte(2))
          x = tmp.prestiges.base
            .add(1)
            .log10()
            .add(1)
            .pow(gp.sub(1).root(1.5))
            .sub(1);
        break;
      case 2:
        if (gp.gte(4))
          x = player.dark.matters.amt[12]
            .add(1)
            .log10()
            .add(1)
            .log10()
      ...
      case 3:
        if (gp.gte(6))
          x = (
            EVO.amt >= 4 && OURO.unl
              ? player.evo.proto.star
              : player.supernova.radiation.hz.add(1).log10()
          )
            .add(1)
            .log10()
            .add(1)
            .pow(2)
            .pow(gp.sub(5).root(1.5))
            .sub(1);
        break;
      case 4:
        if (gp.gte(9))
          x = player.inf.cs_amount
            .add(1)
            .log10()
            .add(1)
            .pow(2)
            .pow(gp.sub(8).root(1.5))
            .sub(1);
        break;
      case 5:
        if (gp.gte(14) && EVO.amt < 4)
          x = player.supernova.bosons.hb
            .add(10)
            .log10()
            .log10()
            .add(1)
            .pow(gp.sub(13).root(1.5))
            .sub(1);
        break;
      case 6:
        if (gp.gte(2) && EVO.amt >= 4)
          x = E(5).pow(gp).mul(player.build.mass_4.amt.pow(2)).div(1e6);
        break;
    }
...
function GPEffect(i, def = 1) {
  return tmp.gp.res_effect[i] || def;
}
...
function updateGPHTML() {
  ...
  if (unl) {
    ...
    tmp.el.gp_btn.setHTML(`
        ...
        Req: ${tmp.gp.res.format()} / ${tmp.gp.req.format()} ${
      evo >= 4 ? "Stardust" : "Supernovae"
    }
        `);
    tmp.el.gp_btn.setClasses({
      btn: true,
      galactic: true,
      locked: tmp.gp.res.lt(tmp.gp.req),
    });

    let h = "",
      res = player.gp_resources,
      res_gain = tmp.gp.res_gain,
      res_effect = tmp.gp.res_effect;
    ...
    if (gp.gte(2))
      h += `You have <h4>${formatMass(res[1])}</h4> ${res[1].formatGain(
        ...
        which ${
          evo >= 2 ? "raises Quarks" : "weakens mass overflow^1-2"
        } by <h4>${
        evo >= 2
          ? "^" + format(res_effect[1].pow(-1)) + " on exponent"
          : formatReduction(res_effect[1])
      }</h4>.<br>`;

    if (gp.gte(2) && evo >= 4)
      h += `You have <h4>${res[6].format(0)}</h4> ${res[6].formatGain(
        res_gain[6]
      )} Limitless Dust (based on overpower and galactic prestige), 
        which scales Stronger Overflows by <h4>${formatMult(
          res_effect[6]
        )}</h4>.<br>`;
    ...
    if (gp.gte(6))
      h += `You have <h4>${res[3].format(0)}</h4> ${res[3].formatGain(
        ...
        evo >= 4 ? "protostars" : "frequency"
      } and galactic prestige), 

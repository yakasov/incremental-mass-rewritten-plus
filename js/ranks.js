const RANKS = {
  ...
  unl: {
    ...
    tetr() {
      return (
        OURO.evo >= 3 || hasUpgrade("atom", 3) || hasTree("unl1") || tmp.inf_unl
      );
    },
    pent() {
      return hasTree("unl1") || tmp.inf_unl;
    },
    hex() {
  ...
  autoUnl: {
    rank() {
      return OURO.evo >= 1 || player.ranks.tier.gte(1) || FORMS.rp.unl();
    },
    tier() {
      return OURO.evo >= 1 || hasUpgrade("rp", 6);
    },
    tetr() {
      return OURO.evo >= 1 || hasUpgrade("atom", 5);
    },
    pent() {
      return OURO.evo >= 1 || hasTree("qol8");
    },
    hex() {
      return true;
    },
...
const PRESTIGES = {
  ...
  bulk(i) {
    ...
    switch (i) {
      case 0:
        if (y.gte(OURO.evo >= 2 ? 2e12 : 2e13))
          x = y
            .div(OURO.evo >= 2 ? 2e12 : 2e13)
            .max(1)
            .log(1.1)
            .max(0)
            .root(1.1)
...
function updateRanksTemp() {
  ...
  tmp.rank_collapse.start = OURO.evo >= 1 ? EINF : rcs;
...
const BEYOND_RANKS = {
  ...
  getRankFromTier(i, r = player.ranks.beyond) {
    let hp = Decimal.pow(
      ...
      Decimal.pow(
        Decimal.sub(
          i.scale(tmp.beyond_ranks.scale_start, tmp.beyond_ranks.scale_pow, 0),
          1,
...
function updateRanksHTML() {
  ...
  if (tmp.rank_tab == 0) {
      ...
      if (unl) {
        ...
        tmp.el["ranks_amt_" + x].setTxt(
          format(player.ranks[rn], 0) +
            (gain.gt(0) ? "+" + format(gain, 0) : ""),
        );
        tmp.el["ranks_" + x].setClasses({
          btn: true,
          reset: true,
          locked: !tmp.ranks[rn].can,
        });
    ...
    tmp.el.rankCollapse.setHTML(
      `Because of Rank Collapse at <b>${BEYOND_RANKS.getRankDisplayFromValue(rc.start)}</b>, Hept's requirement is raised by <b>${rc.reduction.format()}</b>!`,
    );
  } else if (tmp.rank_tab == 1) {
    tmp.el.pres_base.setHTML(
      `${tmp.prestiges.baseMul.format(0)}<sup>${format(tmp.prestiges.baseExp)}</sup> = ${tmp.prestiges.base.format(0)}`,
    );
...
const GAL_PRESTIGE = {
  ...
  gain(i) {
    ...
    switch (i) {
      ...
      case 3:
        if (gp.gte(6)) {
          x = player.supernova.radiation.hz
            .add(1)
            .log10()
            .add(1)
            .log10()
...
function updateGPHTML() {
  ...
  if (unl) {
    ...
    tmp.el.gp_btn.setClasses({
      btn: true,
      galactic: true,
      locked: player.supernova.times.lt(tmp.gp.req),
    });
    ...
    if (gp.gte(1))
      h += `You have <h4>${res[0].format(0)}</h4> ${res[0].formatGain(res_gain[0])} Galactic Stars (based on collapsed stars and galactic prestige), 
        which strengthens star generators by <h4>${formatPercent(res_effect[0].sub(1))}</h4> exponentially.<br>`;

    if (gp.gte(2))
      h += `You have <h4>${formatMass(res[1])}</h4> ${res[1].formatGain(res_gain[1], true)} of Prestige Mass (based on prestige base and galactic prestige), 
    ...
    if (gp.gte(4))
      h += `You have <h4>${res[2].format(0)}</h4> ${res[2].formatGain(res_gain[2])} Galactic Matter (based on fading matter and galactic prestige), 
        which increases to the base of all Matter upgrades by <h4>+${format(res_effect[2])}</h4>.<br>`;

    if (gp.gte(6))
      h += `You have <h4>${res[3].format(0)}</h4> ${res[3].formatGain(res_gain[3])} Redshift (based on frequency and galactic prestige), 
        which reduces Rank requirement by <h4>^${format(res_effect[3], 5)}</h4>.<br>`;

    if (gp.gte(9))
      h += `You have <h4>${res[4].format(0)}</h4> ${res[4].formatGain(res_gain[4])} Normal Energy (based on corrupted star and galactic prestige), 
        which weaken corrupted star reduction by <h4>${formatReduction(res_effect[4])}</h4>.<br>`;

    if (gp.gte(14))
      h += `You have <h4>${res[5].format(0)}</h4> ${res[5].formatGain(res_gain[5])} Dilatons (based on higgs bosons and galactic prestige), 

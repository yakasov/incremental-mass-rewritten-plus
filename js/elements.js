function setupHTML() {
  ...
  for (let x = 0; x < TABS[1].length; x++) {
    table += `<div>
      ...
      TABS[1][x].icon
        ? `<iconify-icon icon="${TABS[1][x].icon}" width="72" style="color: ${
        ...
        if (unl) {
          tmp.el["pres_reward_" + x + "_" + y].setClasses({
            corrupted_text2:
              c16 &&
              CORRUPTED_PRES[x] &&
              CORRUPTED_PRES[x].includes(parseInt(keys[y])),
          });
  ...
  if (player.main_upg_msg[0] != 0) {
    ...
    if (upg2.effDesc !== undefined)
      msg +=
        ...
        tmp.upgs.main[player.main_upg_msg[0]][player.main_upg_msg[1]].effDesc +
        "</span>";
...
function updateBlackHoleHTML() {
  ...
  tmp.el.bhEffect.setTxt(
    hasElement(201) ? "^" + format(tmp.bh.effect) : format(tmp.bh.effect) + "x"
  );
...
function updateOptionsHTML() {
  if (tmp.stab[9] == 0) {
    ...
    tmp.el.mass_dis.setTxt(
      ["Default", "Always show g", "Always show mlt", "Important units only"][

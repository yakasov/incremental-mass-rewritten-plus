...
function updateUpperHTML() {
  ...
  if (mode == "chal") {
    ...
    tmp.el.chal_upper
      .setHTML(`You are now in [${CHALS[player.chal.active].title}] Challenge! Go over ${tmp.chal.format(tmp.chal.goal[player.chal.active]) + CHALS.getResName(player.chal.active)} to complete.
  ...
  if (mode == "req") {
    ...
    tmp.el.requirement.setHTML(
      `<b class='${u.req ? "green" : "red"}'>[${player.supernova.pin_req}]</b> ${typeof u.reqDesc == "function" ? u.reqDesc() : u.reqDesc}`,
    );
  }
  ...
  if (mode == "upg") {
    ...
    if (nt[0] == "el")
      tmp.el.upg_notify_msg.setHTML(
        `[!] ${["", "Muonic "][nt[1]] + ELEMENTS.fullNames[nt[2]]} is available! [!]`,
      );
    if (nt[0] == "sn")
      tmp.el.upg_notify_msg.setHTML(
        `[!] Neutron Tree [${nt[1]}] is available! [!]`,
      );
    if (nt[0] == "ch")
      tmp.el.upg_notify_msg.setHTML(`[!] Charger [${nt[1]}] is available! [!]`);
  }

  tmp.el.saved.setDisplay(mode == "saved");
  if (mode == "saved")
    tmp.el.saved.setOpacity(0.2 - Math.abs(5 - tmp.ouro.time) / 25);
}
        ...
        if (unl)
          if (tmp.el["ranks_eff_" + rn + "_" + y])
            tmp.el["ranks_eff_" + rn + "_" + y].setTxt(
              RANKS.effDesc[rn][keys[y]](RANKS.effect[rn][keys[y]]()),
            );
      }
    }
  }
      ...
      for (let y = 0; y < keys.length; y++) {
        ...
        if (unl) {
          tmp.el["pres_reward_" + x + "_" + y].setClasses({
            corrupted_text2:
              c16 && x == 0 && CORRUPTED_PRES.includes(parseInt(keys[y])),
          });
  ...
  if (tmp.upgs.msg[0] != 0) {
    ...
    if (upg2.effDesc !== undefined)
      msg +=
        ...
        upg2.effDesc(tmp.upgs[tmp.upgs.msg[0]][tmp.upgs.msg[1]].effect) +
        "</span>";
  ...
  for (let x = 1; x <= UPGS.main.cols; x++) {
    ...
    if (unl) {
      tmp.el["main_upg_" + x + "_res"].setTxt(
        `You have ${format(upg.res, 0)} ${upg.resName}`,
      );
      for (let y = 1; y <= upg.lens; y++) {
        ...
        if (unl2)
          tmp.el["main_upg_" + x + "_" + y].setClasses({
            img_btn: true,
            locked: !canGetUpgrade(id, y),
            bought: hasUpgrade(id, y),
          });
      }
...
function updateBlackHoleHTML() {
  ...
  tmp.el.bhEffect.setTxt(
    hasElement(201) ? formatPow(tmp.bh.effect) : format(tmp.bh.effect) + "x",
  );
  ...
  tmp.el.bhOverflow.setHTML(
    `Because of black hole mass overflow at <b>${formatMass(tmp.overflow_start.bh[0])}</b>, your mass of black hole gain is ${overflowFormat(tmp.overflow.bh || 1)}!`,
  );

  tmp.el.bhOverflow2.setDisplay(player.bh.mass.gte(tmp.overflow_start.bh[1]));
  tmp.el.bhOverflow2.setHTML(
    `Because of black hole mass overflow^2 at <b>${formatMass(tmp.overflow_start.bh[1])}</b>, your black hole mass overflow is even stronger!`,
  );
  ...
  tmp.el.bhcEffectOverflow.setHTML(
    `Because of BH Condenser siltation at <b>${format(tmp.overflow_start.BHCEffect[0])}</b>, the exponent of BH Condenser's effect is ${overflowFormat(tmp.overflow.BHCEffect || 1)}!`,
  );
  ...
  if (unl) {
    tmp.el.bhUnstable.setHTML(
      formatMass(player.bh.unstable) +
        " " +
        formatGain(player.bh.unstable, UNSTABLE_BH.calcProduction(), true),
    );
...
function updateOptionsHTML() {
  if (tmp.tab_name == "options") {
    ...
    tmp.el.prefer_notify.setTxt(
      "Notifications: " + (isPreferred("notify") ? "ON" : "OFF"),
    );
...
function updateHTML() {
  ...
  if (
    tmp.start &&
    (!tmp.sn.reached || player.supernova.post_10) &&
    displayMainTab
  ) {
    updateQuantumHTML();
    updateDarkHTML();
    ...
    if (tmp.tab_name == "mass") {
      ...
      if (tmp.rank_tab == 0) {
        ...
        tmp.el.massOverflow.setHTML(
          `Because of mass overflow at <b>${formatMass(tmp.overflow_start.mass[0])}</b>, your mass gain is ${overflowFormat(tmp.overflow.mass || 1)}!`,
        );
        ...
        tmp.el.massOverflow2.setHTML(
          `Because of mass overflow^2 at <b>${formatMass(tmp.overflow_start.mass[1])}</b>, your mass overflow is even stronger!`,
        );

        tmp.el.strongerOverflow.setDisplay(
          BUILDINGS.eff("mass_3").gte(tmp.overflow_start.stronger[0]),
        );
        tmp.el.strongerOverflow.setHTML(
          `Because of stronger overflow at <b>${format(tmp.overflow_start.stronger[0])}</b>, your stronger effect is ${overflowFormat(tmp.overflow.stronger || 1)}!`,
        );

        tmp.el.strongerOverflow2.setDisplay(
          BUILDINGS.eff("mass_3").gte(tmp.overflow_start.stronger[1]),
        );
        tmp.el.strongerOverflow2.setHTML(
          `Because of stronger overflow^2 at <b>${format(tmp.overflow_start.stronger[1])}</b>, your stronger overflow is even stronger!`,
        );
      }

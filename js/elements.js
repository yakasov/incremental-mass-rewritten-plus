function setupHTML() {
  ...
  for (let x = 0; x < PRES_LEN; x++) {
    table += `<div style="width: 300px" id="pres_div_${x}">
      ...
      PRESTIGES.fullNames[x]
        ...
        }, but ${PRESTIGES.fullNames[x]} up.<br>
  ...
  for (let x = 0; x < RANKS.names.length; x++) {
    ...
    for (let y = 0; y < keys.length; y++) {
      table += `<span id="ranks_reward_${rn}_${y}"><b>${RANKS.fullNames[x]} ${
        keys[y]
      }:</b> ${RANKS.desc[rn][keys[y]]}${
  ...
  for (let x = 0; x < PRES_LEN; x++) {
    ...
    for (let y = 0; y < keys.length; y++) {
      table += `<span id="pres_reward_${x}_${y}"><b>${PRESTIGES.fullNames[x]} ${
        keys[y]
      }:</b> ${PRESTIGES.rewards[x][keys[y]]}${
...
function updateUpperHTML() {
  ...
  if (mode == "req") {
    ...
    tmp.el.requirement.setHTML(
      `<b class='${u.req ? "green" : "red"}'>[${
        player.supernova.pin_req
      }]</b> ${typeof u.reqDesc == "function" ? u.reqDesc() : u.reqDesc}`
    );
  }
  ...
  if (mode == "upg") {
    ...
    if (nt[0] == "el")
      tmp.el.upg_notify_msg.setHTML(
        `[!] ${
          ["", "Muonic "][nt[1]] + ELEMENTS.fullNames[nt[2]]
        } is available! [!]`
      );
    if (nt[0] == "sn")
      tmp.el.upg_notify_msg.setHTML(
    ...
    if (nt[0] == "zd")
      tmp.el.upg_notify_msg.setHTML(
        `[!] ${capitalFirst(nt[1])} [${nt[2]}] is available! [!]`
      );
    if (nt[0] == "ch")
...
function updateBlackHoleHTML() {
  tmp.el.bhMass2.setHTML(
    formatMass(player.bh.mass) +
      " " +
      formatGain(player.bh.mass, tmp.bh.mass_gain.mul(tmp.qu.speed), true)
  );
    ...
    if (tmp.tab_name == "mass") {
      ...
      if (tmp.ranks.tab == 0) {
        ...
        tmp.el.massOverflow.setHTML(
          ...
          )}</b>, your mass gain is ${overflowFormat(tmp.overflow.mass || 1)}!`
        );

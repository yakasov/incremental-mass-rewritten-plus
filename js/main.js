...
const FORMS = {
  ...
  massSoftPower3() {
    ...
    if (hasElement(77))
      p = p.pow(
        player.qu.rip.active || tmp.c16active || inDarkRun() ? 0.95 : 0.825
      );
    return p;
  },
  massSoftGain4() {
  ...
  tickspeed: {
    ...
    buy() {
      if (this.can()) {
        if (!player.mainUpg.atom.includes(2))
          player.rp.points = player.rp.points.sub(tmp.tickspeedCost).max(0);
    ...
    effect() {
      ...
      eff = step.pow(
        (hasAscension(0, 1) ? t.add(1).mul(bonus.add(1)) : t.add(bonus)).mul(
          hasElement(80) ? 25 : 1
        )
      );
  ...
  rp: {
    gain() {
      ...
      if (player.mainUpg.bh.includes(6))
        gain = gain.mul(tmp.upgs.main ? tmp.upgs.main[2][6].effect : E(1));
  ...
  bh: {
    ...
    massGain() {
      ...
      if (hasElement(162))
        x = x.pow(tmp.stars.effect).pow(tmp.c16active || inDarkRun() ? 5 : 100);
    ...
    set(id) {
      ...
      if (id == "qu") {
        player.reset_msg =
          ...
          formatMass(mlt(1e4)) +
          " of mass to " +
          (QCs.active() ? "complete Quantum Challenge" : "go Quantum");
        return;
      }
    ...
    if ((ooms.gte(10) && amt.gte(1e100)) || (isMass && md == 2)) {
      ...
      if ((md == 0 || md == 3) && isMass) {
        ...
        if (
          ...
          rate = "(+" + format(arv_next.sub(arv_amt).mul(20)) + " arvs/sec)";
      }

...
const FORMS = {
  ...
  massGain() {
    ...
    if (tmp.atom.unl) {
      ...
      x = hasElement(105)
        ? x.pow(tmp.atom.particles[0].powerEffect.eff1)
        : x.mul(tmp.atom.particles[0].powerEffect.eff1);
    }
  ...
  bh: {
    ...
    DM_gain() {
      ...
      if (tmp.sn.boson && !hasElement(204))
        gain = gain.mul(tmp.sn.boson.upgs.photon[0].effect);
      ...
      if (tmp.sn.boson && hasElement(204))
        gain = gain.pow(tmp.sn.boson.upgs.photon[0].effect);
      ...
      if (evo >= 2) {
        ...
        if (CHALS.inChal(7) || CHALS.inChal(10))
          gain = player.mass.add(1).log10().div(50);
    ...
    massGain() {
      ...
      if (tmp.sn.boson)
        x = hasElement(204)
          ...
          : x.mul(tmp.sn.boson.upgs.photon[0].effect);
      if (CHALS.inChal(8) || CHALS.inChal(10) || FERMIONS.onActive("12"))
        x = x.root(8);
      ...
      if (tmp.dark.run) x = expMult(x, mgEff(0));
      if (tmp.star_unl && hasElement(162))
        x = x.pow(tmp.stars.effect[1]).pow(tmp.dark.run ? 5 : 100);
    ...
    formula() {
      return player.bh.mass
        ...
        .overflow(tmp.bh.fSoftStart, tmp.bh.fSoftPower);
    },
    fSoftStart() {
      let x = uni("e3e9");
...
function getARVName(i, lode) {
  const n = MASS_NAMES[mass_type],
    v = VERSES[mass_type][0][i - 1];
  return i > 0
    ? v
      ? v + (!lode && (mass_type == "standard" || i != 1) ? n[8] : "")
      : (lode ? n[9] : n[9] + n[8]) + formatPow(i, 0)
    : "";
}
...
function formatARV(ex, lode) {
  ...
  if (arv.add(1).gte(1000))
    return format(arv.add(1)) + " " + n[9] + (lode ? "s-" : n[8] + "s");
  return (
    format(mlt.div(Decimal.pow(1e15, arv.floor()))) +
    " " +
    getARVName(arv.add(1).floor().toNumber(), lode) +
    (lode ? "-" : "")
  );
}
...
function formatLDV(ex) {
  ...
  return (
    formatARV(ex.iteratedlog(10, ldv_floor).div(1e9), true) +
    "" +
    (v
      ? v + (mass_type == "standard" ? n[8] : "")
      : n[10] + n[8] + formatPow(ldv_floor, 0))
  );
...
function formatGain(a, e, mass) {
  const g = Decimal.add(a, e),
    f = mass ? formatMass : format,
    n = MASS_NAMES[mass_type],
    verse = n[8],
    arch = n[9],
    lode = n[10];
  ...
  if (g.neq(a)) {
    if (mass) {
      if (a.gte("eee9")) {
        var ldv = E(a).slog(10).toNumber() - 1.9542425094393248,
          ldv_floor = Math.floor(ldv);

        if (a.gte(MAX_LDVS)) {
          ldv = E(g).slog(10).sub(E(a).slog(10)).mul(FPS);
          if (ldv.gte(1))
            return "(+" + ldv.format() + " " + lode + verse + "s/s)";
        }

        var sg = E(g).iteratedlog(10, ldv_floor),
          sa = E(a).iteratedlog(10, ldv_floor),
          rate = "";
        ...
        return (
          ...
          "" +
          (v
            ? v + (mass_type == "standard" ? verse : "")
            : lode + verse + formatPow(ldv_floor, 0)) +
          "/s)"
        );
      }
...
function formatReduction(ex, acc) {
  ex = E(ex);
  return format(E(1).sub(ex).mul(100), acc) + "%";
}

function formatPercent(ex, acc) {
  ex = E(ex);
  return format(ex.mul(100), acc) + "%";
}

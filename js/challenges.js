    ...
    if (tmp.stab[3] == 0) {
        ...
        if (player.chal.choosed != 0) {
            ...
            tmp.el.chal_ch_title.setTxt(
                `[${player.chal.choosed}]${CHALS.getScaleName(
                    player.chal.choosed
                )} ${chal.title} [${
...
const CHALS = {
    ...
    exit(auto = false) {
        if (!player.chal.active == 0) {
            if (player.chal.active == 16 && !auto) {
                player.dark.c16.shard = player.dark.c16.shard.add(
                    tmp.c16.shardGain
                );
                player.dark.c16.totalS = player.dark.c16.totalS.add(
                    tmp.c16.shardGain
                );
            }
    ...
    getMax(i) {
        ...
        if (i == 16) {
            ...
            if (hasElement(125) && i >= 9 && i <= 12)
                x = x.add(elemEffect(125, 0));
            if (hasElement(151) && i == 13) x = x.add(75);
            if (hasElement(171) && (i == 13 || i == 14)) x = x.add(100);
            if (hasElement(186) && (i == 13 || i == 14 || i == 15))
                x = x.add(100);
            if (hasElement(196) && (i == 13 || i == 14)) x = x.add(200);
            if (hasPrestige(1, 46) && (i == 13 || i == 14 || i == 15))
                x = x.add(200);
            if (i == 13 || i == 14 || i == 15)
                x = x.add(tmp.dark.rayEff.dChal || 0);
        }
    ...
    1: {
        ...
        effDesc(x) {
            return hasBeyondRank(2, 20)
                ? formatMult(x.scrank) +
                      " later to Supercritical Rank & All Fermions starting, " +
                      formatReduction(x.over) +
                      " weaker to Super Overpower scaling"
                : "+" +
                      format(x.rank, 0) +
                      " later to Super Rank starting, " +
    ...
    7: {
        ...
        effect(x) {
            ...
            let ret = c
                ? Decimal.pow(0.987, x.add(1).log10().root(2))
                : x.mul(2);
    ...
    9: {
        ...
        effect(x) {
            ...
            if (hasElement(31, 1) && tmp.chal)
                ret = ret.pow(tmp.chal.eff[16] || 1);
    ...
    19: {
        ...
        get desc() {
            return `
        You cannot implode into/generate Supernovas, produce star resources, Dark Rays (capped at ${format(
            1e12
        )}), Dark Shadows, Abyssal Blots, or purchase tree upgrades. You are stuck in a Dark Run with 1000 of each glyph (unaffected by weakness). Entering this challenge resets Supernova.
        `;
        },
        reward: `Generate more Supernovas based on completions.<br><span class="yellow">On 10th completion, unlock sixth row of Infinity upgrades.</span>`,
        max: E(100),
        inc: E("1e10"),
        pow: E(3),
        start: E("ee5555"),

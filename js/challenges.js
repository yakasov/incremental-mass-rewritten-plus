...
function updateChalTemp() {
  ...
  tmp.chal.gain =
    player.chal.active != 0
      ? tmp.chal.bulk[player.chal.active]
          .min(tmp.chal.max[player.chal.active])
          .sub(player.chal.comps[player.chal.active])
          .max(0)
          .floor()
      : E(0);
  tmp.chal.canFinish =
...
const CHALS = {
  ...
  inChal(x) {
    return (
      player.chal.active == x ||
      (player.chal.active == 15 && x <= 12) ||
      (player.chal.active == 20 && x <= 19)
    );
  },
  ...
  exit(auto = false) {
    if (!player.chal.active == 0) {
      ...
      if (tmp.chal.canFinish) {
        player.chal.comps[player.chal.active] = player.chal.comps[
          player.chal.active
        ]
          .add(tmp.chal.gain)
          .min(tmp.chal.max[player.chal.active]);
  ...
  1: {
    ...
    effect(x) {
      ...
      let over = Decimal.pow(0.99, x.add(1).log10().root(2)).max(0.5);
      return { rank: rank, tick: tick, scrank, over };
    },
    effDesc(x) {
      return hasBeyondRank(2, 20)
        ? formatMult(x.scrank) +
            " later to Supercritical Rank & all Fermions scaling start, " +
            formatReduction(x.over) +
            " weaker to Super Overpower scaling"
        : "+" +
            format(x.rank, 0) +
            " later to Super Rank scaling start, " +
            format(E(1).sub(x.tick).mul(100)) +
            "% weaker to Super Tickspeed scaling";
    },
  },
  2: {
    unl() {
    ...
    effDesc(x) {
      return (
        ...
        format(x.mul(100)) +
        "%" +
        (x.gte(0.3) ? " <span class='soft'>(softcapped)</span>" : "")
      );
    },
  },
  3: {
    ...
    effDesc(x) {
      return (
        "^" +
        format(x) +
        (x.gte(3) ? " <span class='soft'>(softcapped)</span>" : "")
      );
    },
  },
  4: {
    ...
    title: "Weakened Rage",
    desc: "Rage Power gain is rooted by 10. Additionally, Mass gain softcap starts 100 OoMs earlier.",
  ...
  5: {
    ...
    reward: () =>
      hasAscension(0, 22)
        ? `Supercritical Rank and Ultra Hex scale weaker based on completions.`
        : hasCharger(3)
        ? `Exotic Rank & Tier and Ultra Prestige Level scale weaker based on completions.`
    ...
    effect(x) {
      ...
      let ret = c
        ? Decimal.pow(0.97, x.add(1).log10().root(4))
        : E(0.97).pow(x.root(2).softcap(5, 0.5, 0));
  ...
  6: {
    unl() {
      return (
        player.chal.comps[5].gte(1) || player.supernova.times.gte(1) || quUnl()
      );
    },
    ...
    effDesc(x) {
      return (
        ...
        format(x) +
        "x" +
        (x.gte(0.5) ? " <span class='soft'>(softcapped)</span>" : "")
      );
    },
  },
  7: {
    unl() {
      return (
        player.chal.comps[6].gte(1) || player.supernova.times.gte(1) || quUnl()
      );
    },
    title: "No Rage Powers",
    desc: "You cannot gain Rage Powers. Instead, Dark Matter is gained from Mass at a reduced rate. The Mass gain softcap is stronger.",
    reward: () =>
      (hasPrestige(2, 25)
  ...
  8: {
    unl() {
      return (
        player.chal.comps[7].gte(1) || player.supernova.times.gte(1) || quUnl()
      );
    },
    title: "White Hole",
    ...
    effect(x) {
      if (hasElement(64)) x = x.mul(1.5);
      let ret = hasElement(133)
        ? x.root(1.5).mul(0.025).add(1)
        : x.root(1.75).mul(0.02).add(1);
  ...
  9: {
    unl() {
      return hasTree("chal4");
    },
    title: "No Particles",
    desc: "You cannot assign quarks. Additionally, Mass gain exponent is raised to 0.9th power.",
    ...
    effDesc(x) {
      return "^" + format(x) + softcapHTML(x, 21);
    },
  },
  10: {
    unl() {
    ...
    reward: `The exponent of the RP formula is multiplied by completions (doesn't apply in this challenge).<br><span class="yellow">On first completion, unlock Fermions!</span>`,
    max: E(100),
    inc: E("e2000"),
    pow: E(2),
  ...
  12: {
    ...
    effDesc(x) {
      return "+" + format(x) + softcapHTML(x, 50);
    },
  },
  13: {
    unl() {
    ...
    desc: "Normal Mass and Black Hole gains are set to lg(x)^^1.5.",
    reward: `Increase Dark Rays earned based on completions.<br><span class="yellow">On first completion, unlock more features!</span>`,
    max: E(25),
    inc: E("e2e4"),
    pow: E(8),
    start: uni("e2e5"),
  ...
  16: {
    ...
    desc: `
        • You cannot gain Rage Powers, all matter formulas are disabled, and they generate each other. Red Matter generates Dark Matter.<br>
        ...
        `,
    get reward() {
      return `Improve Hybridized Uran-Astatine.<br><span class="yellow">On first completion, unlock new prestige layer when reaching ${formatMass(
        Decimal.pow(10, Number.MAX_VALUE)
      )} of normal mass.</span>`;
    },
    max: E(1),
    start: E("e1.25e11"),
    ...
    effDesc(x) {
      return "^" + format(x) + x.softcapHTML(3);
    },
  },
  17: {
    unl() {
  ...
  18: {
    ...
    effDesc(x) {
      return formatPercent(x.sub(1)) + " stronger";
    },
  },
  19: {
    unl() {
      return hasElement(280);
    },
  ...
  20: {
    ...
    desc: "You are trapped in C1-19 and dark run with 1500 of all glyphs. Theorems in the Core don't work. This challenge resets main upgrades.",

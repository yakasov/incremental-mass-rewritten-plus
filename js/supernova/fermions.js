const FERMIONS = {
  ...
  bonus(i, j) {
    ...
    if (hasTree("prim3") && j < 6)
      x = x.add(tmp.prim.eff[5][1].min(j > 2 && !hasElement(172) ? 4 : EINF));
  ...
  names: ["quark", "lepton"],
  sub_names: [
    ["Up", "Down", "Charm", "Strange", "Top", "Bottom", "Meta-Quark"],
    [
      "Electron",
      "Muon",
      "Tau",
  ...
  types: [
    [
      ...
      },
      {
        ...
        desc(x) {
          return (
            `x${format(x)} to Relativistic Particles gain` +
            (x.gte("ee3") ? " <span class='soft'>(softcapped)</span>" : "")
          );
        },
      ...
      },
      {
        ...
        desc(x) {
          return (
            `Z<sup>0</sup> Boson's first effect is ${format(x.sub(1).mul(100))}% stronger` +
            (x.gte(5) ? " <span class='soft'>(softcapped)</span>" : "")
          );
        },
        inc: "Mass",
      ...
      },
      {
        ...
        calcTier() {
          let res =
            OURO.evo >= 1
              ? Decimal.pow(10, player.evo.cp.points.root(2))
              : player.rp.points;
        ...
        desc(x) {
          return (
            `4th Photon & Gluon upgrades are ${format(x)}x stronger` +
            (x.gte(1.5) ? " <span class='soft'>(softcapped)</span>" : "")
          );
        },
        get inc() {
          return OURO.evo >= 1 ? "10^CP^0.5" : "Rage Power";
        },
      ...
      {
        get base() {
          return OURO.evo >= 3 ? 1e2 : OURO.evo >= 1 ? 1e8 : 1e10;
        },
        nextTierAt(x) {
        ...
        eff(i, t) {
          let x = i
            ...
            .add(1);
          return x
            .softcap(15, hasPrestige(1, 300) ? 0.55 : 0.5, 0)
            .overflow(1e8, 1 / 3, 0);
        },
        desc(x) {
    ...
    [
      {
        ...
        desc(x) {
          return (
            `Collapsed Stars gain softcap starts ^${format(x)} later` +
            (x.gte(1.5) ? " <span class='soft'>(softcapped)</span>" : "")
          );
        },
        inc: "Quark",
      ...
      },
      {
        ...
        calcTier() {
          let res =
            OURO.evo >= 2
              ? E(2).pow(player.evo.wh.fabric.sqrt())
              : player.bh.mass;
        ...
        eff(i, t) {
          let x = t
            .pow(1.5)
            .add(1)
            .pow(i.add(1).log10().softcap(10, 0.75, 0))
            .softcap(1e6, 0.75, 0);
        ...
        desc(x) {
          return (
            `x${format(x)} to Higgs Bosons & Gravitons gain` +
            (x.gte(1e6) ? " <span class='soft'>(softcapped)</span>" : "")
          );
        },
        get isMass() {
          return OURO.evo < 2;
        ...
        get inc() {
          return OURO.evo >= 2 ? "2^Fabric^0.5" : "Mass of Black Hole";
        },
      ...
      },
      {
        ...
        eff(i, t) {
          let x = hasElement(17, 1)
            ? t.pow(2).mul(i.add(1).log10().add(1)).add(1)
            : t.pow(0.8).mul(0.025).add(1).pow(i.add(1).log10());
      ...
      },
      {
        ...
        desc(x) {
          return (
            `Tier requirement is ${format(x)}x cheaper` +
            (x.gte(1.5) ? " <span class='soft'>(softcapped)</span>" : "")
          );
        },
        inc: "Collapsed Star",
      ...
      },
      {
        ...
        nextTierAt(x) {
          ...
          return E("e1.5e7")
            .pow(t.pow(2))
            .mul("e3.5e8")
            .pow(evo >= 3 ? 1e-5 : evo >= 2 ? 1e-3 : 1);
        },
        calcTier() {
          let evo = OURO.evo;
          let res =
            evo >= 3
              ? E(2).pow(player.evo.proto.star.cbrt())
              : player.atom.points;
          if (res.lt(evo >= 3 ? "e3.5e3" : evo >= 2 ? "e3.5e5" : "e3.5e8"))
            return E(0);
          let x = res
            .root(evo >= 3 ? 1e-5 : evo >= 2 ? 1e-3 : 1)
            .div("e3.5e8")
            .max(1)
            .log("e1.5e7")
            .max(0)
        ...
        desc(x) {
          ...
          return hasCharger(3)
            ? `Meta & Exotic Supernovas scale ${w} weaker`
            : `Pre-Meta-Supernova Scalings are ${w} weaker`;
        },
        get inc() {
          return OURO.evo >= 3 ? "2^Protostars^0.33" : "Atom";
        },
      ...
      },
      {
        ...
        eff(i, t) {
          ...
          let x = c
            ...
            : i.add(1).log10().pow(0.75).div(100).add(1).pow(t.pow(0.75));
  ...
  productF(i) {
    let s = E(1);
    for (let x = 0; x < 6; x++)
      s = s.mul(player.supernova.fermions.tiers[i][x].add(1));
    return s;
  },
};
...
function updateFermionsTemp() {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    for (let x = 0; x < FERMIONS.types[i].length; x++) {
      ...
      tf.maxTier[i][x] =
        typeof f.maxTier == "function" ? f.maxTier() : f.maxTier || EINF;
      tf.tiers[i][x] = f.calcTier().min(tf.maxTier[i][x]);
      tf.effs[i][x] = f.eff(
        player.supernova.fermions.points[i],
        (FERMIONS.onActive("04") && i == 0) ||
          (FERMIONS.onActive("14") && i == 1)
          ? E(0)
          : player.supernova.fermions.tiers[i][x]
              .add(tf.bonuses[i][x])
              .mul(i == 1 ? radBoostEff(16) : 1)
              .mul(i == 0 ? radBoostEff(19) : 1),
      );
    }
  }
}
...
function updateFermionsHTML() {
  ...
  let r = [
    [
      tmp.atom.unl ? player.atom.atomic : E(1),
      tmp.atom.unl ? player.md.particles : E(1),
      player.mass,
      OURO.evo >= 1
        ? Decimal.pow(10, player.evo.cp.points.root(2))
        : player.rp.points,
      tmp.atom.unl ? player.md.mass : E(1),
      BUILDINGS.eff("tickspeed", "eff_bottom"),
      tf.prod[0],
    ],
    [
      player.atom.quarks,
      OURO.evo >= 2 ? E(2).pow(player.evo.wh.fabric.sqrt()) : player.bh.mass,
      tmp.bh.unl ? player.bh.dm : E(1),
      player.stars.points,
      OURO.evo >= 3
        ? E(2).pow(player.evo.proto.star.cbrt())
        : player.atom.points,
      BUILDINGS.eff("tickspeed", "power"),
      tf.prod[1],
    ],
  ];
  for (i = 0; i < 2; i++) {
    tmp.el["f" + FERMIONS.names[i] + "Amt"].setTxt(
      format(player.supernova.fermions.points[i], 2) +
        " " +
        formatGain(
          player.supernova.fermions.points[i],
          tf.gains[i].mul(tmp.preQUGlobalSpeed),
        ),
    );

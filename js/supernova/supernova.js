const SUPERNOVA = {
  ...
  doReset() {
    ...
    let keep = [];
    for (let x = 0; x < player.mainUpg.atom.length; x++)
      if (list_keep.includes(player.mainUpg.atom[x]))
        keep.push(player.mainUpg.atom[x]);
    ...
    for (let x = 0; x < player.atom.elements.length; x++)
      if (
        list_keep.includes(player.atom.elements[x]) ||
        player.atom.elements[x] > 86
      )
        keep.push(player.atom.elements[x]);
    player.atom.elements = keep;
    if (
      hasTree("qu_qol9") &&
      QCs.active() &&
      !player.atom.elements.includes(84)
    )
      player.atom.elements.push(84);
  ...
  starGain() {
    ...
    if (tmp.qu.mil_reached[6])
      x = x.mul(qs.softcap("ee9", 0.01, 0).softcap("ee10", 0.1, 0));
  ...
  req(x = player.supernova.times) {
    ...
    maxlimit = E(1e20)
      ...
      .mul(1e90);
    bulk = E(0);
    if (player.stars.points.div(1e90).gte(1))
      bulk = player.stars.points
        ...
        .root(1.25)
        .mul(ml_fp)
        .scaleEvery("supernova", true, [1, 1, 1, 1, ff])
        .add(1)
        .floor();
  ...
  passiveGain() {
    ...
    let x = player.stars.points
      .add(1e10)
      .log10()
      .log10()
      .pow(hasElement(279) ? 3.3 : 3)
      .sub(1);
...
function calcSupernova(dt) {
  ...
  if (su.fermions.unl) {
    ...
    if (tmp.fermions.ch[0] != 0 || tmp.fermions.ch[1] >= 6)
      if (hasTree("qu_qol8") && !(!hasTree("qu_qol8a") && QCs.active()))
        for (let i = 0; i < 2; i++)
          for (let j = 0; j < w; j++)
            if (j < FERMIONS.getUnlLength()) {
              su.fermions.tiers[i][j] = su.fermions.tiers[i][j].max(
                tmp.fermions.tiers[i][j]
              );
            }
    for (let x = 0; x < 2; x++)
  ...
  if (tmp.radiation.unl) {
    if (!player.qu.en.eth[0])
      su.radiation.hz = su.radiation.hz.add(tmp.radiation.hz_gain.mul(du_gs));
...
function updateSupernovaTemp() {
  ...
  if (tmp.SN_passive) {
    ...
    tmp.supernova.reached = tmp.stars
      ? player.stars.points.gte(tmp.supernova.maxlimit)
      : false;
  }
    ...
    for (let j = 0; j < tmp.supernova.tree_had2[i].length; j++) {
      ...
      if (branch != "")
        for (let x = 0; x < branch.length; x++)
          if (
            !(
              player.supernova.tree.includes(branch[x]) ||
              player.dark.c16.tree.includes(branch[x])
            )
          ) {
            unl = false;
            can = false;
      ...
      if (can && unl && !(c16 && CORRUPTED_TREE.includes(id)))
        tmp.supernova.tree_afford2[i].push(id);
      if (t.effect) {
...
function updateSupernovaEndingHTML() {
  if (
    ...
    tmp.start &&
    player.supernova.times.lte(0) &&
    !player.supernova.post_10
  ) {
    tmp.tab = 5;
    document.body.style.backgroundColor = `hsl(0, 0%, ${
      7 - Math.min(tmp.supernova.time / 4, 1) * 7
    }%)`;

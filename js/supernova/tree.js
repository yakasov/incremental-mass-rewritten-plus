  ...
  ids: {
    ...
    sn2: {
      ...
      effect() {
        ...
        let x = E(2)
          .add(hasTree("sn4") ? tmp.supernova.tree_eff.sn4 : 0)
          .pow(sn);
        return x;
      },
      effDesc(x) {
    ...
    m1: {
      ...
      effDesc(x) {
        return hasElement(164) || hasElement(219)
          ? "^" + format(x)
          : format(x) +
              "x" +
              (x.max(1).log(1e100).gte(1e3)
    ...
    qol7: {
      ...
      req() {
        return player.supernova.times.gte(40);
      },
    ...
    unl1: {
      ...
      req() {
        return player.supernova.times.gte(44);
      },
    ...
    ct4: {
      ...
      req() {
        return tmp.c16active && player.bh.dm.gte(1e300);
      },
      reqDesc() {
    ...
    ct6: {
      ...
      req() {
        return tmp.c16active && player.atom.atomic.gte(1e8);
      },
      reqDesc() {
    ...
    ct11: {
      ...
      req() {
        return tmp.c16active && player.atom.atomic.gte(1e20);
      },
      reqDesc() {
    ...
    ct14: {
      ...
      req() {
        return tmp.c16active && player.atom.atomic.gte(1e144);
      },
      reqDesc() {
...
function hasTree(id) {
  return (
    (player.supernova.tree.includes(id) || player.dark.c16.tree.includes(id)) &&
    !(tmp.c16active && CORRUPTED_TREE.includes(id))
  );
}

function treeEff(id, def = 1) {
  return tmp.supernova.tree_eff[id] || E(def);
}
...
function drawTree() {
  ...
  for (let x in tmp.supernova.tree_had2[tmp.tree_tab]) {
    ...
    if (branch.length > 0 && tmp.supernova.tree_unlocked[id])
      for (let y in branch)
        if (tmp.supernova.tree_unlocked[branch[y]]) {
...
function updateTreeHTML() {
  ...
  for (let i = 0; i < TREE_TAB.length; i++) {
    ...
    if (tmp.tree_tab == i)
      for (let x = 0; x < tmp.supernova.tree_had2[i].length; x++) {

...
const TREE_UPGS = {
    buy(x, auto = false) {
        if (
            ...
            if (this.ids[x].qf)
                player.qu.points = player.qu.points
                    .sub(this.ids[x].cost)
                    .max(0);
    ...
    ids: {
        ...
        sn2: {
            ...
            effect() {
                ...
                if (!hasTree("qu4"))
                    sn = sn.softcap(15, 0.8, 0).softcap(25, 0.5, 0);
                let x = E(2)
        ...
        bh2: {
            ...
            req() {
                return (
                    player.supernova.chal.noBHC &&
                    player.bh.mass.gte("1.5e1.7556e4")
                );
            },
            reqDesc() {
                return `Reach ${format(
        ...
        bs2: {
            ...
            effDesc(x) {
                return (
                    format(x[1]) + "x to Photon, " + format(x[0]) + "x to Gluon"
                );
            },
        },
        bs3: {
        ...
        fn5: {
            ...
            req() {
                return (
                    player.atom.quarks.gte("e12500") && FERMIONS.onActive("10")
                );
            },
            reqDesc() {
        ...
        qu_qol2: {
            ...
            req() {
                for (let x = 0; x < 6; x++)
                    if (player.supernova.fermions.tiers[0][x].gte(1))
                        return false;
        ...
        qu_qol6: {
            ...
            req() {
                for (let x = 0; x < 6; x++)
                    if (player.supernova.fermions.tiers[1][x].gte(1))
                        return false;
...
function drawTreeBranch(num1, num2) {
    ...
    var y1 =
        start.top +
        start.height / 2 -
        (window.innerHeight - tree_canvas.height);
    var x2 =
        end.left +
        end.width / 2 -
        (document.body.scrollWidth - tree_canvas.width) / 2;
    var y2 =
        end.top + end.height / 2 - (window.innerHeight - tree_canvas.height);

const RADIATION = {
  ...
  buyBoost(i) {
    let [cost, bulk, j] = [
      tmp.sn.rad.bs.cost[i],
      tmp.sn.rad.bs.bulk[i],
      Math.floor(i / 2),
    ];
    if (
    ...
    if (unl) {
      ...
      for (let y = 0; y < 2; y++) {
        ...
        tmp.el[id2 + "_btn"].setClasses({
          btn: true,
          rad: true,
          locked: player.supernova.radiation.ds[x].lt(tmp.sn.rad.bs.cost[b]),
        });
      }
      for (let y = 0; y < 3; y++) {
        let lvl = 3 * x + y;
        ...
        tmp.el[id2].setTxt(
          format(tmp.sn.rad.bs.lvl[lvl], 0) +

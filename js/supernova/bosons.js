const BOSONS = {
  ...
  gain: {
    pos_w() {
      let x = E(0.1)
        .mul(tmp.sn.boson.effect.neg_w ? tmp.sn.boson.effect.neg_w[1] : 1)
        .mul(tmp.sn.boson.effect.z_boson ? tmp.sn.boson.effect.z_boson[1] : 1)
        .mul(
          tmp.sn.boson.effect.graviton ? tmp.sn.boson.effect.graviton[0] : 1,
        );
    ...
    neg_w() {
      let x = E(0.1)
        .mul(tmp.sn.boson.effect.pos_w ? tmp.sn.boson.effect.pos_w[1] : 1)
        .mul(tmp.sn.boson.effect.z_boson ? tmp.sn.boson.effect.z_boson[1] : 1)
        .mul(
          tmp.sn.boson.effect.graviton ? tmp.sn.boson.effect.graviton[0] : 1,
        );
    ...
    photon() {
      ...
      x = hasElement(204)
        ? x.pow(
            tmp.sn.boson.upgs.photon[2]
              ? tmp.sn.boson.upgs.photon[2].effect
              : 1,
          )
        : x.mul(
            tmp.sn.boson.upgs.photon[2]
    ...
    gluon() {
      ...
      x = hasElement(204)
        ? x.pow(
            tmp.sn.boson.upgs.gluon[2] ? tmp.sn.boson.upgs.gluon[2].effect : 1,
          )
        : x.mul(
            tmp.sn.boson.upgs.gluon[2] ? tmp.sn.boson.upgs.gluon[2].effect : 1,
          );
    ...
    graviton() {
      let x = E(0.1)
        .mul(tmp.sn.boson.effect.graviton ? tmp.sn.boson.effect.graviton[0] : 1)
        .mul(fermEff(1, 1));
      if (hasTree("ct2")) x = x.mul(treeEff("ct2"));
      if (QCs.active()) x = x.pow(tmp.qu.qc_eff[3]);
      if (hasPrestige(1, 3)) x = x.pow(prestigeEff(1, 3));
  ...
  effect: {
    ...
    z_boson(x) {
      let a = FERMIONS.onActive("14")
        ? E(1)
        : x.add(1).log10().add(1).pow(fermEff(0, 2));
      if (tmp.c16.in) a = a.softcap("e308", 0.01, 0);
      let b = x.add(1).pow(2 / 3);
      return [a, b];
    },
    graviton(x) {
      let a = expMult(x.add(1), 0.5).pow(
        tmp.sn.boson.effect.hb ? tmp.sn.boson.effect.hb[0] : 1,
      );
  ...
  upgs: {
    ...
    buy(id, x) {
      if (tmp.sn.boson.upgs[id][x].can) {
        player.supernova.b_upgs[id][x] = player.supernova.b_upgs[id][x].max(
          tmp.sn.boson.upgs[id][x].bulk,
        );
        if (!hasTree("qol7"))
          player.supernova.bosons[id] = player.supernova.bosons[id]
            .sub(BOSONS.upgs[id][x].cost(tmp.sn.boson.upgs[id][x].bulk.sub(1)))
            .max(0);
      }
    },
    photon: [
      {
        ...
        cost(x) {
          return E(1.5).pow(x.pow(1.25)).mul(10);
        },
        bulk(x = player.supernova.bosons.photon) {
          return x.gte(10)
            ? x.div(10).max(1).log(1.5).root(1.25).add(1).floor()
            : E(0);
        },
        effect(x) {
          let y = hasElement(204)
            ...
            : player.supernova.bosons.photon
                .add(1)
                .pow(x.mul(radBoostEff(7)).pow(0.8).mul(100));
          if (tmp.c16.in) y = overflow(y, 10, 0.5);
          return y;
        },
        effDesc(x) {
          return hasElement(204) ? formatPow(x) : format(x) + "x";
        },
      },
      {
        ...
        cost(x) {
          return E(2).pow(x.pow(1.25)).mul(100);
        },
        bulk(x = player.supernova.bosons.photon) {
          return x.gte(100)
            ? x.div(100).max(1).log(2).root(1.25).add(1).floor()
            : E(0);
        },
        effect(x) {
      ...
      },
      {
        ...
        cost(x) {
          return E(5).pow(x.pow(1.25)).mul(500);
        },
        bulk(x = player.supernova.bosons.photon) {
          return x.gte(500)
            ? x.div(500).max(1).log(5).root(1.25).add(1).floor()
            : E(0);
        },
        effect(x) {
        ...
        effDesc(x) {
          return hasElement(204)
            ...
            : format(x) +
                "x" +
                (x.gte(1e15) ? " <span class='soft'>(softcapped)</span>" : "");
        },
      },
      {
        desc: "All-Star resources gain is boosted by Photon.",
        cost(x) {
          return E(5).pow(x.pow(1.25)).mul(1e5);
        },
        bulk(x = player.supernova.bosons.photon) {
          return x.gte(1e5)
            ? x.div(1e5).max(1).log(5).root(1.25).add(1).floor()
            : E(0);
        },
        effect(i) {
          let x;
          if (hasElement(213)) {
            x = player.supernova.bosons.photon
              .add(1)
              .log10()
              .add(10)
              .log10()
              ...
              .pow(0.75);
            if (hasElement(23, 1))
              x = x.pow(fermEff(0, 3).log10().add(1).root(2));
          } else {
            x = player.supernova.bosons.photon
              .add(1)
              .log10()
              .add(1)
              .pow(i.softcap(8000, 0.1, 0).pow(fermEff(0, 3)).mul(0.5))
              .softcap("ee11", 0.8, 2)
              .softcap("e4e14", hasElement(99) ? 0.785 : 0.75, 2);
        ...
        effDesc(x) {
          return hasElement(213) ? formatPow(x) : format(x) + "x";
        },
      },
      {
        desc: "Boost Fabric.",
        ...
        cost(x) {
          return E(1e5).pow(x.pow(1.5));
        },
        bulk(x = player.supernova.bosons.photon) {
          return x.gte(1) ? x.log(1e5).root(1.5).add(1).floor() : E(0);
        },
      ...
      },
      {
        ...
        cost(x) {
          return E(1e5).pow(x.pow(5).add(1));
        },
        bulk(x = player.supernova.bosons.photon) {
          return x.gte(1e5) ? x.sub(1).log(1e5).root(5).add(1).floor() : E(0);
        },
        effect: (x) => x.add(1),
        effDesc(x) {
          return formatPow(x);
    ...
    gluon: [
      {
        ...
        bulk(x = player.supernova.bosons.gluon) {
          return x.gte(10)
            ? x.div(10).max(1).log(1.5).root(1.25).add(1).floor()
            : E(0);
        },
      ...
      },
      {
        ...
        bulk(x = player.supernova.bosons.gluon) {
          return x.gte(1e5)
            ? x.div(1e5).max(1).log(10).root(1.25).add(1).floor()
            : E(0);
        },
        effect(x) {
          let y = player.supernova.bosons.gluon
            .add(1)
            .log10()
            .add(1)
            .log10()
            .mul(x.pow(fermEff(0, 3)).root(3))
            .div(10)
            .add(1);
          if (!hasPrestige(0, 28))
      ...
      },
      {
        ...
        cost(x) {
          return E(1e5).pow(x.pow(3).add(1));
        },
        bulk(x = player.supernova.bosons.gluon) {
          return x.gte(1e5) ? x.sub(1).log(1e5).root(3).add(1).floor() : E(0);
        },
      ...
      },
      {
        ...
        cost(x) {
          return E(1e10).pow(x.pow(1.5).add(1));
        },
        bulk(x = player.supernova.bosons.gluon) {
          return x.gte(1e10)
            ? x.sub(1).log(1e10).root(1.5).add(1).floor()
            : E(0);
        },
        effect: (x) =>
          expMult(
...
function updateBosonsTemp() {
  ...
  for (let x in BOSONS.names) {
    ...
    if (BOSONS.effect[id])
      tmp.sn.boson.effect[id] = BOSONS.effect[id](player.supernova.bosons[id]);

    if (BOSONS.upgs.ids.includes(id))
      for (let y in BOSONS.upgs[id]) {
        ...
        tmp.sn.boson.upgs[id][y] = {
          ...
          unl,
          effect: upg.effect(
            FERMIONS.onActive("04") || !unl
              ? E(0)
              : player.supernova.b_upgs[id][y],
          ),
        };
        tmp.sn.boson.upgs[id][y].can = player.supernova.bosons[id].gte(
          tmp.sn.boson.upgs[id][y].cost,
        );
      }
  }
}
...
function updateBosonsHTML() {
  ...
  for (let x in BOSONS.names) {
    ...
    tmp.el[id + "_amt"].setTxt(
      format(player.supernova.bosons[id]) +
        " " +
        formatGain(
          player.supernova.bosons[id],
          tmp.sn.boson.gain[id].mul(tmp.preQUGlobalSpeed),
        ),
    );
    if (tmp.sn.boson.effect[id])
      for (let y in tmp.sn.boson.effect[id]) {
        if (y == "2" && id == "pos_w")
          tmp.el[id + "_eff" + y].setHTML(
            hasElement(250)
              ? ",<br>and raise mass gain by " +
                  format(tmp.sn.boson.effect[id][y])
              : "",
          );
    ...
    if (BOSONS.upgs.ids.includes(id))
      for (let y in BOSONS.upgs[id]) {
        ...
        tmp.el[id2 + "_div"].setClasses({
          btn: true,
          full: true,
          b_btn: true,
          locked: !tmp.sn.boson.upgs[id][y].can,
        });
        tmp.el[id2 + "_lvl"].setTxt(format(player.supernova.b_upgs[id][y], 0));
        tmp.el[id2 + "_eff"].setHTML(
          BOSONS.upgs[id][y].effDesc(tmp.sn.boson.upgs[id][y].effect),
        );

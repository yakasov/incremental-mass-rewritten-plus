const CORRUPTED_STAR = {
  calcNextGain(amt, tick) {
    ...
    if (amt.gte(1)) {
      ...
      let rss1 = tmp.cs_reduce_start1.log(2),
        rss2 = tmp.cs_reduce_start2.log(2);
  ...
  effects: {
    ...
    theorem_luck: {
      ...
      eff_desc: (i) =>
        `Increase luck on Theorem Stars 1-4. <h4>${formatMult(i)}</h4>`,
    },
    inf_speed: {
      unl: () => true,
...
function hasCSEffect(i) {
  return tmp.cs_effect[i] !== undefined;
}
function CSEffect(i, def = E(1)) {
  return EVO.isFed("cs_" + i) ? def : (tmp.cs_effect[i] ?? def);
}
...
function updateCSHTML() {
  let cs = player.inf.cs_amount,
    cs_growth = CORRUPTED_STAR.calcNextGain(cs, tmp.cs_speed.div(FPS))
      .div(cs)
      .pow(FPS);
  ...
  for (var [i, eff] of Object.entries(tmp.cs_effect)) {
    let fed = EVO.fed_msg[tmp.evo.fed["cs_" + i]],

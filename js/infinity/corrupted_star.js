const CORRUPTED_STAR = {
  calcNextGain(amt, tick) {
    ...
    if (amt.gte(1)) {
      ...
      let rss1 = tmp.cs.reduce_start1.log(2),
        rss2 = tmp.cs.reduce_start2.log(2);
      let rs1 = amt.gte(rss1),
        rs2 = amt.gte(rss2);
  ...
  effects: {
    ...
    inf_speed: {
      ...
      eff_desc: (i) =>
        `Increase passive IP generation. <h4>${formatMult(i)}</h4>`,
    },
    sn_speed: {
      unl: () => hasElement(38, 1),
      eff: (cs) => cs.add(1).log10().add(1).pow(1.5),
      eff_desc: (i) =>
        `Increase passive Supernova generation. <h4>${formatMult(i)}</h4>`,
    },
    sd_mult: {
      unl: () => EVO.amt >= 4 && hasElement(38, 1),
      eff: (cs) => cs.add(1).log10().div(1).add(1).pow(2),
      eff_desc: (i) =>
        `Increase stardust generation. <h4>${formatMult(i)}</h4>`,
    },
    ea_reward: {
      unl: () => hasElement(43, 1),
    ...
    prim_reduce: {
      unl: () => hasElement(64, 1),
      eff: (cs) =>
        Decimal.pow(0.9, cs.add(1).log10().overflow(10, 0.5).root(2)),
      eff_desc: (i) =>
        `Weaken Primordium Theorem scalings. <h4>^${format(i)}</h4>`,
    },
  },
};
...
function updateCSTemp() {
  ...
  let ss1 = E(1e3).mul(ss_mul),
    ss2 = E(1e10).mul(ss_mul);
...
function hasCSEffect(i) {
  return tmp.cs.eff[i] !== undefined;
}
function CSEffect(i, def = E(1)) {
  return EVO.isFed("cs_" + i) || !tmp.inf_unl ? def : tmp.cs.eff[i] ?? def;
}
  ...
  switch (i) {
    case 0:
      if (player.inf.cs_amount.gte(getCSUpgRequirement(0))) {
        ...
        player.inf.cs_amount = player.inf.cs_amount
          .sub(getCSUpgRequirement(0, bulk.sub(1)))
          .max(0);
      }
      break;
    case 1:
      if (player.inf.points.gte(getCSUpgRequirement(1))) {
        ...
        player.inf.points = player.inf.points
          .sub(getCSUpgRequirement(1, bulk.sub(1)))
          .max(0);
      }
      break;
  }
...
function updateCSHTML() {
  let cs = player.inf.cs_amount,
    cs_growth = CORRUPTED_STAR.calcNextGain(cs, tmp.cs.speed.div(FPS))
      .div(cs)
      .pow(FPS);
  ...
  tmp.el.cs_upg1.setHTML(`
    ...
    `);
  tmp.el.cs_upg1.setClasses({
    btn: true,
    full: true,
    locked: player.inf.cs_amount.lt(cost[0]),
  });
  ...
  tmp.el.cs_upg2.setHTML(`
    ...
    `);
  tmp.el.cs_upg2.setClasses({
    btn: true,
    full: true,
    locked: player.inf.points.lt(cost[1]),
  });

  tmp.el.cs_overflow.setHTML(
    cs_growth.eq(1)
      ? "Corrupted Star's growth is stalled!"
      : cs.gte(tmp.cs.reduce_start1)
      ? `Corrupted Star's Growth is rooted by <b>${Decimal.log(2, cs_growth)
          .mul(tmp.cs.speed)
          .format()}</b>!`
      : ""
  );
  ...
  for (var [i, eff] of Object.entries(tmp.cs.eff)) {
    let fed = EVO.fed_msg[tmp.ouro.fed["cs_" + i]],
      line = CORRUPTED_STAR.effects[i].eff_desc(eff);

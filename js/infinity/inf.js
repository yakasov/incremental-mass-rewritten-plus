const INF = {
  doReset() {
    ...
    for (let i = 0; i < player.atom.elements.length; i++)
      if (player.atom.elements[i] > 218) e.push(player.atom.elements[i]);
    ...
    for (let i = 0; i < player.atom.muonic_el.length; i++)
      if (MUONIC_ELEM.upgs[player.atom.muonic_el[i]].cs)
        e.push(player.atom.muonic_el[i]);
    ...
    for (let x = 0; x < 2; x++)
      for (let y = 0; y < 7; y++) player.supernova.fermions.tiers[x][y] = E(0);
  ...
  level() {
    let s = player.mass
      ...
      .add(1);
    s = s.mul(
      player.dark.c16.bestBH.add(1).log10().div(3.5e6).max(1).log(1.1).add(1)
    );
  ...
  pe: {
    cost(i) {
      return Decimal.pow(1.2, i.scaleEvery("pe")).mul(1000).floor();
    },
    can() {
      return player.inf.points.gte(tmp.peCost);
    },
    buy() {
      if (this.can()) {
    ...
    buyMax() {
      if (this.can()) {
        player.inf.points = player.inf.points
          .sub(this.cost(tmp.peBulk.sub(1)))
          .max(0);
...
function hasInfUpgrade(i) {
  return player.inf.upg.includes(i);
}
    ...
    if (tmp.stab[8] == 0) updateCoreHTML();
      ...
      for (let t in CORE) {
        ...
        if (f.gt(0))
          hh += `<br>${f.format(0)} ${
            ct.title.split(" ")[0]
          } Fragments | ${ct.fragment[1](tmp.fragment_eff[t])}<br>`;
        if (hh != "")
          h +=
          ...
          (hasElement(235)
            ? " " +
              player.inf.points.formatGain(
                player.inf.best.div(1e2).mul(tmp.cs_effect.inf_speed)
              )
            : "")
      );
      ...
      for (let r in INF.upgs) {
        ...
        let unl =
          ...
          (r < 5 || player.chal.comps[19].gte(10));

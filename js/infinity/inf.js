const INF = {
  ...
  doReset() {
    ...
    if (iu15) e.push(218);
    for (let i of unchunkify(player.atom.elements))
      if (i > 218 && i <= 290) e.push(i);

    player.atom.elements = e;
    player.atom.muonic_el = unchunkify(player.atom.muonic_el).filter(
      (x) => MUONIC_ELEM.upgs[x].cs || MUONIC_ELEM.upgs[x].berry,
    );

    for (let x = 1; x <= (hasElement(229) ? 15 : 16); x++)
      player.chal.comps[x] = E(0);
    ...
    // Ouroboric
    if (OURO.unl())
      player.evo = deepUndefinedAndDecimal(
        {
    ...
    if (tmp.atom.unl) {
      ...
      for (let x = 0; x < MASS_DILATION.upgs.ids.length; x++)
        player.md.upgs[x] = E(0);
    }

    if (tmp.sn.unl) {
      player.stars.unls = 0;
      player.stars.generators = [
        E(0),
        E(0),
        E(0),
        E(0),
    ...
    if (hasInfUpgrade(8)) {
      for (let i = 0; i < infUpgEffect(8); i++)
        dark.c16.tree.push(...TREE_IDS[i][5]);
    }
  ...
  level() {
    ...
    if (hasElement(16, 1))
      s = s.mul(player.inf.dim_mass.add(1).log(1e6).add(1));
    return s
      .max(1)
      .root(2)
      .softcap(tmp.inf_level_ss, 1 / 3, 0);
  },
  gain() {
  ...
  upgs: [
    [
      ...
      },
      {
        ...
        effect() {
          ...
          return [
            x.add(1).pow(2).softcap(1e3, 0.5, 0),
            overflow(x.add(1).softcap(10, 0.5, 0), 10, 0.5),
          ];
        },
        effectDesc: (x) =>
          "^" +
    ...
    [
      ...
      },
      {
        ...
        get desc() {
          return `Automate muonic elements and ${OURO.evo >= 3 ? `exotic nebulae` : `muon-catalyzed fusion`}.`;
        },
        cost: E(6e6),
      },
      {
    ...
    [
      ...
      },
      {
        ...
        get cost() {
          return OURO.evo == 3 ? E(1e220) : OURO.evo == 2 ? E(1e204) : E(1e225);
        },
      },
    ],
  ],
  ...
  dim_mass: {
    ...
    effect() {
      let x = player.inf.dim_mass
        .add(1)
        .log10()
        .pow(hasElement(244) ? 2.2 : 2);
...
function hasInfUpgrade(i) {
  return tmp.inf_unl && player.inf.upg.includes(i);
}
...
function buyInfUpgrade(r, c) {
  ...
  if (
    player.inf.points.gte(cost) &&
    player.inf.theorem.gte(INF.upg_row_req[r])
  ) {
...
function infUpgEffect(i, def = 1) {
  return tmp.iu_eff[i] || def;
}
...
function updateInfHTML() {
  if (tmp.tab_name == "tp") {
      ...
      for (let i = 0; i < MAX_STARS; i++) {
        if (s[i].gt(0)) {
          ...
          hh +=
            ...
            " | " +
            (desc || "???.") +
            ` <b class='sky'>(${ct.effDesc[i](ctmp[i])})</b><br>`;
        }
      }
      let f = player.inf.fragment[t];
      if (f.gt(0))
        hh += `<br>${f.format(0)} ${ct.title.split(" ")[0]} Fragments | ${ct.fragment[1](tmp.fragment_eff[t])}<br>`;
      if (hh != "")
        h +=
          `<h2>${ct.title} <b>(${format(core_tmp[t].total_p.mul(100), 0)}%)</b></h2><br>` +
          hh +
          "<br>";
    }
        ...
        (hasElement(235)
          ? " " +
            player.inf.points.formatGain(
              player.inf.best.div(1e2).mul(CSEffect("inf_speed")),
            )
          : ""),
    );
    ...
    for (let r in INF.upgs) {
      ...
      let unl =
        (r == 0 || player.inf.theorem.gte(INF.upg_row_req[r - 1])) &&
        (r < 5 || player.chal.comps[19].gte([10, 4, 2, 3][OURO.evo]));
      ...
      let ru = INF.upgs[r],
        req = player.inf.theorem.gte(INF.upg_row_req[r]);
      ...
      for (let c in ru) {
        ...
        if (el) {
          ...
          el.setClasses({
            inf_upg: true,
            locked: !b && (player.inf.points.lt(u.cost) || !req),
            bought: b,
          });

          tmp.el[`iu_${id}_desc`].setHTML(
            b
              ? u.effectDesc
                ? "<br>Effect: " + u.effectDesc(infUpgEffect(id))
                : ""
              : "<br>Cost: <b>" + u.cost.format(0) + "</b> Infinity Points",
          );
        }
      }
    }

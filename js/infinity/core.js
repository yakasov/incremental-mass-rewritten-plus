const CORE = {
  mass: {
    ...
    res: `Normal Mass`,
    boost: () =>
      player.mass.add(1).log10().add(1).log10().add(1).log10().add(1),
    eff: [
      (s) => {
      ...
      (s) => {
        if (EVO.amt >= 2) s = s.add(1).log10().div(5).add(1).pow(-0.3);
        if (EVO.amt < 2) {
      ...
      (s) => E(0.95).pow(s.add(1).log10().root(2)),
      (s) => E(0),
    ],
    effDesc: [
      (x) => formatPow(x),
  ...
  atom: {
    ...
    res: `Exotic Atom`,
    boost() {
      return tmp.ea.amount.add(1).log10().add(1).log10().add(1);
    },
    eff: [
      (s) => {
        let x = s.add(1).pow(s.root(2));
      ...
      (s) => {
        if (EVO.amt >= 4) return s.div(tmp.c16.in ? 1e5 : 1e4).min(0.11);
  ...
  proto: {
    ...
    eff: [
      (s) => s.add(1).log10().div(2).add(1).mul(1.25),
      (s) => Decimal.pow(1.25, s.add(1).log10()),
      (s) => s.add(1).log10().div(100).add(1).mul(1.25).pow(-1),
  ...
  time: {
    ...
    preEff: [
      ...
      () =>
        EVO.amt >= 4
          ? `Raise Dark Effect resources.`
          : `Raise Dark Shadow & Abyssal Blots.`,
      `Weaken each glyphic mass nerfing.`,
      () =>
        EVO.amt >= 3
          ? `Boost the softcap of quark's formula from protostars starting.`
    ...
    eff: [
      ...
      (s) =>
        EVO.amt >= 4
          ? s.div(400).max(2).log(2).sqrt()
          : s.add(1).log10().div(100).add(1),
      (s) => s.add(1).log10().div(100).add(1).mul(1.25).pow(-1),
      (s) =>
        EVO.amt >= 3 ? expMult(s.add(1), 1.5) : s.add(1).log10().root(2).div(5),
      (s) => s.add(1).root(3),
    ...
    fragment: [
      (f) =>
        f.overflow(1e100, 0.5).add(1).log10().root(2).div(50).add(1).pow(-1),
      (x) =>
...
function setupCoreHTML() {
  ...
  for (let i = 0; i < 4; i++)
    h += `<div id="preT${i}" class="theorem_div tooltip" tooltip-pos="bottom" onclick="choosePreTheorem(${i})"></div>`;
  new Element("pre_theorem").setHTML(h);

  h = ``;
  for (let i = 0; i < MAX_CORE_LENGTH; i++)
    h += `<div id="coreT${i}" class="theorem_div tooltip" onclick="chooseTheorem('${i}',true)"></div>`;
...
function savageTheorem() {
  if (t_chosen.includes("c") || t_chosen == "-") return;
  player.inf.savage = player.inf.inv[t_chosen];
  delete player.inf.inv[t_chosen];
...
function createPreTheorem() {
  ...
  while (CORE[t].unl ? !CORE[t].unl() : false)
    t = CORE_TYPE[Math.randomInt(0, CORE_TYPE.length)];

  let sav = player.inf.savage ?? {},
    c = [],
    min_pow = sav.power || E(1);
  while (c.length == 0) {
    let m = [],
      n = false;
    for (let i = 0; i < MAX_STARS; i++) {
...
function addSelectedTheorem(onInf) {
  if (!onInf && player.inf.pt_chosen == -1) return;
  if (onInf) {

const CORE = {
  mass: {
    ...
    res: `Normal Mass`,
    boost() {
      return player.mass.add(1).log10().add(1).log10().add(1).log10().add(1);
    },
    eff: [
      (s) => {
      ...
      (s) => {
        if (OURO.evo >= 2) s = s.add(1).log10().div(5).add(1).pow(-0.3);
        if (OURO.evo < 2) {
  ...
  bh: {
    ...
    fragment: [
      ...
      (x) =>
        `Raise mass of unstable black hole gain to the <b>${format(x)}</b>th power.`,
    ],
  },
  atom: {
  ...
  time: {
    ...
    fragment: [
      ...
      (x) =>
        `Weaken beyond rank’s next tier requirement by <b>${formatReduction(x)}</b>.`,
    ],
  },
};
...
function getPowerMult(lvl = tmp.core_lvl) {
  return lvl.sub(1).floor().root(2).div(100).mul(CSEffect("power_mult"));
}
...
var changeCoreFromBestLevel = () => {
  let lvl = Decimal.floor(tmp.core_lvl),
    power = getPowerMult(tmp.core_lvl).mul(100).add(100).round().div(100);
...
function getTheoremPreEffects(data, s, p, level) {
  ...
  if (tmp.tfUnl)
    e += `<br class='line'>Form into <b>+${format(calcFragmentBase(data, s, p, level), 0)}</b> fragment base`;
  return e;
}
...
function updateCoreHTML() {
  ...
  if (TS_visible) {
    ...
    tmp.el.preTReq.setHTML(
      `Reach over <b>${formatMass(INF.req)}</b> of normal mass to show theorems that you will choose.`,
    );
  }

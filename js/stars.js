    ...
    effect() {
        ...
        if (hasElement(162)) {
            ...
            let [s, r, t1, t2, t3] = [
                ...
                player.ranks.tetr
                    .softcap(30000, 0.15, 0)
                    .mul(p)
                    .softcap(5, hasTree("s2") ? 1.5 : 5, 1)
                    .softcap(9, 0.3, 0),
        ...
        unl() {
            if (player.atom.quarks.gte(tmp.stars.generator_req)) {
                ...
                tmp.stars.generator_req =
                    player.stars.unls < tmp.stars.max_unlocks
                        ? STARS.generators.req[player.stars.unls]
                        : EINF;
            }
        },
        gain(i) {
            ...
            if (hasTree("s1") && i == tmp.stars.max_unlocks - 1)
                x = x.mul(tmp.supernova.tree_eff.s1);
            if (player.md.upgs[8].gte(1)) x = x.mul(tmp.md.upgs[8].eff);
            if (hasElement(54)) x = x.mul(tmp.elements.effect[54]);
            x = x.mul(BUILDINGS.eff("star_booster"));
            x = hasElement(213)
                ? x.pow(tmp.bosons.upgs.photon[3].effect)
                : x.mul(tmp.bosons.upgs.photon[3].effect);
            if (hasPrestige(1, 1)) x = x.pow(2);
...
function updateStarsScreenHTML() {
    ...
    if (
        ...
        if (g.gte(1) && player.supernova.post_10) {
            ...
            percent = player.stars.points
                .div(e)
                .max(1)
                .log10()
                .div(d.div(tmp.supernova.maxlimit).max(1).log10())
                .max(0)
                .min(1)
                .toNumber();
        } else
            percent = player.stars.points
                .max(1)
                .log10()
                .div(tmp.supernova.maxlimit.max(1).log10())
                .max(0)
                .min(1)
                .toNumber();
        let size =
            Math.min(window.innerWidth, window.innerHeight) * percent * 0.9;
...
function updateStarsHTML() {
    ...
    tmp.el.stars_Amt.setTxt(
        format(player.stars.points, 2) +
            (tmp.SN_passive ? "" : " / " + format(tmp.supernova.maxlimit, 2)) +
            " " +
            formatGain(
                player.stars.points,
    ...
    tmp.el.star_btn.setClasses({
        btn: true,
        locked: player.atom.quarks.lt(tmp.stars.generator_req),
    });

    for (let x = 0; x < 8; x++) {
        let unl = player.stars.unls > x;
        tmp.el["star_gen_div_" + x].setDisplay(unl);
        if (tmp.el["star_gen_arrow_" + x])
            tmp.el["star_gen_arrow_" + x].setDisplay(unl);
        if (unl)
            tmp.el["star_gen_" + x].setHTML(
                format(player.stars.generators[x], 2) +
                    "<br>" +
                    formatGain(

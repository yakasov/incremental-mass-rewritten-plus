const FORMATS = {
    omega: {
        ...
        format(value) {
            ...
            const omegaAmount = Decimal.floor(
                step.div(this.config.greek.length),
            );
            let lastLetter =
                this.config.greek[step.toNumber() % this.config.greek.length] +
                toSubscript(value.toNumber() % 1000);
            const beyondGreekArrayBounds =
                this.config.greek[
                    step.toNumber() % this.config.greek.length
                ] === undefined;
            if (
    ...
    elemental: {
        ...
        getAbbreviation(group, progress) {
            ...
            return elem > 118
                ? this.beyondOg(elem)
                : this.config.element_lists[group - 1][elemRel];
        },
        beyondOg(x) {
        ...
        getAbbreviationAndValue(x) {
            ...
            const abbreviationLength = this.abbreviationLength(
        ...
        format(ex, acc) {
            ...
            if (e.lt(9)) {
                ...
                return (
                    (e.log10().gte(9)
                        ? ""
                        : m.toFixed(E(4).sub(e.sub(e.div(3).floor().mul(3))))) +
                    "e" +
    ...
    layer: {
        ...
        format(ex, acc, max) {
            ...
            return (
                ...
                (meta.gte(1)
                    ? "meta" +
                      (meta.gte(2) ? "^" + format(meta, 0, max, "sc") : "") +
                      "-"
                    : "") +

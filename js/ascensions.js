const ASCENSIONS = {
    ...
    noReset: [() => hasElement(267), () => false],
    autoUnl: [() => hasElement(267), () => false],
    autoSwitch(x) {
...
function setupAscensionsHTML() {
    ...
    for (let x = 0; x < ASCENSIONS.names.length; x++) {
        ...
        for (let y = 0; y < keys.length; y++) {
            table += `<span id="asc_reward_${x}_${y}"><b>${
                ASCENSIONS.fullNames[x]
            } ${keys[y]}:</b> ${ASCENSIONS.rewards[x][keys[y]]}${
    ...
    for (let x = 0; x < ASCENSIONS.names.length; x++) {
        ...
        if (player.asc_reward == x) {
            ...
            for (let y = 0; y < keys.length; y++) {
                ...
                if (unl) {
                    tmp.el["asc_reward_" + x + "_" + y].setClasses({
                        corrupted_text2: false,
                    });

...
function updateTooltips() {
    ...
    if (hover_tooltip) {
        ...
        if (attr_html == "" || !hover_tooltip.className.includes("tooltip")) {
            ...
            let attr_align =
                    hover_tooltip.getAttribute("tooltip-align") ||
                    TOOLTIP_CONFING.def_align,
                attr_pos =
                    hover_tooltip.getAttribute("tooltip-pos") ||
                    TOOLTIP_CONFING.def_pos,
                text_align =
                    hover_tooltip.getAttribute("tooltip-text-align") ||
                    TOOLTIP_CONFING.def_text_align;
            ...
            else if (attr_pos == "top")
                dy = ht_rect.top - t_rect.height - 8 * ts;
            else if (attr_pos == "left")
                dx = ht_rect.left - t_rect.width - 8 * ts;
            else if (attr_pos == "right") dx = ht_rect.right + 8 * ts;

            if (attr_pos == "left" || attr_pos == "right") {
                if (attr_align == "left" || attr_align == "start")
                    dy = ht_rect.top;
                else if (attr_align == "center")
                    dy = ht_rect.top + (ht_rect.height - t_rect.height) / 2;
                ...
                if (attr_align == "left" || attr_align == "start")
                    ...
                    dx = ht_rect.left + (ht_rect.width - t_rect.width) / 2;
                else if (attr_align == "right" || attr_align == "end") {

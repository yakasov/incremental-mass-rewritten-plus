...
function removeNotify() {
  if (
    tmp.saving > 0 && tmp.notify[0]
      ? (tmp.notify[0].text = "Game Saving")
      : false
  )
    tmp.saving--;
...
function convertStringIntoAGY(s) {
  let ca = ` abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,?!/<>@#$%^&*()_-+=~№;:'"[]{}|`,
    cl = BigInt(ca.length),
    r = 0n,
    sd = SEED[0],
    result = "";
  ...
  for (let i = BigInt(s.length) - 1n; i >= 0n; i--) {
    let q = BigInt(ca.indexOf(s[i])),
      w = q >= 0n ? cl ** (BigInt(s.length) - i - 1n) * q : 0;

    if (i % 2n == 0n && w % 3n == i % ((w % 4n) + 2n))
      w *= ((w % 4n) + 2n) * (i + 1n);
...
function hideNavigation(i) {
  player.options.nav_hide[i] = !player.options.nav_hide[i];
  updateNavigation();
}

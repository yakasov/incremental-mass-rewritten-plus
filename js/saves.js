...
Decimal.prototype.scale = function (s, p, mode, rev = false) {
  ...
  if (x.gte(s)) {
    if ([0, "pow"].includes(mode))
      x = rev ? x.div(s).root(p).mul(s) : x.div(s).pow(p).mul(s);
    ...
    if ([3, "alt_exp"].includes(mode))
      x = rev
        ? x.div(s).max(1).log(p).add(1).mul(s)
        : Decimal.pow(p, x.div(s).sub(1)).mul(s);
  }
  return x;
};
...
Decimal.prototype.scaleName = function (type, id, rev = false, type_index) {
  ...
  if (SCALE_START[type][id] && SCALE_POWER[type][id]) {
    ...
    x = x.scale(
      s,
      e,
      type_index % 4 == 3 ? (type_index >= 7 ? 3 : 1) : type_index >= 6 ? 2 : 0,
      rev
    );
  }
  return x;
...
function softcapHTML(x, start, invisible = false) {
  return !invisible && E(x).gte(start)
    ? ` <span class='soft'>(softcapped)</span>`
    : "";
}

Decimal.prototype.softcapHTML = function (start, invisible) {
  return softcapHTML(this.clone(), start, invisible);
};

function calcOverflow(x, y, s, height = 1) {
  return x.gte(s)
    ? x.max(1).iteratedlog(10, height).div(y.max(1).iteratedlog(10, height))
    : E(1);
}

String.prototype.corrupt = function (active = true) {
  return active
    ? this.strike() + ` <span class='corrupted_text'>[Corrupted]</span>`
    : this;
};
...
function calc(dt) {
  ...
  if (tmp.pass <= 0 && tmp.inf_time == 0) {
    ...
    for (let x = 0; x < RANKS.names.length; x++) {
      ...
      if (
        (tmp.brUnl && x < 4) ||
        (RANKS.autoUnl[rn]() && player.auto_ranks[rn])
      )
        RANKS.bulk(rn);
    }
    if (player.auto_ranks.beyond && (hasBeyondRank(2, 1) || hasInfUpgrade(10)))
      BEYOND_RANKS.reset(true);
    for (let x = 0; x < PRES_LEN; x++)
      if (PRESTIGES.autoUnl[x]() && player.auto_pres[x])
        PRESTIGES.reset(x, true);
    for (let x = 0; x < ASCENSIONS.names.length; x++)
      if (ASCENSIONS.autoUnl[x]() && player.auto_asc[x])
        ASCENSIONS.reset(x, true);
      ...
      if (upg.auto_unl ? upg.auto_unl() : false)
        if (player.auto_mainUpg[id])
          for (let y = 1; y <= upg.lens; y++)
            if (upg[y].unl ? upg[y].unl() : true) upg.buy(y);
    }
    if (player.mainUpg.bh.includes(6) || player.mainUpg.atom.includes(6))
    ...
    if (hasElement(43))
      for (let x = 0; x < MASS_DILATION.upgs.ids.length; x++)
        if (
          (hasTree("qol3") || player.md.upgs[x].gte(1)) &&
    ...
    if (player.bh.unl && !player.qu.en.hr[0])
    ...
    if (hasTree("qol1"))
      for (let x = 1; x <= (player.dark.unl ? 118 : 117); x++)
        if (x <= tmp.elements.upg_length) ELEMENTS.buyUpg(x);
    ...
    if (tmp.SN_passive)
      ...
      player.supernova.times = player.supernova.times.max(tmp.supernova.bulk);
...
function getPlayerData() {
  ...
  for (let x in BOSONS.upgs.ids)
    for (let y in BOSONS.upgs[BOSONS.upgs.ids[x]])
      s.supernova.b_upgs[BOSONS.upgs.ids[x]][y] = E(0);
  for (let x = 0; x < 7; x++) {
...
function loadPlayer(load) {
  ...
  if (
    ...
    player.dark.c16.shard.gt(0)
  )
    player.dark.c16.totalS = player.dark.c16.shard;
  for (i = 0; i < 2; i++)
    for (let x = 0; x < FERMIONS.types[i].length; x++) {
      let f = FERMIONS.types[i][x];
      player.supernova.fermions.tiers[i][x] = player.supernova.fermions.tiers[
        i
      ][x].min(
        typeof f.maxTier == "function" ? f.maxTier() : f.maxTier || 1 / 0
      );
    }
  ...
  for (let k in obj) {
    if (typeof obj[k] == "string") {
      if (
        ...
        if (isNaN(E(obj[k]).mag)) obj[k] = data[k];
    } else {
      if (typeof obj[k] != "object" && isNaN(obj[k])) obj[k] = data[k];
      if (typeof obj[k] == "object" && data[k] && obj[k] != null)
        obj[k] = deepNaN(obj[k], data[k]);
    }
  }
  return obj;
}
...
function deepUndefinedAndDecimal(obj, data) {
  ...
  for (let k in data) {
    ...
    if (obj[k] === undefined) obj[k] = data[k];
      ...
      if (Object.getPrototypeOf(data[k]).constructor.name == "Decimal")
        obj[k] = E(obj[k]);
      else if (typeof obj[k] == "object")
        deepUndefinedAndDecimal(obj[k], data[k]);
    }
  }
  return obj;
}
...
function convertStringToDecimal() {
  for (let x = 1; x <= CHALS.cols; x++)
    player.chal.comps[x] = E(player.chal.comps[x]);
  for (let x = 0; x < MASS_DILATION.upgs.ids.length; x++)
    player.md.upgs[x] = E(player.md.upgs[x] || 0);
  for (let x = 0; x < MASS_DILATION.break.upgs.ids.length; x++)
    player.md.break.upgs[x] = E(player.md.break.upgs[x] || 0);
  for (let x in BOSONS.upgs.ids)
    for (let y in BOSONS.upgs[BOSONS.upgs.ids[x]])
...
function cannotSave() {
  return (
    (tmp.supernova.reached && player.supernova.times.lt(1) && !quUnl()) ||

    ...
    if (tmp.pass <= 0 && tmp.inf_time == 0) {
        ...
        if (hasTree("qol7")) {
            for (let x = 0; x < BOSONS.upgs.ids.length; x++) {
                ...
                for (let y = 0; y < BOSONS.upgs[id].length; y++)
                    BOSONS.upgs.buy(id, y);
            }
        }
        ...
        if (typeof obj[k] == "string") {
            if (
                data[k] == null || data[k] == undefined
                    ? false
                    : Object.getPrototypeOf(data[k]).constructor.name ==
                      "Decimal"
            )
...
function exporty() {
    ...
    a.download =
        "Incremental Mass Rewritten Save - " +
...
function findNaN(obj, str = false, data = getPlayerData(), node = "player") {
    ...
    for (let k in obj) {
        if (typeof obj[k] == "number")
            if (isNaNed(obj[k])) return node + "." + k;
        if (str) {
            if (typeof obj[k] == "string")
                if (
                    data[k] == null || data[k] == undefined
            ...
            if (
                obj[k] == null || obj[k] == undefined
                    ? false
                    : Object.getPrototypeOf(obj[k]).constructor.name ==
                      "Decimal"
            )
                if (isNaN(E(obj[k]).mag)) return node + "." + k;
        }
...
function simulateTime(sec) {
    ...
    let s = {
        ...
        bh_mass: player.bh.mass
            .max(1)
            .div(player_before.bh.mass.max(1))
            .log10(),
